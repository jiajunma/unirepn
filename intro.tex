...
\item
tailed in $\check \CO$,  if $\mathbf r_i(\check \CO)>\mathbf r_{i+1}(\check \CO)$ and $\mathbf r_i(\check \CO)-\mathbf r_{i+1}(\check \CO)$ is odd;
\item
primitive in $\check \CO$, if   $\mathbf r_i(\check \CO)>\mathbf r_{i+1}(\check \CO)$ and $\mathbf r_i(\check \CO)-\mathbf r_{i+1}(\check \CO)$ is even.
\end{itemize}
...
Write $\mathrm{PBP}$ for the set of all painted bipartitions. For every $\tau\in \mathrm{PBP}$ as in Definition \ref{defpbp0}, we write 
\[
...
\[
  \star_\tau:= \left\{
   ...
   \right.
  \]
For every  $\wp\subset \mathrm{PP}_\star(\check \CO)$, write
\[
\mathrm{PBP}_\star(\check \CO, \wp):=\Set{
\tau \in \mathrm{PBP}| \star_\tau = \star \text{ and } (\imath_\tau,\jmath_\tau) = (\imath_\star(\check \CO,  \wp), \jmath_\star(\check \CO,  \wp))
}
\]
 Put
\[
\PBP_\star(\ckcO) := \bigsqcup_{\wp\subset \mathrm{PP}_\star(\ckcO)} \PBP_\star(\ckcO,\wp).
\]
We say that a painted bipartition has good parity if it belongs to the subset 
\[
   \bigsqcup_{(\star, \check \CO)}  \mathrm{PBP}_\star(\check \CO)\subset \mathrm{PBP},
\]
where $\star$ runs over the set $\{ B,\mathrm  C,  D, \widetilde{C},  C^*, D^*\}$, and $\check \CO$ runs over all Young diagrams that have $\star$-good parity.

\delete{
 \begin{defn}\label{defpbp00}
Let $\wp\subset \mathrm{PP}_\star(\check \CO)$. A painted bipartition  $\tau=(\imath, \CP)\times (\jmath, \CQ)\times \alpha$ has shape  $(\check \CO, \wp)$ if 
\[
  \star=\left\{
     \begin{array}{ll}
         B, &\hbox{if $\alpha=B^+$ or $B^-$}; \smallskip\\
...
and
\[
  (\imath,\jmath)=(\imath_\star(\check \CO,  \wp), \jmath_\star(\check \CO,  \wp)).
\]
...
% With the notation of Definition \ref{defpbp00},
With the notation of \Cref{defpbp0} and the assumption  that $\tau$ has good parity,  we define some objects attached to $\tau$ in what follows:
 \[
   \check \CO_\tau,\ \wp_\tau,\   \abs{\tau}, \  \varepsilon_\tau, \  p_\tau, \ q_\tau, \ G_\tau\, \ \dim \tau.
 \]
\delete{
...
%\cite[Proposition 6.9]{RT} 
 ...
 \noindent $\abs{\tau}$: This is the natural number \[
  \abs{\tau}:=\abs{\imath_\tau}+\abs{\jmath_\tau}.
\]
Note that
\[
 \abs{\tau}:= \left\{
...
We will calculate the associated character of $\pi_\tau$ for every painted bipartition $\tau$ that has good parity. The calculation is a key ingredient in the proof of Theorem \ref{thm1}.
