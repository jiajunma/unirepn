...
\]
 and 
\[
\mathrm{PBP}'_\star(\check \CO):=\begin{cases}
...
\[
  \mathrm{Unip}_{\star}(\check \CO):=\left\{
   ...
   \right.
\]
We will use the set $\mathrm{PBP}'_\star(\check \CO)$ to parameterize the set $\mathrm{Unip}_{\star}(\check \CO)$.
...
Let $\star$ and $\check \CO$ be as before. For every $(\tau,\wp)\in \mathrm{PBP}_\star(\check \CO)$, in what follows we will construct a representation $\pi_{\tau,\wp}$ of $G_\tau$ by theta lift.
...
\end{equation}
 ...
 \[
   \pi_{\tau,\wp}:=\left\{
     \begin{array}{ll}
         ...
         \check \Theta_{\tau'}^{\tau}(\pi_{\tau',\wp'})\otimes (1_{p_\tau, q_\tau}^{+,-})^{\varepsilon_{\tau}}, &\hbox{if  $\star_\tau=B$ or $D$}; \\
         \check \Theta_{\tau'}^{\tau}(\pi_{\tau',\wp'}\otimes \det^{\varepsilon_{\wp}}), &\hbox{if $\star_\tau=C$ or $\widetilde C$}; \\
              \check \Theta_{\tau'}^{\tau}(\pi_{\tau',\wp'}), &\hbox{if $\star_\tau=C^*$ or $D^*$}. \\
            \end{array}
   \right.
 \]
 ...
 \[
  \varepsilon_\tau=1\Leftrightarrow  (1,2)\in \wp. 
\].
...
\noindent (a) For every $(\tau,\wp)\in \mathrm{PBP}'_\star(\check \CO)$, the representation $\pi_{\tau,\wp}$ of $G_\tau$ is irreducible and attached to $\check \CO$.
