...
For a nilpotent orbit $\ckcO\in \Nil(\GL_{n}(\bC))$, we set % its Barbasch-Vogan dual is
\[
  \cO:=\dBV(\ckcO) = \cO^{t} \in \Nil(\GL_{n}(\bC)).
\]


We always decompose $\ckcO$ into even parts and odd parts and write %$\ckcO\in \Nil(\ckGc)$ and decompose
\[
...
The general case is also well known to the experts:%, see \cite{V.GL}.
