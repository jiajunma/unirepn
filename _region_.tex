\message{ !name(counting_main.tex)}% !TeX program = xelatex
...
\message{ !name(counting_A.tex) !offset(-1015) }
...
\begin{thm}
  Retain the above setting.
  Then
  \begin{enumearte}[label=(\alph*)]
    \item $\Unip_{G}(\ckcO)\neq \emptyset$  only if $p,q \geq \half \nbb$ and
    $p+q$ is even  when $G = \wtU(p,q)$.
    \item In that case
  \end{enumearte}
  Suppose $\min(p,q)< \fnn$
  $\Unip_{}$
...
\message{ !name(counting_main.tex) !offset(-157) }
