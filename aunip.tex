...
Let $\uptau=(\tau,\wp)\in \PBP_\star(\ckcO)$, $\uptau' := (\tau',\wp'):=\DD(\uptau)$ and  $\ckcOp:=\ckDD(\ckcO)$.

\subsection{Translate to combinatorics problem}
...
associated cycle $\AC(\uptau)$, see \Cref{sec:MYD}. 
...
We have the following more explicit formulas of $\ac_{\uptau}$.
...
We say an element $\cA$ in $\bZ[\MYD_\star]$ is mulitplicity free if $\sF(\cA)$ is a sum of 
distinct elements in $\SYD_\star$ where $\sF$ is defined in \eqref{eq:F}. 

The following lemma implies the non-vanishing and multiplicity freeness of
$\AC(\tau)$ see \Cref{thmac1}. 
...
\Cref{sec:ac} holds for $\ckcO'$. 

Suppose $\sum_{j=1}^{b} m_{j} \cE_{j}\in \bZ[\MYD_{\star}]$ such that $m_{j}\neq
0$ and $\cE_{i}\in \MYD_{\star}$. We define the ``descent signature'' of
...
multiplicity free, $\ac_{\uptau}\neq 0$ and mulitplicity free by \eqref{eq:C}.
...
are the same for $\uptau_{1}$ and $\uptau_{2}$. By \eqref{eq:C}, we get  
...
We now prove the multilicity freeness of $\ac_{\uptau}$.
...
\subsubsection*{Proof of \Cref{lem:C}~(b)}
By consider the $\dsign$,  we get $\pac{\uptau_1}=\pac{\uptau_2}$
