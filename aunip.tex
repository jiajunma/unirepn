...
\begin{enuma}
  ...
  \begin{equation}\label{eq:BD2}
    \ac_{\uptau} =
    \big((\maltese^t (\pac{\uptaupp}\cdot (0,0)) \cdot \pcT_{\uptau}
...
element such that $\DD(\wp) = \wp'$. Then $\uptau = (\tau, \wp)$ is the anxiety
element. 
...
\begin{equation}
 \label{eq:BD4} 
  ...
  \end{split}
\end{equation}
Recall that $k = \frac{\bfrr_1(\ckcO)-\bfrr_2(\ckcO)}{2}+1$, see \Cref{sec:tail}. 
...
$\Upsilon^+_{\ckcO''}$ is injective \Cref{lem:BD3}~(b). So  $\ac_{\uptaupp_{1}}
= \ac_{\uptaupp_{2}}$.  
...
This finished the proof of \Cref{lem:BD}~(b).
...
We first prove the injectivity of $\Upsilon^{+}_{\ckcO}$. Note that
  $\ac_{\uptau}\sqsupseteq (2,0)$ is equivalent to $\pac{\uptau}\sqsupseteq
  (1,0)$. We clearly have the following  bijection
  \[
  ...
  \]
  since  $\ac_{\uptau})\lhd (1,0)$ for each element in the domain by
  \eqref{eq:BD2}.
  % Hence we
  % $\ac_{\uptau} := \pac{\uptau}\cdot 1^{(1,0)}$.

  Now we consider the set $\set{\ac_{\uptau}|\uptau\in \PBPes(\ckcO),
  \pac{\uptau}\neq 0, \ac_{\uptau}\nsqsupseteq (2,0)}$. Let $\ac_{\uptau_{1}}$
