...
\[
 \check \vartheta_{\CO'}^{\CO}:= \check \vartheta_{\sfss', \CO'}^{\sfss, \CO}: \CK_{\mathsf s'}(\CO')\rightarrow \CK_{\mathsf s}(\CO)
\]
such that
\[
...
\abs{\mathsf s}:=p+q. 
\]
Let $\check \CO$ be a Young diagram that has $\star$-good parity and satisfies that
\[
\abs{\check \CO}=\begin{cases}
    \abs{\sfss}-1,\qquad&\textrm{if $\star=B$};\\
      \abs{\sfss}+1,\qquad&\textrm{if $\star\in \{C, C^*\}$};\\
     \abs{\sfss},\qquad&\textrm{if $\star\in \{D, \widetilde C, D^*\} $}.
\end{cases}
\]
Suppose that  $\CO\in \mathrm{Nil}(\g_\mathsf s)$ is the  Barbasch-Vogan dual of $\check \CO$.

\begin{lem}\label{bvdual}
The  Barbasch-Vogan dual $\CO$ is determined by
\begin{eqnarray*}
 && (\mathbf c_i(\CO), \mathbf c_{i+1}(\CO))\\
  &=&\begin{cases}
...
$R_1:=P_1/N_1$. Write
\[ 
r_1:  \fgg_{1, \bR}/\fnn_{1, \bR}\longrightarrow  \, \frr_{1, \bR}
\]
We define a morphism 
\[
  \Indrg \colon \bZ[\Nil(\frror)]\longrightarrow  \bZ[\Nil(\fggor)]
\]
by 
\[
  ...
  C_{P_1}(v)} \sOr
\]
where $\sOr$ running over all open orbits in $G\cdot(\sOpr+\fnnor)$, $v\in
\sOr$, $C_{G_1}(v)$ and $C_{P_1}(v)$ are the component groups of the
centralizers of $v$ in $G_1$ and $P_1$ respectively. Note that the definition
only depends on the Levi subgroup $R_1$ but indpendent of the choice of the
parobolic subgroup $P_1$.  
The induction of complex nilpotent orbit is defined similarly. 


\begin{thm}[\emph{cf}. Barbasch {\cite[Corollary~5.0.10]{B.Orbit}}]
  \label{thm:Bar} 
  Let $\pi'$ be a Casselman-Wallach representation of $R_1$. 
Then
\[
...
While Barbasch proved the theorem when $G_1$ is the real points of a connected
reductive algebraic group, his proof still works in the slightly more general
setting of \Cref{thm:Bar}.
...
correspondence. We thank Professor Vilonen for confirming that their result
extends to nonlinear groups. Therefore the weak associated cycle of a
parabolically induced representation can be read from \Cref{thm:Bar}.

\medskip

Using our parameterization of rational nilpotent orbits, we now present the following lemma on the orbit induction. 
\begin{lem}\label{lem:ind} 
Suppose $\sfss' = (\star',p',q')$ and
$\cOp\in\Nil(\fggsp)$. Fix $l\in \bN$ such that $l\geq \bfcc_2(\cOp)$ and let
$\sfss'' =(\star',p'+l,q'+l)$. 
Let $\cOpp:=\indss(\cOp)$, $\sOp\in
\Nil_{\sfss'}(\cOp)$ and $t = l-\bfcc_1(\cOp)$. 
 Then $\cOpp$ satisfies 
