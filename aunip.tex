...
\newcommand{\ACw}{\mathrm{AC}^{\text{weak}}}
...
\begin{align*}
 \varepsilon_{\uptau} &:=
 \begin{cases}
 1 &  \text{if } \star\in \set{C,\wtC,D^{*}} \text{ and } (1,2)\in \wp\\
 0 &  \text{if }  \star\in \set{C,\wtC,D^{*}} \text{ and } (1,2)\notin \wp\\
 % \end{cases}\\
 % \varepsilon_{\tau} & :=
 % \begin{cases}
...
\end{align*}
 ...
 \]
 The descent map naturally extends to the
 \[
...
Put $\uptau' := (\tau',\wp'):=\nabla(\uptau)$.
 ...
 \]
 where $\widehat \otimes$ indicates the complete projective tensor product. This
 is a Casselman-Wallach representation of $G_\tau$.
...
% The following formulation of the proposition dose not work for type B and  r_2=0!
...
The key properties of the descent map when $\star\in \set{D,B,C^*}$ are
...
\begin{proof}
  Suppose $\star=D$ and $\bfrr_{2}(\ckcO)>\bfrr_{3}(\ckcO)$. Then the painted Young partition $\cP_{\tau}$ is obtained
  by attaching a column with $\frac{\bfcc_{2}(\ckcO)}{2}$ ``$\bullet$'' and
  $\cP_{\tau_{\bftt}}$ on the left of $\cP_{\DD^{2}(\tau)}$;
  $\cQ_{\tau}$ is obtained
  by attaching a column with $\frac{\bfcc_{2}(\ckcO)}{2}$ ``$\bullet$''  on the
  left of $\cQ_{\DD^{2}(\tau)}$.
  This yields the injectivity and signature formula.
...
We do induction on the number of columns of the nilpotent orbits $\cO$.
...
\begin{prop}\label{prop:LLS}
   We have:
  \begin{enumS}
    \item For each $\uptau'\in \PBPesp(\ckcOp)$, $\cL_{\uptaup}\neq 0$.
    \item For each $\uptau\in \PBPes(\ckcO)$, $\cL_{\uptau}\neq 0$.
  ...
  \item Let $\uptau_{1},\uptau_{2}$ be two distinct elements in
    ...
    \begin{itemize}
      \item $\cL_{\uptau_{1}}\neq \cL_{\uptau_{2}}$; or
      \item $\tau'_{1}\neq \tau'_{2}$ and $\varepsilon_{1} = \varepsilon_{2})$
       where $(\tau'_{i},\varepsilon_{i}) := \DD(\uptau_{i})$ for $i=1,2$.
  \end{itemize}
    ...
    \begin{equation}\label{eq:LS.dis}
      \set{\cL_{\uptau} | \uptau \in \PBPes(\ckcO)} \cap \set{\cL_{\uptau}\otimes \det| \uptau\in \PBPes(\ckcO)} = \emptyset.
    \end{equation}
  \item
  We have
  \begin{enumT}
    \item If $x_{\uptau} = s$, then $\pcL_{\uptau} = \ncL_{\uptau} = 0$.
    \item If $x_{\uptau} \in \set{r,c}$, then $\pcL_{\uptau} \neq 0$ and $\ncL_{\uptau}=0$.
    \item If $x_{\uptau} = d$, then $\pcL_{\uptau}\neq 0$ and $\ncL_{\uptau}\neq 0$.
  \end{enumT}
  % \item
  ...
  % (resp. $x_{\uptau}=r$).
  \item When $\ckcOp$ is noticed, the map
...
For a $\uptau\in \PBPes(\ckcO)$, we let
\[
  \uptaup := (\tau',\wp') := \DD(\uptau) \quad\text{ and }\quad
  \uptaupp := (\tau'',\wp'') := \DD(\uptaup)
\]
...
\begin{thm}\label{prop:calas}\label{prop:Ch.eq}
  Let $\cO\in \Nil_{\bfG}(\fgg)$ and $\cOp\in \Nil_{\bfG'}(\fgg')$. Suppose that
  one of the following condition holds:
  \begin{enumC}
    \item \label{it:case.D}
    $\cOp = \DD(\cO)$, and  if $G$ is a real orthogonal group $\bfcc_{1}(\cO)> \bfcc_{2}(\cO)$;
    \item \label{it:case.GD}  $G$ is a real symplectic group, $\cO$ is good for generalized descent (see
    \Cref{def:GD.good}) and $\cO' = \gDD_{\bfV,\bfV'}(\cO)$.
  \end{enumC}
  Then for every $\mathbb p$-genuine $\cO'$-bounded Casselman-Wallach
  representation $\pi'$ of $\wtG'$ such that $(\pi', \bfV)$ is in the convergent
  range (see \Cref{defn:CR}), $\Thetab_{\bfV',\bfV}(\pi')$ is $\cO$-bounded and
  \begin{equation}\label{eq:LCh}
    \AC_{\cOp}(\Thetab_{\bfV,\abfV}(\pi))= \dliftv_{\cO,\acO} (\AC_{\cO}(\pi)).
  \end{equation}
\end{thm}
\begin{remark}
  When $(\bfV', \bfV)$ is in the stable range, \Cref{prop:Ch.eq} is proved for unitary representations $\pi'$ in \cite{LM}.
\end{remark}


We keep the setting of Theorem \ref{prop:Ch.eq}. First observe that the
Harish-Chandra module of $\Thetab_{\bfV,\abfV}(\pi')$ is isomorphic to a quotient
of $\Thetav(\pi'^{\mathrm{al}})$, where $\pi'^{\mathrm{al}}$ denotes the
Harish-Chandra module of $\pi'$. Thus Theorem \ref{prop:GDS.AC} implies that
$\Thetab_{\bfV',\bfV}(\pi')$ is $\cO$-bounded and
\begin{equation}\label{boundch}
\AC_{\cO}(\Thetab_{\bfV',\bfV}(\pi'))\preceq \vartheta_{\cO',\cO}(\AC_{\cO}(\pi')).
\end{equation}
We will devote the rest of this section to prove that the equality in \eqref{boundch}
holds.

\subsubsection{Proof of the generalized descent case}
Let $\bfU = \bfE\oplus \bfE'$ be a $(1,1)$-space such that
$\dim(\bfE) = \dim \bfV +1$. We embed $\bfV'$ as a non-degenerate subspace in
$\bfU$. Now
\[
  \begin{split}
    &\Ind_{\wtP_{\bfE_0}}^{\wtG_{\bfV'^\perp}} \left(\pi'\otimes
    ...
    \cong \cR_{\Thetab_{\bfV,\bfU}(1_{\bfV})}(\pi)\oplus \CR_{\Thetab_{\bfV,\bfU}(1_{\bfV}) \otimes \sgn_{\bfU}}(\pi)\\
    &\cong \Thetab_{\bfV,\bfV'^\perp}(\Thetab_{\bfV',\bfV}(\pi'))\oplus
    (\Thetab_{\bfV,\bfV'^\perp}(\Thetab_{\bfV',\bfV}(\pi'\otimes \sgn_{\bfV'})))\otimes
    \sgn_{\bfV'^\perp}.
  \end{split}
\]

Let $\cO'^{\perp} = \Ind_{\bfP_{\bfE_{0}}}^{\bfG'^{\perp}}\cO'$.
By \Cref{thm:Bar} and \cite[Theorem~1.4]{SV}, the representation
  $\Ind_{\wtP_{\bfE_0}}^{\wtG_{\bfV'^\perp}} \left(\pi\otimes
    \chi_{\bfE_0}\right))$ is $\cO'^\perp$-bounded, and
  \begin{equation}
    \label{chop1}
    \ACw_{\CO^\perp}(\Ind_{\wtP_{\bfE_0}}^{\wtG_{\bfV^\perp}} \left(\pi\otimes
      \chi_{\bfE_0}\right))= \sum_{\sO'^{\perp}} \rcc_\sO(\pi) [\sO'^\perp ],
  \end{equation}
  where the summation runs over all $\bfK_{\bfV'^{\perp}}$ orbit
  $\sO'^\perp \subset \cO'^{\perp}\cap \fpp_{\bfV'^{\perp}}$
  and $\sO$ is the unique orbit such that $\sO'^{\perp}\subset \Ind_{P_{\bfE_0}}^{G_{\bfVpe}}\sO$.

  Note that by the assumption $\cO$ is good for generalized descent,
  $\bfcc_{1}(\cO') > \bfcc_{2}(\cO')$.
  Now $\Ind_{P_{\bfE_0}}^{G_{\bfVpe}}\sO$ consists the orbits
  $\sO'^{\perp}_{2}:=\dagger\dagger \sO^{+}\cdot \dagger_{(0,1)}$
  and $\sO'^{\perp}_{2}:=\dagger\dagger \sO^{-}\cdot \dagger_{(0,1)}$.

  \def\AOD{\mathrm{AOD}}

  On the other hand, for any $\cL'\in \AOD_{\cO}(\bfK_{\bfV'})$ supported on $\sO$,
  \[
    \vartheta_{\bfV, \bfV'^{\perp}}(\vartheta_{\bfV',\bfV}(\cL')) +
    \vartheta_{\bfV, \bfV'^{\perp}}(\vartheta_{\bfV',\bfV}(\cL'\otimes \sgn))\otimes \sgn
    % = \cL^{+}\cdot \dagger_{(1,0)}
    %  + (\cL\otimes \sgn)^{+}\cdot (\dagger_{(1,0)}\otimes \sgn)
    %  + \cL^{-}\cdot \ddagger_{(1,0)}+ (\cL\otimes \sgn)^{-}\cdot \dagger_{(0,1)}
  \]
  has at most two irreducible components with support $\sO'^{\perp}_{1}$ and
  $\sO'^{\perp}_{2}$
  respectively.
  Hence
  \[
    \begin{split}
    & \AC_{\cO'}(\Thetab_{\bfV,\bfV'^\perp}(\Thetab_{\bfV',\bfV}(\pi'))\oplus
    (\Thetab_{\bfV,\bfV'^\perp}(\Thetab_{\bfV',\bfV}(\pi'\otimes \sgn_{\bfV'})))\otimes
    \sgn_{\bfV'^\perp}) \\
    &=
    \vartheta_{\bfV, \bfV'^{\perp}}(\vartheta_{\bfV',\bfV}(\AC_{\cO'}(\pi'))) +
    \vartheta_{\bfV, \bfV'^{\perp}}(\vartheta_{\bfV',\bfV}(\AC_{\cO'}(\pi')\otimes \sgn))\otimes \sgn
    \end{split}
  \]

  If $\cL$ occures in $\vartheta_{\bfV',\bfV}(\cL')$, then
  $\vartheta_{\bfV, \bfV'^{\perp}}(\cL)$ is non-vanish and irreducible.

  Therefore, we conclude that
  \[
    \AC_{\cO}(\Thetab_{\bfV',\bfV}(\pi')) = \vartheta_{\bfV',\bfV}(\AC_{\cO'}(\pi')).
  \]

  \trivial{
  This is because, if $\dagger_{(1,0)}\leq \sO'$, then
   exactly one of $\cL^{+}$ or $(\cL\otimes \sgn)^{+}$ is non-zero.
...
\bib{BMSZ2}{article}{
  ...
  author = {Sun, B.-Y.},
  author = {Zhu, C.-B.},
  title = {The counting the special unipotent representations},
