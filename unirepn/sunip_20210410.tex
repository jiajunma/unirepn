...
In this section, we study the properties of the descent maps. The results of this section are
...
When $\ckcO$ is weakly-distinguished, $\ckcO'$ is also weakly-distinguished and
we get $\uptau'_{1}=\uptau'_{2}$ by \Cref{lem:BD}~(b) for $\cOp$. Hence
$\uptau_{1}=\uptau_{2}$ by $\varepsilon_{\wp_{1}}=\varepsilon_{\wp_{2}}$ and
\Cref{prop:CC.bij}. 
...
Suppose $\ac_{\uptau_{1}}=\ac_{\uptau_{2}}$. By \eqref{eq:dsign1}, we conclude
that $\Sign(\uptau'_{1})=\Sign(\uptau'_{2})$. Since we have
...
By consider the $\dsign$,  we get $\pac{\uptau_1}=\pac{\uptau_2}$ and
$\nac{\uptau_1}=\nac{\uptau_2}$. So $\pac{\uptau'_{1}}=\pac{\uptau'_{2}}$ and
$\nac{\uptau'_{1}}=\nac{\uptau'_{2}}$ by \eqref{eq:C}. Suppose $\ckcO$ is
weakly-distinghuished, then $\ckcO'$ is weakly-distinghuished. Due to the
injectivity of $\Upsilon_{\ckcO'}$ in \Cref{lem:BD3}~(a), we get
$\ac_{\uptau'_{1}}=\ac_{\uptau'_{2}}$ and so $\uptau'_{1}=\uptau'_{2}$ by
\Cref{lem:BD}~(b). Now $\varepsilon_{\wp_{1}}=\varepsilon_{\wp_{2}}$ and  
\Cref{prop:CC.bij} implies $\uptau_{1}=\uptau_{2}$, which proves
\Cref{lem:C}~(b).
...
\smallskip
Retain the notation in \Cref{lem:BD,lem:BD2,lem:BD3}. We now assume
$\bfcc_{2}(\cO)>0$ and all the lemmas in \Cref{sec:ac} holds for
$\ckcO':=\ckDD(\ckcO)$ and $\ckcO'' := \ckDD(\ckcO')$.  
...
Then $\ckcO'$ is quasi-distinguished and $\ac_{\uptau'}\in \MYD_{\star}$. 
By \Cref{eq:BD}, $\ac_{\uptau}\in \MYD_{\star}$.

%The bijectivity of $\acme\colon $

For $\cE\in \MYD_{\star}$, we now construct $(\uptau, \epsilon)\in
\PBP_\star(\ckcO)\times \bZ/2\bZ$ such that $\ac_{\uptau}\lotimes(\epsilon, \epsilon)=\cE$,
which implies $\acme(\PBP_\star(\ckcO)) = \MYD_{\star}(\cO)$. By the initial case
(see \Cref{sec:init}), there is a unique pair $(\tau_0, \epsilon)$ such that 
\[
 \cE(1) = \ac_{(\tau_0,\emptyset)}\lotimes (\epsilon,\epsilon). 
\]
By \Cref{lem:C}~(c), there is a unique $\uptau' = (\tau',\wp')\in \PBP_{\star'}(\ckcO')$
such that 
\[
 \big(((\maltese^y\ac_{\uptau'})\cdot (p_{\tau_0},q_{\tau_0}))\lotimes (0,\varepsilon_{\tau_0})\big) (i) 
 = \big(\cE\lotimes (\epsilon,\epsilon)\big)(i) \qquad \text{for all} i\geq 2.
\]
Using \Cref{cor:D.inj1}, we obtain a unique $\tau$ such that $(\DD(\tau),\taut) = (\tau',\tau_0)$.
Let $\wp\in \PP(\ckcO)$ be the unique element such that $\DD(\wp) = \wp'$. 
Then $\uptau = (\tau, \wp)$ is the anxiety element. 


\subsubsection*{Proof of \Cref{lem:BD2,lem:BD3}}

Using \eqref{eq:TBD}, all the claims follows from that of $\ckcO_{\bftt}$, see
\Cref{sec:tail}. 
