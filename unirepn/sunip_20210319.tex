...
A Casselman-Wallach representation  of $G_{\mathsf s'}$ is over-convergent  for $\check \Theta_{\mathsf s'}^{\mathsf s}$ if 
...
Assume that $\abs{\mathsf s}\geq \nu^\circ_{\mathsf s'}$. Let $\pi'$ be a Casselman-Wallach representation of $G_{\mathsf s'}$ that is over-convegent  for $\check \Theta_{\mathsf s'}^{\mathsf s}$. If $\pi'$ is unitarizable, then so is $\Thetab_{\mathbf s'}^{\mathbf s}(\pi')$.
\end{thm}
...
the integral
\[
  \int_G \la g. u_i, u_j\ra\,\Xi_G (g) \od\!g %\quad (\textrm{$\od\! g$  is a Haar measure  on $G$})
\]
...
\ee
 ...
 $\Thetab^{\sfss}_{\sfss'}(\pi'\otimes \alpha')$ is over-convergent for $\check \Theta^{\sfss''}_{\sfss}$. 
 \end{lem}
\begin{proof}
...
Let $\chi_{\bfE}$ be as in \Cref{secdege}. 
