\message{ !name(aunip.tex)}\documentclass[12pt,a4paper]{amsart}
...
\message{ !name(aunip.tex) !offset(-3) }
...
When no confusion is possible, we will not distinguish a nilpotent orbit in $\GL_n(\C)$, $\oO_n(\C)$ or $\Sp_{2n}(\C)$ with its corresponding Young diagram. In particular, the zero orbit is represented by the Young diagram consisting of one nonempty column at most. 
 ...
 \[
  ...
  \[
   ...
   \check \nabla_{\mathrm{naive}}(\check \CO), \quad & \textrm{otherwise},
    \end{cases}
  \]
  where $\tytb{\   }$ denote the Young diagram that has total size $1$. 
...
The following injectivity result will be  proved in Section \ref{Secpd}. %will be important for us. 
...
\]
where $\mathbf e:=M_\mathsf s(\phi)\in \sO$,  $\mathsf s_1$ is a classical signature of type  $\star'$, $(K_{\mathsf s,\C}\times K_{\mathsf s', \C})_\phi$ is the stabilizer of $\phi$ in $K_{\mathsf s,\C}\times K_{\mathsf s', \C}$, and $(K_{\mathsf s,\C})_{\mathbf e}$ is the stabilizer of $\mathbf e$ in $K_{\mathsf s,\C}$.
\end{lem}
...
\[
 S_{\star, k}:=\{\sfss_1=(\star, p_1,q_1) \mid  \textrm{$\sfss_1$ is a classical signature}, \, p_1+q_1=k\}
\]
...
\end{lem}
\begin{proof}
Note that the trivial representation $1$ of $G_{\mathsf s_1}$ is $0$-bounded, and $0>\nu_{\mathsf s_1}+\nu_{\mathsf s_1}^\circ -\abs{\dsfss}$. This implies the first assertion.
...
Recall that  $\CO\in \mathrm{Nil}(\g_\sfss)$ is regular for $\DD_{\mathsf s'}^{\mathsf s}$. We investigate the geometric properties of regular descent.


Let $\ww\in \CMs^{{-1}}(\cO)\cap \Woss$, and $\Xp = \CMsp(\ww)$.
By the Jacobian criterion for
regularity (see \cite[Theorem~2.19]{LiuAG}), the following sequence
\begin{equation}\label{eq:tt1}
  0\longrightarrow T_{\ww} \CMss^{{-1}}(\Xp) \longrightarrow T_{\ww} \Wss \xrightarrow{\ \rdd \CMsp \ } T_{\Xp}\fggsp \end{equation}
...
$
 ...
 -p_0-1.
$
Let $\rho'$ be the   $(\g_{\mathsf s'}, K_{\mathsf s'})$-module associated to $\pi'$.  Then $\Thetab_{\mathsf s'}^{\mathsf s}(\pi')$, $\check \Theta_{\mathsf s'}^{\mathsf s}(\pi')$  and $\check \Theta_{\mathsf s'}^{\mathsf s}(\rho')$ are all $\CO$-bounded,  and
    \[
  \mathrm{AC}_{\cO}(\Thetab_{\mathsf s'}^{\mathsf s}(\pi'))=    \mathrm{AC}_{\cO}(\check \Theta_{\mathsf s'}^{\mathsf s}(\pi'))= \mathrm{AC}_{\cO}(\check \Theta_{\mathsf s'}^{\mathsf s}(\rho'))=   \check \vartheta_{\cO'}^\cO(\mathrm{AC}_{\cO'}(\pi'))\in \CK_\sfss(\CO).
  \]
\end{thm}
...
Then $p_0=q_0\geq 0$.  Define three classical signatures
\[
...
to be the unique nilpotent orbit in $\mathrm{Nil}(\g_{\sfss''})$ that contains a non-empty Zariski open subset of  $
  \CO'+\mathfrak n_{\sfss'',\sfss_0}$ (see \cite{CM}).
...
For all $\sfss_1=S_{\star, \abs{\sfss}}$,  $\g_{\sfss_1}$ is identified with $\g_{\sfss}$ and we consider $\CO$ also as an element of $\mathrm{Nil}(\g_{\sfss_1})$. 
...
$\Ind_{P_{\sfss'',\sfss_0}}^{G_{\sfss''}} \pi_1$ is an $\CO''$-bounded Casselman-Wallach representation  of $G_{\sfss''}$ (\cite{?}).
...
$ \mathrm{AC}_{\cO''}(\CJ)=0$. Thus the  lemma in the case when $\star=C^*$  is also a consequence of Theorem \ref{doubtt} and Proposition \ref{prpaseq}. 

\end{proof}


\subsection{Proof of Theorem \ref{thm:GDS.AC}}
...
\ee
Then Theorem \ref{prop:GDS.AC} and  Lemma \ref{lem74} imply that 
\[
   \mathrm{AC}_{\cO}( \Thetab_{\mathsf s'}^{\mathsf s}(\pi'))=  \check \vartheta_{\sfss', \cO'}^{\sfss, \cO}(\mathrm{AC}_{\cO'}(\pi')).
\]
In view of \eqref{ineq000}, this proves Theorem \ref{thm:GDS.AC}. 
...
$\uptau=(\tau, \wp)\in \PBPe_\star(\check \CO,\mathsf s)$.  When $\check \CO\neq \emptyset$, we have the dual descent 
\[
  \check \CO':=\check \nabla(\check \CO), 
\]
and the descent 
 ...
 In general, we set 
\[
  \mathrm d'_\tau:=\begin{cases}
    \dim \tau', &\qquad \textrm{if $\check \CO\neq \emptyset$};\\
    0, &\qquad \textrm{if $\check \CO= \emptyset$}.
    \end{cases}
    \]
...
Wwe will prove in the rest of this subsection the following theorem, which implies  Theorem \ref{thmpitau}. 
...
We prove Theorem \ref{thmpitau222} by induction on $\mathbf c_1(\check \CO)$ (which equals the number of nonempty rows of $\check \CO$).  Theorem \ref{thmpitau222} is obvious when $\mathbf c_1(\check \CO)=0$. Now we assume $\mathbf c_1(\check \CO)>0$ and Theorem \ref{thmpitau222} holds for  smaller $\mathbf c_1(\check \CO)$. 
 ...
 \end{lem}
\begin{proof}
Similar to Lemma \ref{dualdesc}, this is  elementary to check by using Lemma \ref{bvdual}.
\end{proof}
...
\[
 ...
 -\frac{\abs{\sfss_0}}{2}-3,\quad& \textrm{if $\star =C^*$};\\
   ...
   \end{cases}.
\]
In particular, $\pi_{\uptau'}\otimes \chi$ is over-convergent for $\check \Theta_{\sfss'}^{\sfss}$ for every unitary character $\chi$ of $G_\sfss$.  
...
\]
Hence $\bar \pi_{\uptau}$ is irreducible, since $\bar \pi_{\uptau}$ is a quotient of $\pi_{\uptau}$ and $\pi_{\uptau}$ has a unique irreducible quotient. Write 
$\CJ_\uptau$ for the kernel of the natural surjective homomorphism $\pi_{\uptau}\rightarrow \bar \pi_{\uptau}$. Then $\oU(\g_\sfss)^{G_{\sfss, \C}}$ acts on $\CJ_{\uptau}$ through the character $\chi(\check \CO)$ as defined in the Introduction. Moreover, $\CJ_\uptau$  is $\CO$-bounded and  $\mathrm{AC}(\CJ_{\uptau})=0$. This implies that $\CJ_{\uptau}=0$. 
...
indentification via the Kostant-Sekiguchi correspondence.
...
signed Young diagram $\sO$ (see \Cref{eg:MYD}) is also useful, which is given by
\[
  \lsign(\sO) =
    \sum_{i\in \bN^{+}} \big(q_{2i},p_{2i} \big)
      + \sum_{i\in \bN^{+}} \big(p_{2i-1},q_{2i-1} \big).
\]
For each $\sO\in \SYD_\star$ define the naive desent $\DDn(\sO)\in
\SYD_{\star'}$ by
\[\big(\DDn(\sO)\big)(i) = \sO(i+1) \quad \text{for } i \in \bN^+.\]
Now we can explicitly describe the decent map $\DDss$. 
...
\end{equation}
When \eqref{eq:DDss1} is satisfied, the desecnt $\sO' := \DDss(\sO)$ 
is given by 
\begin{itemize}
...
Then $t$ is always a nonnegative even integer. For $\cEp\in \MYD_{\sfss}(\cO')$, we define
...
lifted to admissible orbit data thanks to \Cref{lem:aod}.
Now it is routine to check the following lemma.
...
\[
  ...
  \end{tikzcd}
\]
Here the vertical arrows are given by our parameterization of admissible orbit data, the top horizontal map is defined in  \Cref{sec:dlift}. \qed
\end{lem}
...
In this section, we describe the weak associated cycle  
of the induced representations appeared in \Cref{doubtt}. 
...
\[
  ...
  C_{P_1}(v)} \sOr
\]
where $\sOr$ running over all open orbits in $G_1\cdot r_1^{-1}(\sOpr)$, 
$v\in \sOr$, $C_{G_2}(v)$ and $C_{P_1}(v)$ are the component groups of the
centralizers of $v$ in $G_1$ and $P_1$ respectively. It can be shown that the
definition only depends on the Levi subgroup $R_1$ but indpendent of the choice
of the parobolic subgroup $P_1$.  

Via \eqref{eq:KS}, we translates  $\Ind_{R_1}^{G_1}$  to a morphism 
\[
...
The notion of induced nilpotent orbits was first
introduced by Lusztig and Spaltenstein for complex reductive groups \cite{LS}.
Let $r_{1,\bC}\colon \fpp_1 \rightarrow \frr_1$ be the natrual quotient map
...
unique open orbit in $G_{1,\bC}\cdot r_{1,\bC}^{-1}(\cO')$ which is defined to 
be the induced orbit $\Indrgc(\cO')$.
...
\end{lem}
\begin{proof}
Barbasch proved the formula of weak wavefront cycles when $G_1$ is the
real points of a connected reductive algebraic group. The proof still works in
...
correspondence. Therefore, we get the formula for weak associated cycles. 
\end{proof}
We thank Professor Vilonen for confirming that their result
extends to nonlinear groups. 

\medskip
\def\Rsppo{R_{\sfss'',\sfss_0}}
\def\Rso{R_{\sfss_0}}

We now retain the notation in \Cref{sec:DP} and describe the inductions of
certain nilpotent orbits explicitly using the signed Young diagram
...
In this section, we study the properties of the descent maps. These results are
crucial for the calculations of associated cycles in \Cref{sec:ACC}.


\subsection{Vanishing propotition}

Because of the following vanishing proposition, we only consider the
quasi-distinghuished nilpotent orbits $\ckcO$ when $\star\in \set{C^*, D^*}$.
...
\label{sec:tail}
In this subsection, we assume that $\star\in\{B, D, C^*\}$ and define the notation of ``tail'' of a painted bipartition.
...
Note that  in all these three cases
 ...
 C^*,
\end{equation}
respectively if $\star=B, D$ or $C^*$. %Here $k$ can be zero if $\star = C^*$.
...
\mathrm{PBP}_\star(\check \CO) $. The tail $\tau$ is a painted bipartition in
$\PBP_{\star_\bftt}(\ckcO_\bftt)$. We define it case by case now. 
...
In this case, we define the tail $\tau_\bftt$ of $\tau$ to be the second painted
...
\label{sec:DDP}
In this section, we estabilsh the properies 
of descent of painted bipartitions. 
These properties will be used extensively in \Cref{sec:ACC}
Recall that $\ckcO$ is assumed to be
quasi-distinghuished when $\star\in \set{C^*,D^*}$.
...
\end{equation}
is well-defined, and the inverse of \eqref{eq:DD.CC2} can be construced straightfowardly.
Therefore \eqref{eq:DD.CC2} is also bijective. In view of \Cref{tailtip}, this
proves the proposition in the case we are considering.
\trivial[h]{
...
The following lemma deal with the case when the Barbasch-Vogan dual $\cO$ of
$\ckcO$ has only one column. This is the most basic case and can be verified
easily.
...
\set{D,B,C^*}$ are summarized in the following lemmas.
...
\begin{proof}
  ...
  The proofs in the other  cases are similar. We  left them to the reader.
...
To establish \eqref{eq:sign.GD}, we discuss case by case according to the symbol $x_{1}$.
...
\begin{equation}\label{eq:sign.GD2}
  ...
  \end{split}
\end{equation}
Here $\Sign$ counts the signature of various symbols in the same way of
\eqref{defpbp0}. In second line of the above formula, the terms count the
...
It clear that the image of $\delta$ is in \eqref{eq:delta.I} and is
straightfoward to construct the inverse of $\delta$ from \eqref{eq:delta.I} to
...
$\alpha:=B^{-}$. Most of the entries in $\cP$ and $\cQ$ are already dertermined
by  
...
\]
This gives $\tau := \delta^{-1}(\tau'',\tau_0)$ and finished the proof.
\trivial[h]{
...
Combining the above \Cref{lem:delta} with \Cref{lem:D.sign}, we get the following.
...
In this section, we study the properties of the associated cycles $\AC(\uptau)$
defined in \Cref{subsecass}.
Let $\uptau=(\tau,\wp)\in \PBP_\star(\ckcO)$, $\uptau' := (\tau',\wp'):=\DD(\uptau)$ and  $\cO':=\DDtss(\cO)$.

\subsection{Translate to combinatorical problem}
...
case, $\cO$ is regular for $\DDtss$ by \Cref{lem78}. Therefore $\AC(\uptau)\in 
\bN[\AOD_{\sfss_\tau}(\cO)]$  by \Cref{lem:aod} and the induction
hypothesis that $\AC(\uptaup)\in \bN[\AOD_{\sfss_{\tau'}}(\cO')]$.
\end{proof}


Let $\ac_{\uptau}\in \bN[\MYD_\star]$ be the element corresponds to the
associated cycle $\AC(\uptau)$ via \eqref{eq:myd}. 
The study of $\AC(\uptau)$ now is translated to 
the combinatorics problem of computing $\ac_{\uptau}$. 
Via \Cref{lem:clift}, $\ac_{\uptau}$ is also
defined combinatorically as the following:  
\begin{itemize}
\item
When $\abs{\ckcO}=0$,
\[
...
We have the following more explicit formulas on $\ac_{\uptau}$.
...
\end{lem}
\begin{proof}
 The lemma follows from the signature formulas in \Cref{lem:delta}, 
\end{proof}
...
\label{sec:ac}
In this section, we summarizes the properties of $\ac_{\uptau}$. The proofs are
given in the next three sections. 

The following lemma implies the non-vanishing and multiplicity freeness of $\AC(\tau)$, see \Cref{thmac1}.
 \begin{lem}\label{lem:ac0} 
  For each $\uptau\in \PBPes(\ckcO)$,
  $\ac_{\uptau}\neq 0$. Moreover, the multiplicity of each $\sO\in \SYD_{\star}$
  in $\sF\circ \ac_{\uptau}$ is either $0$ or $1$.
\end{lem}

\smallskip

The following three lemmas implie \Cref{thmac2} \Cref{thmac3}, \Cref{thmac4},
\Cref{thmac5} and \Cref{thmac0} part (b) and (c).
...
Note that $\ckcO$ is quasi-distinguished when $\star = C^*$, see \Cref{prop:CD*}.
\begin{lem}\label{lem:C*}
...
\begin{enuma}
  ...
  \item When $\bfrr_{1}(\ckcO)> \bfrr_{3}(\ckcO)$, the maps
   ...
   \]
    ...
    In particular, there is no $\uptau_2\in \PBP_\star(\ckcO)$ satisfies 
    \[
...
All the claims for $\star\in \set{C^{*},D^{*}}$ are straightforward to verify.
So we will focus on the non-quaternionic cases in the next three sections.

\smallskip

We prove by induction on the number of columns in $\cO$.

\subsection{The initial cases}\label{sec:pfDC.init} Suppose $\bfcc_{1}(\cO)=0$.
...
$\star \in \set{C,\wtC,D^*}$. There there is $k\in\bN^{+}$ such that $\cO =
...
Suppose that $\star \in \set{B,D,C^{*}}$. There is $k\in \bN^{+}$ such that
...
in \Cref{sec:ac} are straightforward to verify.
...
\message{ !name(aunip.tex) !offset(-9017) }
