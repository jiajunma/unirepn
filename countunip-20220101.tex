...
\def\nng{n_{\mathrm g}}
\def\nnb{n_{\mathrm b}}
\providecommand{\pib}{\pi_{\mathrm b}}
\def\Gb{G_{\mathrm b}}
...
or $C_n$. As always, $\sgn$ denotes the sign character (of an appropriate Weyl group). Define a quadratic character
\be\label{defep}
...
%Recall the unique non-trivial character $\epsilon$ of $\sfW_{n}$ which is trivial on $\sfS_{n}$. 
...
% have
% \[
%   \Ind_{H_{t}}^{W'_{2t}}\tilde{\varepsilon} = \sum_{\sigma\in \Irr(S_{t})} (\sigma,\sigma)_{I}.
% \]
...
\begin{itemize}
  ...
  \item Suppose that $\star=C^{*}$. Define
        \[
        \begin{split}
          \cC_{\mathrm b}^{n} & :=
          \begin{cases}
            % \Res_{\sfW_{n}}^{\sfW'_{n}}
            \Ind_{\sfH_{t}}^{\sfW_{n}}\tilde{\varepsilon}, &\quad 
            \text{if $n=2t$ is even}; \\
         ...
         \tilde{\varepsilon} \otimes \sgn \otimes \sgn.\\
  ...
  \item Suppose that $\star=C$. Define
        ...
        \begin{split}
          ...
          \Ind_{\sfH_{t} \times \sfS_a}^{\sfW_{n}}\tilde{\varepsilon}\otimes 1, %\right)
          \\
          \cC_{\mathrm g}^{n,n} &:= \bigoplus_{2t+a+c+d=n}\Ind_{\sfH_{t} \times \sfS_{a} \times \sfW_c\times \sfW_d}^{W_{n}}\tilde{\varepsilon} \otimes
...
\[
    \begin{split}
      \abs{\PBP_{\star}(\ckcO_{\mathrm b};\tau_{\mathrm b})} = \abs{\PP_{A^{\bH}}(\ckcO'_{\mathrm b})} = 1.
    \end{split}
\]
  Suppose that $\star \in \Set{B,C,\wtC,D}$. Write $\tau_{\mathrm b} = (\tau_{L,b},\tau_{R,b})$ and
...
\subsection{Reduction to the good parity}

Recall that $G$ is a classical Lie group of type $\star \in \set{B,\wtC, C,D,C^{*}, D^{*}}$.
...
\[
  ...
  \begin{cases}
    ...
    % (\Sp(2n_{\mathrm g},\bR),\Sp(2n_{\mathrm b},\bR)), &\quad \text{if } \star = C; \\
    (\Sp(2n_{\mathrm g},\bR),\SO(\nnb,\nnb)), &\quad \text{if } \star = C; \\
    % ( \Sp(p-\frac{n_{\mathrm b}}{2},q-\frac{n_{\mathrm b}}{2}), \Sp(n_{\mathrm b},n_{\mathrm b})), &\quad \text{if
    % } \star = C^{*}, \\
    ( \Sp(p-\frac{n_{\mathrm b}}{2},q-\frac{n_{\mathrm b}}{2}), \rO^{*}(n_{\mathrm b}), &\quad \text{if
    } \star = C^{*}, \\
    (\tSp(2n_{\mathrm b},\bR),\SO(\nnb,\nnb+1)), &\quad \text{if
    } \star = \wtC; \\
    % (\tSp(2n_{\mathrm b},\bR),\widetilde \Sp(2n_{\mathrm g},\bR)), &\quad \text{if
    % } \star = \wtC; \\
...
\begin{proof} First note that $\WF(\pi')=[\sO']$.
  ...
  \begin{claim*}
    The map $\fIb$ is injective.
  \end{claim*}
  \begin{proof}[Proof of the claim]
    When $\star' = A^{\bH}$, the set $\Unip_{\ckcOpb}(\Gpb)$ is a singleton and
    the claim is trivial.
    Now we retain the notation in \Cref{sec:GLR} consider the case when $\star'=A^{\bH}$.
   Then  $\Gb = \SO(\nnb,\nnb+a)$ with $a \in \set{0,1}$.

    %We first consider the case when  $\Gb = \SO(\nnb,\nnb+a)$ with $a \in \set{0,1}$.

    We fix a maximally split Cartan subgroup
    $(\GL_{1}(\bR))^{\nnb}=H\subseteq \Gpb\subseteq G$ and let $H = MA$ where
    $M \cong \set{\pm 1}^{\nnb}$ is the compact part of $H$ and
    $A = \bR^{\nnb}_{>0}$ is the split part of $H$. We identify an irreducible
    representation $A$ with a vector in $\bC^{\nnb}$. Let
    $K=S(\rO(\nnb)\times \rO(\nnb+a))$ be a maximal compact subgroup of $\Gb$ and
    $B$ be a minimal parabolic subgroup of $\Gb$ contained in $\Pb$.

    Suppose $l$ is an integer such that $0\leq l \leq \nnb$. Let
    \[
      \delta_{l} = \underbrace{1_{1}\otimes \cdots \otimes 1_{1}}_{l} \otimes
      \underbrace{\sgn_{1} \otimes \cdots \otimes \sgn_{1}}_{\nnb-l}
    \]
    which is a fine $M$-type (\cite{Vg}*{Definition~4.3.8}). Let $\mu_{l}$ be
    the restriction to $K$ of the the irreducible
    $\rO(\nnb)\times \rO(\nnb)$-representation
    $\wedge^{l}\,\bC^{\nnb}\otimes \wedge^{0}\,\bC^{\nnb+a}$. Then $\mu_{l}$ is a
    fine $K$-type, see \cite{BGG.M}*{\S 6}.

    \def\cusp{\fC}
    Let $W_{\sigma_{l}}$ be the stabilizer of $\sigma_{l}$ in
    the real Weyl group of $H$ in $\Gb$ which acts naturally on
    $\Irr(A)=\bC^{*}$.
    Let
    \[
      \cusp := \bigcup_{0\leq l\leq \nnb}\Set{(\sigma_{l},[\nu])| \nu\in \bC^{\nnb}}
    \]
    to be the set of cuspidal data where $[\nu]$ denote the $W_{\sigma_{l}}$-orbit of $\nu$.
    Vogan proved that
    the map
    \[
      \begin{array}{lccc}
       \fX\colon & \cusp &\longrightarrow & \Irr(G)\\
       & (\sigma_{l},[\nu])& \mapsto & (\Ind_{B}^{\Gb}\sigma_{l}\otimes \nu)(\mu_{l})
      \end{array}
    \]
    is injective, see \cite{Vg}*{Theorem 4.4.8}.
    Here $(\Ind_{B}^{\Gb}\sigma_{l}\otimes \nu)(\mu_{l})$ denote the unique
    irreducible subquotient in the principal series
    $\Ind_{B}^{\Gb}\sigma_{l}\otimes \nu$ containing the $K$-type $\mu_{l}$.

    Now take $\pi'\in \Unip_{\Gpb}(\ckcOpb)$ and take $n_{i}$ ($1\leq l\leq k$) are positive integers such
    that
    $\pi' = 1_{n_{1}}\cdots 1_{n_{r}}\times \sgn_{n_{r}}\times \cdots \sgn_{n_{k}}$
    we attach $\delta_{\pi'}:=\delta_{l}$ with $l=n_{1}+n_{2}+\cdots + n_{r}$
    and
    \[
      \begin{split}
        \nu_{\pi'} =& \big( {\tfrac{n_{1}-1}{2},\tfrac{n_{1}-3}{2},\cdots \tfrac{-n_{1}+1}{2}, \tfrac{n_{2}-1}{2},\tfrac{n_{2}-3}{2},\cdots \tfrac{-n_{2}+1}{2}, \cdots \tfrac{n_{k}-1}{2},\tfrac{n_{k}-3}{2},\cdots \tfrac{-n_{k}+1}{2}} \big)
      \end{split}
    \]
    Clearly the map $\fP: \pi'\mapsto (\sigma_{\pi'}, [\nu_{\pi'}])$ is
    injective. By the construction of $\pib$, we see that $\fI(\pi') = \fX\circ \fP(\pi')$ and the
    injectivity of $\fI$ follows from the injectivity of $\fX$ and $\fP$.


    \trivial[h]{
      Let's avoid the metaplectic group case!
   \smallskip

   Now consider the case when $\Gb = \wtSp(\nnb,\bR)$.
   Now
   \begin{itemize}
     \item $H = \GL(1)^{\nnb}\times \set{\pm 1}$,
     \item $\sigma_{l}$ is the twist of  by the non-trivial
           character of $\set{\pm 1}$
     \item $W_{\sigma_{l}}$
     \item $K$ is the $\det^{2}$ double cover of $\rU(\nnb)$ which is equipped
           with a genuine character $\det^{\half}$,
     \item $\mu_{l}$ is the twist of $\rU(\nnb)$-representation
           $\wedge^{l}\bC^{\nnb}$ by $\det^{\half}$.
   \end{itemize}
   The rest parts of the proof are the same as that in the orthogonal group
   case.

 }
    % \trivial[]{ Note that $G$ is quasi-split in our case.
    ...
    %   When $\Gb = \SO(\nnb,\nnb)$, then
    %   $K = S(\rO(n)\times \rO(n))\subset \rO(n)\times \rO(n)$ the fine $K$-type
    %   is $\wedge^{\nnb-l}\bC^{\nnb}\otimes 1$. } Now \cite{Vg}*{Theorem~4.4.8}
    % implies that $\fIb$ is injective.
  \end{proof}


  The bijection follows from the claim above and the counting
  inequalities below:
  \[
