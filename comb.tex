...
\begin{split}
    ...
    \end{cases}\\ 
    R_{\ckcO} &:= \max\left(0,\frac{\bfrr_2(\ckcO)-3}{2}\right) 
\end{split}
\]
Suppose $\star=C$ and $\tau= (\imath,\cP)\times(\jmath,\cQ)\times \star_\tau\in \pbpst(\ckcO)$. 
We define the leg 
\[
\LEG(\tau) := (\imath_0,\cP_0)\times (\jmath_0,\cQ_0)\times \star_\tau
\]
of $\tau$ as the following:
   \begin{itemize}
   \item $\imath_0 = \set{(i,j)| (i+R_{\ckcO},j)\in \imath \text{ and } j\leq 2}$ and $\cP_0\colon \imath_0\rightarrow \set{\bullet,s,r,c,d}$  
   such that $\cP_0(i,j):= \cP(i+R,j)$;
   \item $\jmath_0 = \set{(i,j)| (i+R_{\ckcO},j)\in \jmath \text{ and } j\leq 2}$ and $\cQ_0\colon \jmath_0\rightarrow \set{\bullet,s,r,c,d}$ 
   such that $\cR_0(i,j) := \cQ(i+R,j)$.
   \end{itemize}
 The leg $\LEG(\tau)$ is also a painted bipartition.
 The body of $\tau$ is a pair \[\BODY(\tau) = (\imath_1,\cP_1)\times (\jmath_1,\cQ_1)\] such that 
\begin{itemize}
    \item $\imath_1\subset \bN^+\times \bN^+$ is the complement of $\imath_0$ in $\imath$ and $\cP_1 := \cP|_{\imath_1}$.
    \item $\jmath_1\subset \bN^+\times \bN^+$ is the complement of $\jmath_0$ in $\jmath$ and $\cQ_1 := \cQ|_{\jmath_1}$.
\end{itemize}
...
First assume $\tau = \LEG(\tau)$. Then $\tau$ has the following shape
\[
  \ytb{{y_{0}}{y_4},{*(srcol)r}{y_{3}},{\enon[srcol]{\vdots}},{*(srcol)r},{y_{0}},{y_{2}} }
    \times\ytb{{y_5}{y_6},\none,\none ,\none ,\none ,\none }
\]
