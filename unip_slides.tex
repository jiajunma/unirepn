...
% \begin{frame}[c]{Outline}
   ...
   \def\Coh{\mathrm{Coh}}
    \begin{frame}[label=CT]
        ...
        \begin{itemize}[<+->]
            \item  Fix an inf. char. $\mu \in \fhh^*/W$
            \item  $[\mu] := \mu+ Q^*$ ($Q^*$ is the root lattice),
           ...
           \] 
           \item[] Double cell $\cD$  in $\widehat{W(\mu)}$
            $\longleftrightarrow$ the specail repn. $\tau_0\in \cD$
           \item[] $\longrightarrow$ truncated induction  $j_{W(\mu)}^W \tau_0$
           \item[] $\xrightarrow{\text{Springer corr.} }$ $\cO$. 
            \[
                W_\mu := \set{w\in W | w\cdot \mu = \mu}.
            \]
            \item $\Coh_{[\mu]}(G)$:  the coherent cont. repn. of
                $G$-modules based on $[\mu]$.
            \item {\lblue Theorem: }
            \[
                \begin{split}
             & \#\set{\pi \in \Irr_{\mu}(G) | \AV_\bC(\pi) = \overline{\cO}}
            \leq \sum_{\substack{\tau\in \cD\\\cD\leadsto \cO}} [\tau:1_{W_\mu}]\cdot [\tau: \Coh_{[\mu]}(G)]
                \end{split}
            \]
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[label=CT]
        ...
        \begin{itemize}[<+->]
            ...
            \item Assume $\lambda_{\ckcO}\in \rho + Q^*$ %$\ckcO\in \Nil^{gp}(\bfG^\vee)$, i.e. $\ckcO$ only has odd length rows.
            ...
            \item $\Coh_{[\rho]}(G)$:  the Coh. repn. based on $[\rho]$.
            \item[]
            \vspace{-1em}
            \[
            \lblue
            \#\Unip_{\vcO}(G) = 2^l \cdot [\tau:\Coh_{[\rho]}(G)]
            \]
            \vspace{-1em}
            \item[] $W(\lambda_{\ckcO}) = S_n \ltimes \set{\pm 1}^n$,
            \vspace{-1em}
            \item[] 
            \[
            \Coh_{[\rho]}(\Sp(2n,\bR)) = \sum_{\substack{p,q,t,s,\\
            ...
            \item $[\tau:\Coh_{[\rho]}(G)]$ is counted by painted bi-partitions $\PBP(\ckcO)$.
   ...
   \delete{ 
    ...
    \begin{frame}[label=CT]
        ...
        $\ckcO = 3^2 1^1 = \ytb{\ \ \ ,\ \ \ ,\ }$ and  $\cO = 2^3$.
        $\ckcO' = 3^{2} 1^{1} = \ytb{\ \ \ ,\ }$.

        \[
        \begin{tikzcd}[ampersand replacement=\&,column sep=2em, row sep=.5em]
            \& \& \Sp(6,\bR)\\
            %\hline
    ...
    \begin{frame}[label=CT]
        ...
        \begin{itemize}[<+->]
            ...
            \begin{tikzcd}[ampersand replacement=\&]
               \LS(\ckcO) \& \PBPe_{\Sp_{\bR}}(\ckcO) \ar[r,<->]\ar[d,"\nabla"'] \ar[l,"\AC"]\& \Unip_{\ckcO}(\Sp_{\bR})  \\
               \LS(\ckcO') \ar[u,"\vartheta^{\text{geo}}"]\& \PBPe_{\rO_{\bR}}(\ckcO') \ar[r,<->]\ar[l,"\AC"] \&  \Unip_{\ckcO'}(\rO_{\bR}) \ar[u,"\theta"']
            \end{tikzcd}
            \]
            \item[]
            \[
                \pi_{\uptau} := \check\Theta(\pi_{\nabla(\uptau)}\otimes {\det}^{\varepsilon_{\uptau}})
                \]
            \[
                \pi_{\uptau} := \Theta(\pi_{\nabla(\uptau)}\otimes \chi'_{\uptau})\otimes \chi_{\uptau}
            \]
