...
Recall the set
\[
  \mathrm{AOD}_{K_{\sfss}}(\cO)=\bigsqcup_{\sO\textrm{ is a $K_{\sfss}$-orbit in
      $\cO\cap \p_{\sfss}$}} \mathrm{AOD}_{K_{\sfss}}(\sO),
\]
where $\mathrm{AOD}_{K_{\sfss}}(\sO)$ is the set of isomorphism classes of admissible orbit data over $\sO$ (\Cref{defaod}). We may thus identify $\mathrm{AOD}_{K_{\sfss}}(\cO)$ with a subset of
 $\MYD_{\sfss}(\cO)$. We also set
\[
