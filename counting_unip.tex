...
\begin{itemize}
  \item Suppose $\star= B$, $p+q=2m+1$ is odd. Define
        ...
        \begin{split}
          ...
          \cC_g^{p,q} &:=\bigoplus_{\substack{0\leq p-(2t+a+r)\leq 1\\0\leq q - (2t+a+s)\leq 1}} \Ind_{\sfH_{t} \times \sfS_{a}\times \sfW_s\times \sfW_r}^{\sfW_{n}}
          \hsgn \boxtimes 1 \boxtimes \sgn \boxtimes \sgn \\
          &\cong \bigoplus_{\substack{0\leq p-(2t+c+d+r)\leq 1\\0\leq q - (2t+c+d+s)\leq 1}} \bigoplus_{\sigma} \Ind_{\sfW_{2t} \times \sfW_{c}\times \sfW_{d}\times \sfW_s\times \sfW_r}^{\sfW_{n}}
  ...
  \item Suppose $\star=C^{*}$. Define
        ...
        \begin{split}
          \cC_b^{n} & =
          \begin{cases}
           \Res_{\sfW_{n}}^{\sfW'_{n}} \Ind_{\sfH_{t}}^{\sfW_{n}} \hsgn &
            \text{if $n=2t$ is even} \\
            0 & \text{otherwise}\\
          \end{cases}\\
  ...
  \item Suppose $\star=C$. Define
        \[
        \begin{split}
          \cC_b^{n} & :=\bigoplus_{\substack{2t+a=n}} \Res_{\sfW_{n}}^{\sfW'_{n}} \left( \Ind_{\sfH_{t} \times \sfS_a}^{\sfW_{n}}
            \hsgn\otimes 1\right) \\
  ...
  \item Suppose $\star=D$ and $p+q=2m$ is even. Define
        \[
        \begin{split}
          \cC_b^{n} & = \bigoplus_{\substack{2t+a=n_{1}}}
          \Ind_{\sfH_{t}\times \sfS_{a}}^{\sfW'_{n}} \hsgn\otimes 1\\
          \cC_g^{p,q} %& = \bigoplus_{p+q=m} \Cint{\rho}(\Sp(p,q)) \\
          &= \bigoplus_{\substack{2t+c+d+2r=p\\2t+c+d+2s=q}} \Res_{\sfW_{m}}^{\sfW'_{m}}\left(\Ind_{\sfH_{t} \times \sfW_s\times \sfW_r\times \sfW'_{c}\times \sfW'_{d} }^{\sfW'_{}} \hsgn \otimes \bsgn \otimes \bsgn \otimes 1\otimes 1 \right)
          \\
        \end{split}
        \]
  \item Suppose $\star=D^{*}$. Define
        ...
        \begin{split}
          \cC_b^{n} & =
          \begin{cases}
            \Res_{\sfW_{n}}^{\sfW'_{n}} \Ind_{\sfH_{t}}^{\sfW'_{n}} \hsgn &
            \text{if $n=2t$ is even} \\
            0 & \text{otherwise}\\
          \end{cases}\\
          ...
          \hsgn \otimes \sgn \\
          & =\bigoplus_{\substack{2t+r+s=n}} \bigoplus_{\sigma} \Ind_{\sfW'_{2t}\times \sfW_r\times \sfW_s}^{\sfW_{n}}
          (\sigma,\sigma)_{1}\otimes \sgn \otimes \bsgn \\
        \end{split}
        \]
\end{itemize}
...
\begin{prop}
  ...
  \[
    \Coh_{\Lambda_{n_{b},n_{g}}}(G)\cong \cC_{b}^{n_{b}}\otimes\cC_{g}^{p,q}
  \]
  with $(p,q) = \Sign(G) - (n_{b},n_{b})$.
\end{prop}
\begin{remark}
...
\begin{proof}
  ...
  \[
    \begin{split}
    [\tau_{b}: \cC_{b}] =\# \PBP_{\star,b}(\ckcO_{b}) =\# \PBP_{G'}(\ckcO'_{b})
    = \# \Unip_{\ckcO'_{b}}(G'_{n_{b}}) = 1.
    \end{split}
  \]
  For the good parity,
  \[
  ...
  Suppose $\star = D^{*}$.
  The bad part is the same as that of type $C^{*}$. For the good part,
  there is only one block of representations.
  Note that, the left and right parts in $\tau_{\sP}$ are never the same.
  By abuse of notation, let $\tau_{\sP}$ denote also the $\sfW$-representation.
  Then
  $[\tau_{\sP}|_{\sfW'}:\Ind_{\sfH_{t}\times \sfS_{a}}^{\sfW'}\hsgn\boxtimes \sgn] =
[\tau_{\sP}|_{\sfH_{t}\times \sfS_{a}}:\hsgn\boxtimes \sgn] =
[\tau_{\sP}:\Ind_{\sfH_{t}\times \sfS_{a}}^{\sfW}\hsgn\boxtimes \sgn] = \#\PBP.
  $
  

  This follows from the branching rules of the Weyl group representations.
