...
\begin{prop}
  There is a bijection
    \[
      \begin{array}{ccc}
        \Unip_{\star'}(\ckcO'_{b})\times \Unip_{\star}(\ckcO_{g})&         \longrightarrow &\Unip_{C^{*}}(\ckcO) \\
        (\pi',\pi_{0}) & \mapsto
                         & \pi'\rtimes \pi_{0}.
      \end{array}
    \]
\end{prop}
We use Kazhdan-Lusztig theory to reduce the problem to bad and good parts
separately. The method was already carefully explained in \cite{Mat} and see
...
\begin{enumerate}[label*=(\roman*)]
  ...
  \item there is an isomorphism $\ff\colon \fhha_{1}\rightarrow \fhha_{2}$
        between abstract Cartan subalgebras $\fhha_{1}$ and $\fhha_{2}$ of
  ...
  \[
    \ff \colon B_{1}\rightarrow B_{2}
  \]
        where $b_{1}$ and $B_{2}$ are blocks of conjugate classes of regular
        characters with infinitesimal character $\lambda_{1}$ and $\lambda_{2}$
        satisfies the following compatibility conditions: Suppose
        $\gamma_{2} = \phi(\gamma_{1})\in B_{2}$ such
        that \begin{enumerate}[label=(\alph*)]
               \item $\ff\circ \theta_{\gamma_{1}} = \theta_{\gamma_{2}}\circ \ff$
                     where $\theta_{\gamma_{i}}$ are the Cartan involution
                     induced on the corresponding abstract root systems.
                     \trivial[]{ This condition implies that, the notion of
                     compact/complex/real and
                   ...
                   % $\alpha_{2}:= \ff(\alpha_{1})$ is noncompact type I (resp.
                   % type II).
               \item For simple roots in $R^{+}_{\lambda_{i}}$, the notions of
                     noncompact/real type I/II are preserved by $\ff$:
                     $\alpha_{1}$ is noncompact type I if and only if
               ...
               \item The Cayley transforms are compatible:
                     ...
                     and
                     \[
                     \ff( c_{\alpha_{1}} \gamma_{1}) = c_{\ff(\alpha_{1})}(\ff(\gamma_{1})) \qquad \forall \gamma_{1}\in B_{1}, \alpha_{1} \text{is
                     real and satisfies parity conditon}
                     \]
               \item The $\tau$ invariants are compatible:
                     ...
                     \]
                     \trivial[]{
                       This condition seems not been used below, but used in
                       Gan-Ichino's proof!
                       For linear group the matching of $\tau$-invariant is
                       automatic since it is explicitly determined by the types
                       of simple roots.
                       For metaplectic groups, it needs extra information to
                       determine the $\tau$-invariant, see
                       \cite{RT1}*{Lemma~6.28}.
                       It is not clear to me that the $\tau$-invariant must match.

                       On the other hand, the proof of \Cref{lem:ff.irr} seems
                       implies that $\tau$-invariants match automatically!
                     }

             \end{enumerate}
\end{enumerate}

For a fixed block $B$ a regular infinitesimal character $\lambda$,
let $\Grt_{B}$ be the span of $\set{\pi_{\gamma}|\gamma\in B}$ in the
Grothendieck group, and $\Coh_{B} := \ev{\lambda}^{-1}(\Grt(B))$.
...
\begin{itemize}
  ...
  is irreducible.
\end{itemize}
Under the above assumption, $\ff$ is given by
sending $\Theta_{\gamma_{1}}$ to $\Theta_{\ff(\gamma_{1})}$.
Moreover, the above map on coherent continuation representations
can be lifted to maps between Hecke-modules and the validity of Kazhdan-Lusztig
conjecture implies the preservation of irreducibility at $\lambda_{1}$.
...
\begin{proof}
  In all cases, we let $G_{1} = G_{b}\times G_{g}$ and $G_{2}=G$.
  The map $\ff$ is the natural embedding when $\star \in \set{C,C^{*}, D,D^{*}}$
  is given by \cite{GI}*{\S 3G} when $\star = B$, and is the natural double
  covering when $\star =\wtC$.

  Since coherent continuation is compatible with induction, we have the
  following commutative diagram
  \[
    \begin{tikzcd}[column sep=large]
     &  \Coh_{[\lambda'_{b}]}(G'_{b})\otimes \Coh_{[\lambda_{g}]}(G_{g}) \ar[dl]\ar[dr]&\\
     \Coh_{[\lambda_{b}]}(G_{b})\otimes \Coh_{[\lambda_{g}]}(G_{g}) \ar[rr]& &
     \Coh_{[\lamck]}(G)
    \end{tikzcd}
  \]
  which sends $\pi'\boxtimes \pi_{0}$ to the corresponding induced modules.
  Now the irreducibility follows from that of the LHS.
\end{proof}
