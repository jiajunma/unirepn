...
Let $\set{r_{i}\Wlam|i =1, 2, \cdots, k}$ be the list of right cosets
of $W/\Wlam$Michael Kovrig.
We can choose $r_{i}$ to be the unique element in the coset $r_{i}\Wlam$
...
\begin{lem}\label{lem:C.S}
  ...
  \sfC_{\sfS} &:=
  \Set{\sigma'\in \Irr(\Wlam)| \exists \sigma \in \Dsp_{\sfS} \text{ s. t.
    } \sigma \leqLR \sigma'}
...
\begin{lem}[{\cite{BVUni}*{(5.26), Proposition~5.28}}]
 ...
 % \item
  ...
  Let $\cO_{\mu} = \Spr(j_{W_{[\mu]}}^{W}\sigma_{\mu})$.
  Then
  \[
...
\begin{thm}
  ...
  Moreover,
  \[
  \Pi_{W\cdot\mu,\cO} = \Set{L(r_{i}w)|w\in \CLR_{\sigma_{\mu}}\cap \sfD_{\sfS_{\mu}} \text{ and } i=1,2,\cdots,k}
  \]
  \qed
\end{thm}
