...
Note that the trivial representation of the trivial group has symbol
\[
\binom{0,1, \cdots, k}{0,1, \cdots, k-1}.
\]
Now it easy to deduce that
\[
\begin{split}
\cksigma_{[\underbrace{2r,2r, \cdots, 2r}_{2k}]}
=& ([\underbrace{r,r, \cdots, r}_{k}], [\underbrace{r,r, \cdots, r}_{k}]),\\
\cksigma_{[\underbrace{2r,2r, \cdots, 2r}_{2k+1}]}
=& ([\underbrace{r,r, \cdots, r}_{k+1}], [\underbrace{r,r, \cdots, r}_{k}])\quad
\text{and}\\
\ckLC_{[\underbrace{2r',2r', \cdots, 2r',2r}_{2k-\text{terms}}]}
=&
\begin{cases}
  ([\underbrace{r',r', \cdots, r',r'}_{k}], [\underbrace{r',r', \cdots, r',r}_{k}]) &
  \text{if } r'=r\geq 0 \\
  ([\underbrace{r',r', \cdots, r',r'}_{k}], [\underbrace{r',r', \cdots, r',r}_{k}]) &\\
 +([\underbrace{r',r', \cdots, r',r}_{k}], [\underbrace{r',r', \cdots, r',r'}_{k}])
  \text{if } r'>r \geq 0\\
...
\[
  \begin{split}
    ...
    =& ((\ceil{C_{1}/2},\ceil{C_{2}/2}, \ceil{C_{1}/2}),
    (\floor{C_{1}/2},\floor{C_{2}/2}, \floor{C_{1}/2}))\\
    =& \ckLC_{[\underbrace{2r_{2k-1},2r_{2k-1}, \cdots,2r_{2k-1},2r_{2k-1},2r_{2k}}_{2k-\text{terms}}]}\\
  % \left(([\underbrace{r_{2k-1},r_{2k-1},\cdots,r_{2k-1},r_{2k-1}}_{2k-\text{terms}}],
   %  [\underbrace{r_{2k-1},r_{2k-1},\cdots,r_{2k-1},r_{2k}}_{2k-\text{terms}}]))\right.\\
   % & \left. + ([\underbrace{r_{2k-1},r_{2k-1},\cdots,r_{2k-1},r_{2k}}_{2k-\text{terms}}],
   %  [\underbrace{r_{2k-1},r_{2k-1},\cdots,r_{2k-1},r_{2k-1}}_{2k-\text{terms}}])\right)\\
  & \cuprow \LC_{[2(r_{1}-r_{2k-1}), 2(r_{2}-r_{2k-2}), \cdots, 2(r_{2k-2}-r_{2k-1})]}.
  \end{split}
\]
By induction, we conclude that $\ckLC_{\ckcO}$ is in one-one corresponds to
the subsets of
\[
\CPP(\ckcO) = \set{(2i-1,2i)\in \bN\times \bN| \bfrr_{2i-1}(\ckcO)> \bfrr_{2i}(\ckcO)}.
\]

Note that according to Lusztig and BV, the subsets of $\CPP(\ckcO)$ is in
one-one correspondence to the canonical quotient $\CQ(\ckcO) = \bF_{2}[\CPP(\ckcO)]$.

We do induction on the number columns.
