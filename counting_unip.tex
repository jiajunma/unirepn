...
\subsection{Heighest weight module}
...
For each $w\in \WLam$, it give a coherent family such that  
\[
M_w(\mu) = M(w\mu) \quad \forall \mu \in \Lambda\text{ dominant} 
\] 
\trivial{
Note that the Gorthendieck group $\cK(\cO)$ of category $\cO$ is naturally embedded in the space of formule character.  
$M_w$ is a coherent family: the formal character $\ch M_w (\mu)$ of $M_w(\mu)$ is $e^{w\mu}/\prod_{\alpha\in R^+} (1-e^{-\alpha})$. 
It is clear that $\ch M_w$ satisfies the condition for coherent continuation.  
}
