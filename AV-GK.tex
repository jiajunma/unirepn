...
\title[Minimal highest weight modules]{A classification of minimal  highest weight modules for classical Lie algebras}
...
Let $L(\lambda)$ be a simple highest weight module with highest weight $\lambda-\rho$ where $\rho$ is half the sum of positive roots.
The highest weight modules with minimal Gelfand-Kirillov dimension play an important role in the study of representations of Lie groups and Lie algebras. In this paper, we will give a characterization for these modules for all classical type Lie algebras. Our characterization is given by Young tableaux and the information of highest weights of these modules. We also describe the associated varieties of these modules. This work is a generalization of Joseph's work on quantization of minimal orbital varieties.
...
Let $\mathfrak{g}$ be a complex simple Lie algebra with adjoint group $G$. We fix a triangular decomposition $\mathfrak{g}=\mathfrak{n}\oplus \mathfrak{h} \oplus \mathfrak{n}^-$ so that $\mathfrak{b}=\mathfrak{h}+\mathfrak{n}$ is a Borel subalgebra and $\mathfrak{h}$ is a Cartan subalgebra of $\mathfrak{g}$. Let $B$ denote the Borel subgroup of $G$ corresponding to $\mathfrak{b}$. Let $\Phi\subseteq \mathfrak{h}^*$ be the set of roots and $\Phi^+$ be the set of positive roots defining $\mathfrak{n}$. We have  a simple system $\Delta=\{\alpha_i|1\leq i\leq n\}\subset\Phi^+$. We use $e_{\alpha}$ to denote a root vector for a given root $\alpha \in \Phi$. Let $\rho$ be half the  sum of roots in $\Phi^+$. We  choose a subset $I\subset \{1,2,...,n\}$. Then $\Delta_I=\{\alpha_i|i\in I\}$ generates a subsystem
$\Phi_I\subset\Phi$.
Let $\mathfrak{p}_I$ be the standard parabolic subalgebra corresponding to $I$ with Levi decomposition $\mathfrak{p}_I=\mathfrak{l}_I\oplus \mathfrak{u}_I$.  We will drop the
...
Let $L(\lambda)$ be a simple highest weight $\mathfrak{g}$-module with highest weight $\lambda-\rho$.
 We use $L_w$ to denote the simple highest weight $\mathfrak{g}$-module with highest weight $-w\rho-\rho$, where  $w$ is an element in the Weyl group $W$. Actually all of the simple highest weight modules of infinitesimal character $\rho$ are like this form. Let $\mathcal{O}\subseteq \mathfrak{g}$ be a nilpotent $G$-orbit. The irreducible components of $\overline{\mathcal{O}}\cap \mathfrak{n}$ are called {\it orbital varieties} of  $\mathcal{O}$. They all take the form $\mathcal{V}(w)=\overline{B(\mathfrak{n}\cap w\mathfrak{n})}$ for some $w\in W$. In Joseph \cite{Jo84}, his definition for the orbital variety means one irreducible component of $\mathcal{O}\cap \mathfrak{n}$. So our orbital variety is the closure of Joseph's orbital variety.  For a finitely generated $U(\mathfrak{g})$-module $M$, Bernstein \cite{Be} constructed a variety $V(M)$ in $\mathfrak{g}^{\ast}$, which is called the associated variety of $M$ and whose dimenison is equal to the Gelfand-Kirillov dimension of $M$.
  From Joseph \cite{Jo84} we know the associated variety $V(L_w) $ of $L_w$ is a union of some orbital varieties $\mathcal{V}(y)$.
 Tanisaki \cite{Ta} showed that there exist examples with reducible associated vaieties in type $B$ and $C$.
For a long time, people conjectured that $V(L_w) $ is irreducible for  type  $A$, see for example \cite{BoB3, Mel}. But in 2014,
Williamson \cite{Wi} showed that there exist counter examples in type $A$. So the structure of $V(L_w) $ or $V(L(\lambda)) $  is still mysterious. For example, the following questions are still  unknown:
\begin{itemize}
...
\end{Lem}
\begin{proof}
	Note that $\fnn^- = \fuu^- \oplus \fnn_\fll^-$ where
	$\fnn_\fll^- := \fnn^- \cap \fll$.  Take a good filtration $M_i$ of $M$ so
	that $M_i = \cU^i(\fnn^-) M_0 = \cU^i(\fgg)M_0$ are $\fbb$-stable and finite
	dimensional for big enough $i$.  The subvariety $\fuu = (\fuu^-)^*$ is defined
	by the ideal $\fnn_\fll^- S(\fnn^-)$.  The condition $V(M)\subset \fuu$
	implies the $\fnn_\fll^-$ action on $\Gr M_i$ is nilpotent.  Therefore, there
	is $j$ such that $\cU^j(\fnn_\fll^-) M_i \subset M_{i+j-1}$.  Let $j(M_i)$ be
	the minimal integer satisfying the above condition.
	
	Clearly when $j(M_i)=1$, every $M_i$ is $\fnn_\fll^-$-stable. In particular,
	$M_i$ is $\mathfrak{p}$-stable, which implies $M\in \mathscr{O}^{\mathfrak{p}}$ (since $M$ is a quotient
	of $\cU(\fgg)\otimes_{\cU(\mathfrak{p})} M_i$).
	
	Now we do induction on $j(M_i)$ to prove the existence of $\fnn_\fll^-$-stable
	filtration.
	
	When $j(M_i)=1$, there is nothing to prove.
	
	Suppose $j(M_i)>1$.  Define $\tM_i = \fnn_\fll^- M_{i-1} + M_{i-1}$.  Note
	that $\tM_i$ is a good filtration. In fact,
	\[
	\begin{split}
	\tM_{i+1} & = \fnn_\fll^- M_i + M_i \subset \fnn_\fll^- \fnn^- M_{i-1} +
	\fnn^- M_{i-1}\\
	& \subset \fnn^- \fnn_\fll^- M_{i-1} + [\fnn^-, \fnn^-_\fll] M_{i-1}
	+ \fnn^- M_{i-1} \\
	& \subset \fnn^- (\fnn_\fll^- M_{i-1}+ M_{i-1}) = \fnn^- \tM_{i}
	\end{split}
	\]
	and $\tM_i$ is $\fbb$-stable (note that $M_{i-1}$ is $\fbb$-stable), so we have
	\[
	\begin{split}
	...
	\end{split}
	\]
	Therefore $j(\tM_i)< j-1$ and we get the existence of
$\fnn_\fll^-$-stable good filtration by induction. This filtration is also $\mathfrak{p}$-stable since we have $\mathfrak{p} = \fll\oplus \fuu= \fnn_\fll^-\oplus \mathfrak{b}$ , which implies $M\in \mathscr{O}^{\mathfrak{p}}$.
\end{proof}
