...
That by convension, $\Sp(0,\bR)=\Mp(0,\bR)$ is the trivial group.
Let
\[
...
\[
  \begin{split}
    \cL_{\uptau} & := \Ch_{\cO}(\uppi_{\uptau}), \text{ and } \\
...
\eqref{eq:sp-nsp.C}. The correspondence between $(x_{0},x_{1}, x_{2}, x_3)$, with
...
\begin{enumerate}[resume*=alg1]
  \item Suppose $\abs{\taulf} - \abs{\taurf} < 2$, i.e. $\uptau$ has special shape.
   ...
   \item Suppose $\abs{\taulf} - \abs{\taurf} \geq 2$.
        ...
        special diagram corresponding to $\uptau$ defined in \Cref{def:sp-nsp.C}.
\end{enumerate}
...
A map $f\colon \sfA\rightarrow \sfB$ uniquely extends to an morphism
$f\colon \sfC(\sfA)\rightarrow \sfC(\sfB)$:
\[
  f(\set{a_{1},\cdots, a_{k}}) = \set{f(a_{1}), \cdots, f(a_{k})}.
\]

\subsection{marked Young diagram}
We now explain the combinatiorial objects to parameterize local systems.
...
\begin{itemize}
  \item {\bf Young diagram } We identify a Young diagram with a finite multi-set
        $\cO$ of positive integers. Suppose $\cO = \set{R_{1}, \cdots, R_{k}}$
        with $R_{i-1}\geq R_{i}$ where $R_{i}$ is the length of the $i$-th row
        of the Young diagram. $\cO$ consists of the row lengths appeared in the
        corresponding Young diagram.

        Let $\YD$ be the set of all Young diagrams. Let $\YD(\star)$ to denote
        the Young diagrams corresponding to partitions of type $\star$. The set
        $\YD(\star)$ parameterizes the set of nilpotent orbits of type $\star$.
        Note that $\YD(C)=\YD(M)$.

  \item {\bf Signed Young diagram }
        Let $\sfS$ be the set of finite length strings of alternating signs of
        the form
        $+-+\cdots$ or $-+-\cdots$.
        We identify a Signed Young diagram with a
        finite multi-set $\sO$ generated by $\sfS$.

        Let $\SYD$ be the set of all signed Young diagrams. We have an obvious
        map
        \[
        \begin{tikzcd}[row sep=0em]
          \YD \ar[r] & \SYD\\
        ...
        \end{tikzcd}
        \]
        where $\abs{\bfrr_{j}}$ denote the length of the string $\bfrr_{j}$.

        For $\sO\in \YD$, we define the two signatures of $\cO$:
        \[
          \ssign(\sO) = (p,q) \quad
          \lsign(\sO) = (p_{1},q_{1})
        \]
        where $p$ and $q$ are the total number of $+$ and $-$ signs in $\sO$
        respectively; $p_{1}$ and $q_{1}$ are the total number of strings in
        $\sO$ begin with $+$ and $-$ signs respectively.

        We define $\SYD(\star)$ to be the preimage of $\YD(\star)$ under the
        above map. The set $\SYD(\star)$ parameterizes the real nilpotent orbits
        of type $\star$ and $\bfK$-orbits in $\fpp$.

  \item {\bf Marked Young diagram}
        Let $\sfM$ be the set of finite length strings of alternating marks of
        the form
        $+-+\cdots$,$-+-\cdots$, $\upp\umm\upp\cdots$, or $\umm\upp\umm\cdots$.
        We identify a Marked Young diagram with a multiset
        ...
        \begin{tikzcd}[row sep=0em]
          \sF: \MYD \ar[r] & \SYD\\
          \set{\bfrr'_{1},\cdots, \bfrr'_{k}} \ar[r,maps to] & \set{\bfrr_{1},\cdots, \bfrr_{k}}.
        \end{tikzcd}
        \]
        Here $\bfrr'_{j}$ is a row in $\cL$ and it determines the string
        $\bfrr_{j}$ of the same length $\abs{\bfrr'_{j}}$ according to the
        following formula
        \[
        \bfrr_{j} := \sF(\bfrr'_{j}) := \begin{cases}
          +-+\cdots & \text{ if } \bfrr'_{j} = +-+\cdots \text{ or } \upp\umm\upp\cdots\\
          -+-\cdots & \text{ if } \bfrr'_{j} = -+-\cdots \text{ or } \umm\upp\umm\cdots\\
        \end{cases}
        \]
...
\end{itemize}
The sets $\YD$, $\SYD$ and $\MYD$ are obviously commutative monoids under the following
``$\cdot$'' operation:
\[
  \set{\bfrr_{1},\cdots, \bfrr_{k_{1}}}\cdot \set{\bfrr'_{1}, \cdots \bfrr'_{k_{2}}}
  := \set{\bfrr_{1},\cdots,\bfrr_{k_{1}}, \bfrr'_{1}, \cdots, \bfrr'_{k_{2}}}.
\]
Also note that it could happen that $\cL_{1}\cdot \cL_{2}\notin \MYD(\star)$ for
...
The empty set $\emptyset$ is the ``identity'' element in $\MYD$ and is always in
$\MYD(\star)$ by convention.


For an local system $\cL,\cC\in \MYD$, $\cL\succ\cC$ means there is $\cB\in \MYD$
such that the
...
filled with the strings in $\cL$.
If we do not care the mark is $\upp/+$ or $\umm/=$, we will use $\uup$ or $\uum$
...
\begin{eg}
  ...
  \]
  denote the local system on the trivial orbit of $\rO(p,q)$ which has the
  trivial character on $\rO(p)$ and $\det$ on $\rO(q)$. Similarly,
  \[
    \dagger_{p,q}:= \tytb{+,\vdots,+,-,\vdots,-}
  \]
  denote the trivial local system on the trivial orbit.
\end{eg}



\subsubsection{Local systems}
We define  $\KM$ to be the free commutative monoid (i.e.
multiset) generated by $\MYD$ and identify $\MYD$  as the subset of $\KM$ naturally.
The addition in $\KM$ is denoted by ``$+$''.
Let $\KM(\star)$ be the sub-monoid generated by $\MYD(\star)$.

The operation `''$\cdot$'' naturally extends to $\KM(\star)$:
\[
  (\sum_{i}  m_{i}\cL_{i})\cdot (\sum_{j} m_{j}\cL_{j}) := \sum_{i,j} m_{i}m_{j}(\cL_{i}\cdot \cL_{j}).
\]
Let $0$ be the identity element in $\KM$, then $\cL\cdot 0=0$ for any
$\cL\in \KM$.

For $\cL,\cD\in \KM$, $\cL\succ \cD$ means there exists $\cB\in \KM$ such that
$\cL=\cB\cdot \cD$.
...
\[
  \chi=(\chi^{+},\chi^{-})\leftrightarrow (\bfone^{-,+})^{\chi^{+}}\otimes (\bfone^{+,-})^{\chi^{-}}.
\]

We define the $\bZ/2\bZ\times \bZ/2\bZ$ action on the set of
...
In the following, ``$l$-row'' means a row of length $l$.
...
\subsubsection{Signs of the local system}
For each local system $\cL\in\LLS(\cO)$, let $\lsign( \cL )$ be the set of signs of the most left
column of the irreducible components of $\cL$.
...
\subsection{Bijection between ``special'' and ``non-special'' diagrams for type M}
We define a bijection between ``special'' diagram $\uptau^{s}$ and
``non-special'' diagrams $\uptau$.

\begin{defn}\label{def:sp-nsp.M.sp}

  Let $\cO\in \Nil(C)$ such that $C_{2k}=2c_{2k}-1$ and $C_{2k-1}=2c_{2k-1}+1$.
  A representation $\tau^{s}$ attached to $\cO$ has the shape
  \[
...
\begin{equation}\label{eq:sp-nsp.M}
  \uptau^{s}:
  \ytb{\cdots\cdots\cdots,{v_{0}}{\cdots}\cdots,{x_{3}}\cdots\cdots,{*(srcol)s},{*(srcol)\vdots},{*(srcol)s},{x_{4}}}
  \times
  \ytb{\cdots\cdots\cdots,{x_{0}}{\ast}{\cdots},{x_{1}}{x_{2}}{\cdots},\ ,\ ,\ ,\ }
  \longleftrightarrow \hspace{2em}
  \uptau^{ns}:
  \ytb{\cdots\cdots\cdots,{w_{0}}{\cdots}\cdots,{x_{3}}\cdots\cdots,\ ,\ ,\ ,\ }
  \times
  \ytb{\cdots\cdots\cdots,{y_{0}}{\ast}{\cdots},{y_{1}}{y_{2}}\cdots,{*(srcol)r},
    {*(srcol)\vdots},{*(srcol)r},{y_{4}} }
\end{equation}
Here the grey parts have length $(C_{2k}-C_{2k-1})/2$ (could be zero length), the row contains
$x_{0}$ and $y_{0}$ may be empty, and $*$/$\cdots$ are
arbitrary entries. We remark that the value of $v_{0}$ and $w_{0}$ are
...
\subsection{Special and non-special shapes of type B}
Now we define the notion of special and non-special shape of type B.
\begin{defn}\label{def:sp-nsp.D.sp}
  Suppose
  $\cO=(C_{2k+1}=2c_{2k+1}+1,C_{2k}=2c_{2k}-1,C_{2k-1}=2c_{2k-1}+1,\cdots,C_{0}\geq 0)$
  with $k\geq 1$.\footnote{Note that $C_{2k}$ and $C_{2k-1}$ are non-zero
    even integers.}
  A representation $\tau^{s}$ attached to $\cO$ has the shape
  \[
    \tau^{s} = \tau_{L}^{s}\times \tau_{R}^{s}=
    (c_{2k},c_{2k-2},\cdots, c_{2},0) \times (c_{2k+1},c_{2k-1},c_{2k-3},\cdots, c_{1})
  \]
  is paired with
  \[
    \tau^{ns} = \tau_{L}^{ns}\times \tau_{R}^{ns} =
    (c_{2k-1},c_{2k-2},\cdots, c_{2},0) \times (c_{2k},c_{2k-3},\cdots, c_{1})
  \]
  where the $c_{j}$ are unchanged for $j\leq 2k-2$.
  We call $\tau^{s}$ the \idxemph{special shape} and $\tau^{ns}$ the
  corresponding \idxemph{non-special shape}
\end{defn}
