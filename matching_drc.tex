...
complicated as the local system $\cL_{\uptau}$ may consists of two hydras.
...
\begin{equation}\label{eq:gd.ls}
  \cL_{\uptau} =
  \begin{cases}
    (\bfone^{+,-}\otimes \dagger\maltese^t \dagger \pcL_{\uptaupp}) \cdot \pcE_{\uptau}
    + (\bfone^{+,-}\otimes \dagger\maltese^{t} \dagger \ncL_{\uptaupp})  \cdot \ncE_{\uptau}
    & \text{if } x_{\uptau} \neq d\\
    (\dagger\maltese^t \dagger \pcL_{\uptaupp}) \cdot \pcE_{\uptau}
...
%   In this case, $\set{\bfpp_{\uptau_{i}}}$ must consists of the following three elements
  ...
  \begin{enumPF}
    ...
    Then
   \[
    \cL_{\uptau_{1}} = \dagger\maltese^{\frac{\abs{\bfuu_{\uptau_{1}}}}{2}}\dagger\pcL_{\uptaupp_{1}}\cdot \pcE_{\uptaupp_{1}}
    \quad \text{and} \quad
