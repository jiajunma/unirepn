...
\[
  \maltese_{(p,q)}(\cL) := \begin{cases}
    \cL & \text{if } p-q \equiv 0 \pmod{4}\\
    \maltese_{C}(\cL) & \text{if } p-q \equiv 2 \pmod{4}\\
  \end{cases}
\]
...
For each signature $(p,q)\in \bN\times \bN$, we define a map
\[
...
\[
  \vartheta_{CD,(p,q)}(\cL) =
  \begin{cases}
    (\dagger \maltese_{(p,q)}(\cL))\cdot \dagger_{p_{0}, q_{0}} &
    \text{if } p_{0}, q_{0} \geq 0 \text{ and } p-q\equiv 2\pmod{4}\\
    0 & \text{otherwise}
  \end{cases}
\]
...
\[
  \vartheta_{CD,n} \colon \KM(C)\rightarrow \KM(D)
\]
as the following:
...
\[
  \vartheta_{DC,(p,q)}(\cL) =
  \begin{cases}
    \maltese_{(p,q)}((\dagger \cL)\cdot \dagger_{n_{0}, n_{0}}) &
    \text{if } n_{0}\in \bZ_{\geq 0}\\
    \maltese_{(p,q)}(\dagger \cL^{+}  + \dagger \cL^{-})& \text{if } n_{0}=-1\\
    0&\text{otherwise}
  \end{cases}
\]
We remark that
$\maltese_{(p,q)}((\dagger \cL)\cdot \dagger_{n_{0}, n_{0}})= (\maltese_{(p,q)}(\dagger\cL))\cdot \dagger_{n_{0},n_{0}}$.


{
  \color{red}
  In the proof later, we don't care above the mark for rows with length longer
