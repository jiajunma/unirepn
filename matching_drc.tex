...
\newcommand{\uum}{{\dotminus}}
\newcommand{\uup}{\divideontimes}
...
\begin{enumS}
  ...
  $\uppi_{\uptau_{\cO}}=\bfone$ of $\Sp(2c_{0},\bR)$ or $\Mp(0,\bR)$ to $\rO(p,q)$ for
...
\begin{defn}\label{def:sp-nsp.C.sp}
  Let $\cO\in \Nil(C)$ such that $C_{2k}=2c_{2k}$ and $C_{2k-1}=2c_{2k-1}$.
  A representation $\tau^{s}$ attached to $\cO$ has the shape
  \[
    \tau^{s} = \tau_{L}^{s}\times \tau_{R}^{s}=(c_{2k-1},\tau_{L,2k-3},\cdots, \tau_{L,1} )
    \times (c_{2k},\tau_{R,2k-2},\cdots, \tau_{R,0})
  \]
  is paired with
  \[
    \tau^{ns} = \tau_{L}^{ns}\times \tau_{R}^{ns} = (c_{2k}+1,\tau_{L,2k-3},\cdots, \tau_{L,1} )
    \times (c_{2k-1}-1,\tau_{R,2k-2},\cdots, \tau_{R,0})
  \]
  where the $\tau_{L,j}$ and $\tau_{R,j}$ are unchanged for $j\leq 2k-2$.
...
\begin{defn}\label{def:sp-nsp.D.sp}
  ...
  A representation $\tau^{s}$ attached to $\cO$ has the shape
  \[
    \tau^{s} = \tau_{L}^{s}\times \tau_{R}^{s}=(c_{2k+1},c_{2k-1},\tau_{L,2k-3},\cdots, \tau_{L,1} )
    \times (c_{2k},\tau_{R,2k-2},\cdots, \tau_{R,0})
  \]
  is paired with
  \[
    \tau^{ns} = \tau_{L}^{ns}\times \tau_{R}^{ns} = (c_{2k+1},c_{2k}+1,\tau_{L,2k-3},\cdots, \tau_{L,1} )
    \times (c_{2k-1}-1,\tau_{R,2k-2},\cdots, \tau_{R,0})
  \]
  where the $\tau_{L,j}$ and $\tau_{R,j}$ are unchanged for $j\leq 2k-2$.
  We call $\tau^{s}$ the \emph{special shape} and $\tau^{ns}$ the
...
\begin{enumerate}[label=(\arabic*).,series=alg1]
  ...
  \[
    \upepsilon_{\uptau} :=
    \begin{cases}
      0, & \text{if }\abs{\taulf}-\abs{\taurf} <  2;\\
      1, & \text{if }\abs{\taulf} - \abs{\taurf}\geq 2.
    \end{cases}
  \]
\end{enumerate}
...
\begin{enumerate}[resume*=alg1]
  ...
  \item When $\uptau$ has non-special shape, then
    ...
    \mapsto
    \uptau':\ytb{\cdots\cdots\cdots,{\ast}{\ast}{\cdots},{*(srcol)r}{*}\cdots,
      {*(srcol)\vdots},{*(srcol)r},{z_{1}},{z_{2}},\ ,\ }
    \times\ytb{{\cdots}\cdots\cdots,{\ast}{\cdots}\cdots,\ ,\ ,\ ,\ ,\ ,\ ,\ }
        \]
...
\begin{lem}\label{lem:sp-nsp.D}
  ...
  \begin{enumS}
    ...
    of $\eDD^{2}(\uptau)$ is
    \[
      \taupp = (c_{2k-1},\tau_{L,2k-3},\tau_{L,1})\times (\tau_{R,2k-2},\cdots, \tau_{R,0})
    \]
...
\subsection{Convention on the local system}
An irreducible local system is represented by a signed young diagram. A general
local system is understand as the union of irreducible ones. By our computation
later, it would be clear that the local systems appeared in our cases are always
\emph{multiplicity one}.

We use $+/-$ (resp. $\ast / =$) to denote the trivial character on the
...
If we do not care about the associated character, we will use $\uup/\uum$ to denote the
line.

For a local system $\cL$, $\dagger\cL$ means adding a column on the left of
every irreducible components of $\cL$, the sign is determined the LS lifting
algorithm and we do not care about it in most of the time.
...
where
\[
  (p_{1},q_{1})= \ssign(\cL) - (n_{0},n_{0}) \quad \test{and} \quad 2n_{0} = \abs{\eDD(\cO)}.
\]
...
\begin{defn}\label{def:sp-nsp.M.sp}
  ...
  \[
    ...
    (c_{2k},\tau_{L,2k-2},\cdots, \tau_{L,2}) \times (c_{2k-1},\tau_{R,2k-3},\cdots, \tau_{R,1})
  \]
  is paired with
  \[
    \tau^{ns} = \tau_{L}^{ns}\times \tau_{R}^{ns} =
    (c_{2k-1},\tau_{L,2k-2},\cdots, \tau_{L,2}) \times (c_{2k},\tau_{R,2k-3},\cdots, \tau_{R,1})
  \]
...
Note that the shape
$\tau = (\tau_{L,2k},\cdots, \tau_{2},0)\times (\tau_{2k-1},\cdots, \tau_{1})$
is a special shape if and only if $\tau_{L,2k}>\tau_{R,2k-1}$.
...
% \item If the first row of $\uptau$ ends with $d$, $\pcL$ and $\ncL$ are both non-zero.
% \end{enumS}
% \ref{p:drcls.1} follows from the above properties.
...
% \item When $\uptaup_{L,0}$ ends with $c/r$, $p_{1}\geq 1$ ans so $\bsign{\cL_{\uptaup}} = (p_{1}-1,q_{1})$.
% \item When $\uptaup_{L,0}$ ends with $d$,  $\bsign{\cL_{\uptaup}}=\set{(p_{1},q_{1}-1),(p_{1}-1,q_{1})}$
% \end{enumI}
