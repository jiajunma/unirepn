...
\begin{equation}\label{eq:sp-nsp.M}
  \uptau^{s}:
  \tytb{\cdots\cdots\cdots,{x_{3}}\cdots\cdots,{*(srcol)s},{*(srcol)\vdots},{*(srcol)s},{x_{4}}}
  \times
  \tytb{\cdots\cdots\cdots,{x_{1}}{x_{2}}{\cdots},\ ,\ ,\ ,\ }
  \longleftrightarrow \hspace{2em}
  \uptau^{ns}:
  \tytb{\cdots\cdots\cdots,{y_{3}}\cdots\cdots,\ ,\ ,\ ,\ }
  \times
  \tytb{\cdots\cdots\cdots,{y_{1}}{y_{2}}\cdots,{*(srcol)r},
...
\begin{enumerate}[resume*=alg1]
  \item Suppose $\abs{\taulf} \geq \abs{\taurf} $, i.e. $\uptau$ has special shape.
   ...
   \item Suppose $\abs{\taulf} < \abs{\taurf}$, i.e. $\uptau$ has non-special shape.
        We define $\uptaup=\eDDo(\uptau):=\eDDo(\uptau^{s})$ where $\uptau^{s}$ is the
        special diagram corresponding to $\uptau$ defined in \Cref{def:sp-nsp.M}.
\end{enumerate}
