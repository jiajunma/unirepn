...
%         \item[]    \hhgraph{0.2\textwidth}{Pontryagin.jpg}{Lev S. Pontryagin}
%               \hspace{0.5em}
    ...
    \begin{frame}
      ...
      \begin{itemize}[<+->]
        ...
        \begin{tikzpicture}[overlay, remember picture]
          \draw [red] (4.9cm,1.05cm) circle (0.2cm);
          \draw [red] (5.8cm,1.05cm) circle (0.1cm);
      \end{tikzpicture}
   ...
   \begin{frame}[label=CO]
        ...
        \begin{itemize}[<+->]
            ...
            $\Unip_{\ckcO}(G)$
             ...
             \item [] \hspace{2em} {\color{red} Special unip. repn. are not enough in general}\\
                      \hspace{2em}  E.g., Complex Spin group, by Wong-Zhang.\\
                      \hspace{2em}   Losev, Mason-Brown and Matvieievskyi for the generalization.
             \item \emph{Conjecture (1980s)}:
             $\Unip_{\ckcO}(G)$ consists of {\red unitary} repn.
            \item[] {\red Question 1:} Size of $\Unip_{\ckcO}(G)$?\\
             {\red Question 2:} Construction of unipotent repn. \\
             {\red Question 3:} Character formula? % of $\Unip_{\ckcO}(G)$?
            % of elements in $\Unip_{\ckcO}(G)$?
            \item \emph{Barbasch-Vogan} 1985: Classification for {\lblue
              complex groups}.
...
%     Answer for \emph{exceptional groups}.
%      \begin{itemize}[<+->]
...
%    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}[<+->]
           ...
           \item[] \hspace{2em} $ W_\mu = \set{w\in W | w \mu = \mu}.  $ 
            ...
            \begin{block}{Lemma: \uncover<6->{\color{red}If $\fgg$ has no $E_8$ factor, then}}
            \[
                \begin{split}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \end{cases}
            \]
            \item[] $\ckcO$ good parity $\leadsto \chi_{\ckcO}$ is integral.
            \vspace{-1em}
    ...
    \begin{frame}
        ...
        \begin{itemize}[<+->]
            \item Example: $G = \Sp(2n,\bR)$ and  $\chi_{\ckcO}\in \rho_G +$ weight lattice. %$\ckcO\in \Nil^{gp}(\bfG^\vee)$, i.e. $\ckcO$ only has odd length rows.  
           ...
           \] 
            ...
            \item $[\tau_i:1_{W_{\lambda_{\ckcO}}}]=1$ %is counted by painted bi-partitions $\PBP(\ckcO)$.
...
%              tool for
%        \emph{``small''} repn.% and auto. forms.
...
%    \end{frame}
    ...
    \begin{frame}
      ...
      \]
      \\[-1em]\pause
        \hhgraph{0.10\textwidth}{Kraft}{\tiny Kraft}
        \hhgraph{0.10\textwidth}{Procesi}{\tiny Procesi} 
    ...
    \begin{frame}
        \frametitle{Example: Coincidences of theta liftings}
    ...
    \begin{frame}
        ...
        \begin{itemize}[<+->]
            ...
            \end{tikzcd}
            \]
            \item[] For $\tau\in \ePBP(\ckcO)$, define  
            \[
...
\begin{frame}
  ...
  \frametitle{Lifting of Associated characters I}
  \begin{itemize}
  \item Example $(G,G')=(\Sp(2n,\bR), \rO(p,q))$ 
    \[
      \begin{tikzcd}[ampersand replacement=\&,column sep=3em, row sep=1em]
        ...
        \ar[ld,"\varphi'"'] \& \\
        M_{p,q}\cong {\mathfrak{p}'} \&  \text{\footnotesize $w = (A,B)$} \ar[rd,mapsto]\ar[ld,mapsto]\& {\mathfrak p}\cong
  ...
  \item[]
   ...
   \item[] such that
    \[
     ...
     \]
     for every $\pi'$ with $\AV(\pi')\subset \overline{\cO'}$
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Lifting of Associated Characters II}
  \begin{itemize}[<+->]
  \item Recall 
    $(G,G')=(\Sp(2n,\bR), \rO(p,q))$
  \item For $\sL' \in \cK_{\cO'}(G')$, $\sL = \vartheta(\sL')\in \cK_{\cO}(G)$,
    \[
      \sL_X = \vartheta_{w}(\sL_{X'}):= {\red {\det}^{(p-q)/2}|_{K_{X}}} 
     \otimes (\sL'_{X'})^{K'_{2,X'}} \circ \alpha_w,
    \]
  \item[] $\alpha_w\colon K_X \longrightarrow K'_{1,X'}$:
...
\end{frame} 
    ...
    \begin{frame}
      \frametitle{Key ideas in the proof}
      \begin{itemize}
        \item double $\theta$-lift $\approx \text{parabolic induction}$
        \item[] $\theta$-lift $\approx \sqrt{\text{parabolic induction}}$
        \item[]  use the structure of degenerate principle series. \pause
        \item Estimation of matrix coeff. integrals  $\leadsto$ \emph{unitarity} \pause
        \item  {Sharp formula of Asso. Char.}: {\color{orange} lower bound=upper bound}
              \begin{itemize}
                \item \emph{double $\theta$-lift} $\leadsto$ lower bound.
                \item \emph{double fibration of moment maps}  $\leadsto$ upper bound
              \end{itemize}
        \item {Exhaustion}: {\color{orange} lower bound=upper bound} (Combinatorics)
              \begin{itemize}
                \item Character theory (Kazhdan-Lusztig-Vogan theory)
                \item[] $\leadsto$ upper bound by counting painted bipartitions.
                \item {Asso. Char.+Injectivity of $\theta$} $\leadsto$
                      lower bound.
              \end{itemize}
      \end{itemize}
    \end{frame}
    ...
    \begin{frame}
      %\frametitle{Related papers (Dan Barbasch, M. ,  Binyong Sun and Chen-Bo Zhu)}
      \frametitle{Relevant papers}
   	...
   	On the notion of metaplectic Barbasch-Vogan duality
        \href{https://arxiv.org/abs/2010.16089}{https://arxiv.org/abs/2010.16089}
          \item \emph{Counting and reduction to good parity}\\
