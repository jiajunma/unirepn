...
are invariant under the $J$ and $L$ actions. 
...
%classifies the set $\Nil(\fgg_{\sfss})$ of
...
orbits $\bigsqcup_{\sfss}\Nil(\fgg_{\sfss})$. 
...
signature $\Sign(\sO)$ is computed by the following formula:
\[
  \begin{split}
    \Sign(\sO) = &
...
\begin{lem}\label{lem:KX1}
  The isotropoic subgroup have decomposition 
  $(\KCsX= R_\X\ltimes U_\X$, where
  $U_\X$ is the unipotent radical of $\KCsX$ and $R_{\X}$ is a
  reductive group canonically identified with
  \[
...
Fix a one-dimensional character $\chi$ of the connected component $\KCsoX$ of 
$\KCsX$.
Let
\[
\DCO{\chi} := \set{\text{line bundle $\cE$ on $\sO$ such that $\KCsoX$
    acts on the fiber $\cE_{x}$ by $\chi$}}.
\]
By the structure of $R_{\X}$, a line bundle $\cE\in \DCO{\chi}$ is completely
determined by the restruction of the $\KCsX$-module $\cE_{\X}$ on the
