...
of trace zero,
\[
  \Xslt := \begin{pmatrix}1/2 & \phantom{-} \ci /2 \\ \mathbf \ci /2 & -1/2 \end{pmatrix},
  \quad \text{and} \quad
...
\end{equation}
Here $\St{i}=\bC^{i}$ denote the irreducible $\slt$-module $\SLT_{i}$ of dimension $i$, and
\[
...
%  of $\slt$ on $\St{i}$.
 ...
 Here
 \begin{itemize}
   \item   the classical signature of $\St{i}$ is $(B,1,1)$ if $i$ is odd and
         $(C,-1,-1)$ if $i$ is even,
   \item $J_{\St{i}}$ is the complex conjugation on $\St{i}=\bC^{i}$,
   \item $L_{\St{i}} = (-1)^{\floor{\frac{i-1}{2}}}\, \SLT_{i}({\tiny\begin{pmatrix}\phantom{-}0 & 1 \\-1 & 0 \end{pmatrix}})$,
...
structure
$(\VV{i}, \inn{}{}_{\VV{i}},J_{i},L_{i})$ with classical signature $\sfss_{i}$
such that, restricted on the $\slt$-isotypic component $\VV{i}\otimes \St{i}$,
\begin{itemize}
  ...
  \item $\star_{\sfss_{i}} = \star_{\sfss}$ if $i$ is odd and
        $\star_{\sfss_{i}} = \star'_{\sfss}$ if $i$ is even  ($\star'_{\sfss}$
        is the Howe dual of $\star_{\sfss}$).
\end{itemize}

The signatures $\sfss_{i}$ are independent of the choice of $\SLT$
and uniquely determined the orbit $\sO$.

\def\CCSS{\mathsf{CS}}
Let $\CCSS$ denote the set of all classical signatures.

Moreover, the choice of $\SLT$ also classifies the set $\Nil(\fgg_{\sfss})$ of
real nilpotent orbits, and
$G_{\sfss}\cdot \SLT(\eslt) \leftrightarrow  \KC[\sfss] \SLT(Xslt)$ give
the Kostant-Sekiguchi correspondence.

This is nothing but the signed Young diagram classification of rational nilpotent orbits.
Hence we identify $\sO$ with the function
\[
...
$\sfss=(\star,p,q)\mapsto p+q$. Now the  complexification
$\GC\cdot \sO$ of a rational nilpotent orbit is given by $\cO := \sfmm\circ \sO$.

Define
\[
  \begin{split}
  \SYD_{\star}(\cO) &:=\Set{\sO\colon \bN^{+}\rightarrow \CCSS| \begin{minipage}{11em}
...
Now $\SYD_{\star}$ is naturally identified with the set $\bigsqcup_{\sfss}\Nil(\fgg_{\sfss})$
...
\begin{lem}\label{lem:KX1}
  Suppose $\sO\in \Nil(\bfpp_{\sfss})$, $\SLT$ is compatible with $\sO$ and
  $X = \SLT(\Xslt)\in \sO$.
Let $K_{X}:=\Stab_{\GC^{L_{\sfss}}}(X)$ be the stabilizer of $X$ in $\GC^{L_{\sfss}}$, and $R_X$ the
centralizer of $\Im(\phi)$ in $\KC[\sfss]$.
Then $K_{X} = R_X\ltimes U_X$,
  where $U_X$ is the unipotent radical of $\KC[X]$ and
 there is a canonical isomorphism
 \[
   R_X \cong \prod_{i} \KC[ \sfss_{i} ].
 \]
 {\color{red} covering is a problem}
\end{lem}
...
Let $\cE\in \AOD_{\sfss}(\sO)$ be an admissible orbital data on
$\sO\in \Nil(\sfss)$,
see \Cref{defaod}.
the  $\KC[X]$-module $\cE_{X}$ is determined by its restriction on
the connected components of the orthogonal factors.
...
\]
it has a natural map $\sF\colon \MK\rightarrow \CCSS$ given by
$(\star,r,s)\mapsto (\star,\abs{r},\abs{s})$.
We identify $\cE$ with the map
\[
...
\begin{itemize}
  ...
  \item $\cE(i) := (\star_{i},(-1)^{\epsilon^{+}}p_{i},(-1)^{\epsilon^{-}}q_{i})$
        if the restriction of $\cE$ restricts on the factor
        $\rO_{p_{i}}(\bC)\times \rO_{q_{i}}(\bC)$ is
        ${\det}^{\epsilon^{+}}\otimes {\det}^{\epsilon^{-}}$.
\end{itemize}

Let
\[
  \MYD_{\sfss}(\cO) := \Set{\cE\colon \bN^{+}\rightarrow \MK|\sF\circ \cE \in \SYD_{\sfss}(\cO)}
\]
and $\MYD_{\star}(\cO):=\bigsqcup_{\sfss}\MYD_{\sfss}(\cO)$. Clearly, if
$\AOD_{\sfss}(\cO)$ is non-empty, then $\AOD_{\sfss}(\cO)$ is identified with
$\MYD_{\sfss}(\cO)$.
...
\begin{eg}
 ...
 \[
   1^{r,s}_{\star}(i) := = \begin{cases}
     (\star,r,s) & \text{if } i=1\\
     (\star,0,0) & \text{if $i>0$ and $i$ is odd}\\
     (\star',0,0) & \text{if $i>0$ and $i$ is even}
   \end{cases}
 \]
\end{eg}

\subsection{Operations on the orbit data}
...
\begin{prop}
    For each $\uptau\in \PBPes(\ckcO)$, $\oAC{\uptau}\neq 0$.
\end{prop}
...
\begin{prop}
  ...
  \begin{enuma}
    ...
    $\PBPesp(\ckcOp)$ such that $\oAC{\uptau_{1}} =  \oAC{\uptau_{2}}$.
    Then
...
\begin{prop}
  ...
  %   $\uptau_{2}=(\tau_{2},\wp_{2})$ be two distinct elements in $\PBPes(\ckcO)$.
  %   Then
  %   \begin{itemize}
  %     \item either $\oAC{\uptau_{1}}\neq \oAC{\uptau_{2}}$;
  %     \item or $\uptau'_{1} \neq \uptau'_{2}$ and $\varepsilon_{\tau_{1}} = \varepsilon_{\tau_{2}}$
  %      where $\uptau'_{i}:= \DD(\tau_{i})$ for $i=1,2$.
  % \end{itemize}
  %
  \item When $\ckcO$ is weakly-distinguished, the following map is injective.
  \[
...
recover it from $\oAC{\uptau_{i}}$. So $\pAC{\uptaupp_{1}} = \pAC{\uptaupp_{2}}$.
Since $\bfrr_{1}(\ckcO'')>\bfrr_{3}(\ckcO'')$,  we get
$\oAC{\uptau_{1}}=\oAC{\uptau_{2}}$.
Since $\cO''$ is noticed,
...
%       $\oAC{\uptau}\succeq 1^{(1,1)}$ and we are done.
...
\begin{proof}
  ...
  Suppose $\oAC{\uptau}\supset 1^{(2,0)}$. This is equivalent to
  $\pAC{ \uptau })\supset 1^{(1,0)}$.

  Then we get the bijectivity of
  \[
    \set{\oAC{\uptau}|\oAC{\uptau}\supset 1^{(2,0)}} \rightarrow \set{\pAC{\uptau}|\pAC{\uptau}\supset 1^{(1,0)}}.
  \]
  $\oAC{\uptau} := \pAC{\uptau}\cdot 1^{(1,0)}$.

  Consider the set $\set{\oAC{\uptau}|\pAC{\uptau}\neq 0, \oAC{\uptau}\nsupset 1^{(2,0)}}$. Let
  $\oAC{\uptau_{1}}$ and $\oAC{\uptau_{2}}$ be two elements in it such that
  ...
  $\oAC{\uptaupp_{1}}=\oAC{\uptaupp_{2}}$ by induction hypothesis.
  Now \eqref{eq:gd.ls} yields $\oAC{ \uptau_{1} }=\oAC{ \uptau_{2} }$.
\end{proof}
...
$\pAC{\uptau_{i}}\neq 0$ for $i=1,2$.
  ...
  so $\oAC{ \uptau_{1} }=\oAC{ \uptau_{2} }$ by \eqref{eq:gd.ls}.
  ...
  we conclude that $\oAC{ \uptau_{1} }=\oAC{ \uptau_{2} }$.
\end{proof}
...
\begin{proof}
  ...
  Suppose $\oAC{\uptau}\supset 1^{(0,2)}$. This is equivalent to
  $\pAC{ \uptau })\supset 1^{(0,1)}$.

  Then we get the bijectivity of
  \[
    \set{\oAC{\uptau}|\oAC{\uptau}\supset 1^{(0,2)}} \rightarrow \set{\nAC{\uptau}|\nAC{\uptau}\supset 1^{(0,1)}}.
  \]
  $\oAC{\uptau} := \pAC{\uptau}\cdot 1^{(0,1)}$.

  Consider the set $\set{\oAC{\uptau}|\nAC{\uptau}\neq 0, \oAC{\uptau}\nsupset 1^{(0,2)}}$. Let
  $\oAC{\uptau_{1}}$ and $\oAC{\uptau_{2}}$ be two elements in it such that
  $\nAC{\uptau_{1}}=\nAC{\uptau_{2}}$.
  ...
  \end{itemize}
  Since $\bfrr_{1}(\ckcO'')>\bfrr_{3}(\ckcO'')$, we get
  $\oAC{\uptau_{1}} = \oAC{\uptau_{2}}$ by the injectivity of
