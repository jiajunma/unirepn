...
Suppose $\star \in \set{B,D}$ and $\ckcO \neq \emptyset$.
  ...
  \item When $\bfrr_{1}(\ckcO)> \bfrr_{3}(\ckcO)$, the maps
   ...
   \]
    are injective.
  \item Suppose $\ckcO$ is weakly-distingushied and $\bfrr_{1}(\ckcO)> \bfrr_{3}(\ckcO)$.
  If $x_{\tau}=d$, then there is $\cE\in \MYD$ with non-zero coefficient in
...
Using \eqref{eq:TBD}, all the claims (a-c) in \Cref{lem:BD2} follows from that of
$\ckcO_{\bftt}$, see \Cref{sec:tail}. (Note that we always have
$\bfrr_{1}(\ckcO)>\bfrr_{3}(\ckcO)$. So \Cref{lem:BD2}~(c) is not vacant.)
...
{\bf Proof of \Cref{lem:BD}~(a). }
\def\opac{\cL'^+}
\def\onac{\cL'^-}
\def\tpac{\widetilde{\cL'}^+}
\def\tnac{\widetilde{\cL'}^-}
The following proof only depends on \Cref{lem:BD2}~(a).
...
Using \Cref{lem:BD2}~(a), we have the following criteria for
...
{\bf Proof of \Cref{lem:BD}~(b). }
First not that
$\bfrr_{1}(\ckcO'') = \bfrr_{3}(\ckcO)=\bfrr_{2}(\ckcO)>\bfrr_{5}(\ckcO) = \bfrr_{3}(\ckcO'')$.

Suppose that  $\ac_{\uptau_{i}} \sqsupset (0,1)$ or $\ac_{\uptau_{i}} \sqsupset (0,-1)$.
The first term in \eqref{eq:BD2} for both $\uptau_{1}$ and $\uptau_{2}$ must be
non-zero and equal.
This implies $\pac{\uptaupp_{1}}=\pac{\uptaupp_{2}}$.
By \Cref{lem:BD2}~(c) and \Cref{lem:BD}~(b), $\ac_{\uptaupp_{1}} = \ac_{\uptaupp_{2}}$ and  $\uptaupp_{1}=\uptaupp_{2}$.
...
We consider case by case according to the set $\set{\Sign((\tau_{i})_{\bftt})|i=1,2}$:
  \begin{enumPF}
    ...
    $\uptaupp_{1}=\uptaupp_{2}$ by \Cref{lem:BD2}~(c) and \Cref{lem:BD}~(b).
    \item
    ...
    \]
    which is contradict to \Cref{lem:BD2}~(d).
