...
The marked Young diagram  $(1^{(1,-1)}2^{(2,0)}3^{(0,-1)})_{B}$ is drew as the
following.
\[
  \ytb{\umm\upp\umm,-+,-+,+,\umm}
\]
...
\]
Morever, we define an involution $\sharp\colon \bZ\times \bZ$ by
$(a,b)\mapsto (a,b)^{\sharp}:=(b,a)$.
...
We set $t := \bfcc_{1}(\cO')-\bfcc_{2}(\cO)$.
...
\[
  \dlift(\cEp) :=
  \begin{cases}
    (\maltese^{\frac{p-q}{2}}(\Lambda_{(\frac{t}{2},\frac{t}{2})}( \cE )))\cdot (p_{0},q_{0}) &
    \text{if } ( p_{0},q_{0} ) \succeq (0,0) \text{ and } \star=D \\
    (\maltese^{\frac{p-q+1}{2}}(\Lambda_{(\frac{t}{2},\frac{t}{2})}( \cE )))\cdot (p_{0},q_{0}) &
    \text{if } ( p_{0},q_{0} ) \succeq (0,0) \text{ and } \star=B \\
    (\Lambda_{(\frac{t}{2},\frac{t}{2})}( \cE ))\cdot (p_{0},q_{0}) &
...
\[
  \dlift(\cEp) :=
  \begin{cases}
    (\maltese^{\frac{p-q}{2}}\Lambda_{(\frac{t}{2},\frac{t}{2})}(\cE)) \cdot (D,0,0) &
    \text{if } ( p_{0},q_{0} ) \succeq (0,0) \\
    0 & \text{otherwise}
  \end{cases}
\]
...
\[
  \dlift(\cE) :=
  \begin{cases}
    \maltese^{\frac{p-q}{2}}\big(\sum_{j=0}^{t} \Lambda_{(j,t-j)}( \cE)\cdot (n_{0},n_{0})\big) & \text{if
    } \star = C\\
    \maltese^{\frac{p-q-1}{2}}\big(\sum_{j=0}^{t} \Lambda_{(j,t-j)}( \cE)\cdot (n_{0},n_{0})\big) & \text{if
    } \star = \wtC\\
    \sum_{j=0}^{\frac{t}{2}} \Lambda_{(2j,t-2j)}( \cE)\cdot (n_{0},n_{0}) & \text{if
    } \star = D^{*}
  \end{cases}
\]
...
\[
  \dlift(\cE) :=
  \begin{cases}
    \maltese^{\frac{p-q}{2}}\big(\cE \cdot (C,n_{0}, n_{0}) \big) &
    \text{if } t=0\\
