...
\def\Ks{{K}_{\sfss}}
\def\Ksp{{K}_{\sfss'}}
\def\Kspo{{K}_{\sfss'_1}}
\def\Kspt{{K}_{\sfss'_2}}
...
\begin{equation}\label{eq:tt1}
  0\longrightarrow T_{\ww} \CMss^{{-1}}(\Xp) \longrightarrow T_{\ww} \Woss \xrightarrow{\ \rdd \CMsp \ } T_{\Xp}\fggs \end{equation}
...
\begin{equation}\label{eq:GG}
  \fgg\times(\fggs)_{\Xp} \xrightarrow{\ (A,A')\mapsto A'\ww - \ww A \ } W
...
Now take $\ww\in \CXss$. Then the sequence \eqref{eq:GG} is compatible with the
...
Using the argument we also have the following lemma which we will not use in the
paper.
