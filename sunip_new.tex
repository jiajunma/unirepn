...
\begin{lem}
  ...
  \begin{enuma}
    ...
    Then $\cOpp:=\indss(\cOp)$ is the nilpotent oribt in $\fggspp$ such that
    ...
    For $\sOp\in \Nil_{\sfss'}(\cOp)$, we have
    \[
      \indsss(\sOp) =
      \begin{cases}
...
%\begin{defn}\label{def:W}
  ...
  \[
    ...
    \]
     with the form
     \[
    \innw{T_1}{T_2} := \tr(T_1^* T_2), \qquad\textrm{for all }T_1,T_2\in \bfW.
  \]
  Here
  $\xymatrix{ \star \colon \Hom_{\bC}(\bfV,\bfV') \ar[r]^<>(.5){\cong} &
    \Hom_{\bC}(\bfV',\bfV) }$ is the adjoint map induced by the non-degenerate
  forms on $\bfV$ and $\bfV'$:
  \[
    \inn{Tv}{v'}_{\bfV'} = \inn{v}{T^* v'}_{\bfV},  \qquad\textrm{for all }v\in
...
a (real) reductive dual pair in $\Sp(W)$, where $W:=\bfW^{J_{\bfW}}$.
  ...
  \trivial[h]{ If one switch the role of $\bfV$ and $\bfV'$ the form on
    $\inn{}{}_{\bfW}$ will switch the sign, i.e $\tr(T^* T) = -
    \tr((T^*)^* (T^*))$.  Also note that $(J' T J^{-1})^* = J T^* J'^{-1}$ and
    $(L'TL^{-1})^* = L T^* L'^{-1}$. Hence, once we verified $(J'
    ...
    $\tr(T^* T) = \sum \inn{Te_j}{Te_j}$ ), the $J_\bfW$ action is just
