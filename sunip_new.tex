...
When no confusion is possible, we will not distinguish a nilpotent orbit in $\GL_n(\C)$, $\oO_n(\C)$ or $\Sp_{2n}(\C)$ with its corresponding Young diagram. In particular, the zero orbit is also regarded as the Young diagram  with  one nonempty column at most.
...
\]
 ...
 \smallskip
  ...
  \noindent $p_{\tau}$ and $q_\tau$: If $\star_\tau\in \{B, D, C^*\}$, this is a pair of natural numbers given by counting  the various symbols appearing in $(\imath, \CP)$, $(\jmath, \CQ)$ and $\{\alpha\}$ :
  \[
  \left\{
     \begin{array}{l}
...
The study of the special unipotent representations in general case reduced to the case when $\check \CO$ has $\star$-good parity (see \cite{BMSZ2}). 
%Appendix \ref{secapp}. 
Now we suppose that  $\check \CO$ has  $\star$-good parity. Equivalently but in a completely combinatorial setting, we consider $\check \CO$ as a Young diagram that has  $\star$-good parity in the following sense:
\[
  \left\{ \begin{array}{l}
...
Let $\star$ and $\check \CO$ be as before. For every $\uptau = (\tau,\wp)\in \mathrm{\PBPes}(\check \CO)$, in what follows we will construct a representation $\pi_{\uptau}$ of $G_\tau$ by theta lift.
...
\end{equation}
 ...
 \]
We define the descent of $\uptau = (\tau,\wp)\in \mathrm{\PBPes}(\check \CO)$ to be the element 
 \[
...
%For $\uptau = (\tau,\wp)$, p
 ...
 Now we define the representation $\pi_{\uptau}$ of $G_\tau$ by induction on $\mathbf c_1(\check \CO)$:
 \[
   \pi_{\uptau}:=\left\{
     \begin{array}{ll}
...
%\]
  ...
  In various context, we use $\emptyset$ to denote the empty set, the empty Young diagram or the painted Young diagram whose underlying Young diagram is empty. 
...
The following injectivity result is important for us. 
...
If $\star \in \set{D,B,C^*}$, then the map 
\begin{equation}
  \begin{array}{rcl}
...
\section{Associated cycles} \label{sec:Nil}
...
\item  $ \inn{Lu}{Lv}=
  ...
  \item $LJ =  JL$;
  \item  the Hermitian form $(u,v)\mapsto \innbfv{Lu}{Jv}$ on
...
In the notation of Lemma \ref{lem:cartan}, we  call $(V, \la\,,\,\ra, J,L)$ a classical space of signature $\mathsf s$, and use
 $ (V_{\mathsf s}, \la\,,\,\ra_{\mathsf s}, J_{\mathsf s},L_{\mathsf s})$ to denote it. 

Denote by $G_{\mathsf s, \C}$ the isometry group of $ (V_{\mathsf s}, \la\,,\,\ra_{\mathsf s})$, which is an complex orthogonal group if $\epsilon=1$ and a complex symplectic group if $\epsilon=-1$. Respectively denote by  $G_{\mathsf s, \C}^{J_\mathsf s}$  and  $G_{\mathsf s, \C}^{L_\mathsf s}$ the centralizes of $J_\mathsf s$ and $L_\mathsf s$ in  $G_{\mathsf s, \C}$, Then $G_{\mathsf s, \C}^{J_\mathsf s}$ is a real classical group that is isomorphic to 
\[
 \left\{
     \begin{array}{ll}
...
Using the trace form, we identify $\g_{\mathsf s}^*$ with $\g_\mathsf s$. Denote by $\mathrm{Nil}(\g_\mathsf s)$ the set of nilpotent $G_{\mathsf s, \C}$-orbits in $\g_\mathsf s$, and by 
...
Then we have the following two well-defined algebraic maps:
  ...
  \]
These two maps are called the moment maps.  These two maps are both $K_{\mathsf s,\C}\times K_{\mathsf s', \C}$-equivariant. Here  $K_{\mathsf s', \C}$ acts trivially on $\p_\mathsf s$, 
...
\ee
is a single $K_{\mathsf s,\C}\times K_{\mathsf s', \C}$-orbit. Moreover, for every element $\phi$ in this set, there is an exact sequence of algebraic groups:
\[
...
\]
where $\mathbf e:=M_\mathsf s(\phi)$,  $\mathsf s_0$ is a certain classical signature of form $(\star', p_0,q_0)$ ($p_0,q_0\in \bN$), $(K_{\mathsf s,\C}\times K_{\mathsf s', \C})_\phi$ is the stabilizer of $\phi$ in $K_{\mathsf s,\C}\times K_{\mathsf s', \C}$, and $(K_{\mathsf s,\C})_{\mathbf e}$ is the stabilizer of $\mathbf e$ in $K_{\mathsf s,\C}$.
\end{lem}
...
\]
This is  called the descent of $\sO$. It is an element of  $\mathrm{Nil}(\p_{\mathsf s'})$ if $\sO\in \mathrm{Nil}(\p_{\mathsf s})$. 

\subsection{Geometry theta lift}
Let $\zeta_{\mathsf s, \mathsf s'}$ denote the algebraic character on $K_{\mathsf s, \C}\times K_{\mathsf s', \C}$  satisfying the following conditions:
\begin{itemize}
\item
if $\star\in \{B, D, C^*\}$, then 
\[
  \begin{cases}
    (\zeta_{\mathsf s, \mathsf s'})|_{K_{\mathsf s, \C}}=1; \\
    (\zeta_{\mathsf s, \mathsf s'}^2)|_{K_{\mathsf s', \C}}=({\det}_{\sqrt{-1}})^{q-p};\\
    (\zeta_{\mathsf s, \mathsf s'})|_{K_{\mathsf s', \C}}\textrm{ is genuine if $\star=B$};\\
  \end{cases}
\]
\item
if $\star\in \{C, \widetilde C, D^*\}$, then 
\[
  \begin{cases}
    (\zeta_{\mathsf s, \mathsf s'})|_{K_{\mathsf s', \C}}=1; \\
    (\zeta_{\mathsf s, \mathsf s'}^2)|_{K_{\mathsf s, \C}}=({\det}_{\sqrt{-1}})^{p'-q'};\\
    (\zeta_{\mathsf s, \mathsf s'})|_{K_{\mathsf s, \C}}\textrm{ is genuine if $\star=\widetilde C$}.\\
  \end{cases}
\]
\end{itemize}
Note that the genuine condition is automatic unless the two element metaplectic group is concerned. 

Suppose that $\sO$ is a $K_{\mathsf s, \C}$-orbit in $\p_{\mathsf s}$ that is contained in the image of the moment map $M_{\mathsf s}$. Write $\sO':=\nabla^{\mathsf s}_{\mathsf s'}(\sO)$. 
...
\]
 is an algebraic representation of $(K_{\mathsf s,\C})_{\mathbf e}$. Write $\CE:=\check \Theta_{\sO'}^\sO(\mathcal E')$ for the  $K_{\mathsf s,\C}$-equivariant algebraic vector bundle  over $\sO$ whose fibre at $\mathbf e$ equals this coinvariant space representation. In this way, we get an exact functor $ \check \Theta_{\sO'}^\sO$ from the category of 
$K_{\mathsf s,\C}$-equivariant algebraic vector bundle  over $\sO$ to the category of $K_{\mathsf s',\C}$-equivariant algebraic vector bundle  over $\sO'$. Write  $\CK_{\sO'}(K_{{\mathsf s'},\C})$ and $   \CK_{\sO}(K_{\mathsf s,\C})$ for these two categories. Then we get a  homomorphism of the  Grothendieck groups:
\[
  \check \Theta_{\sO'}^\sO:  \CK_{\sO'}(K_{{\mathsf s'},\C})\rightarrow   \CK_{\sO}(K_{\mathsf s,\C}).
\]
This homomorphism is independent of the choice of $\phi$. 


\subsection{Associated cycles of painted bipartitions}

As before, let $\check \CO$ be a Young diagram that has $\star$-good parity, and let $\CO$ be its Barbasch-Vogan dual. Suppose that 
\[
p+q=\abs{\CO}
\]
...
\[
  ...
  \CK_{\CO}(K_{\mathsf s,\C}):=\bigsqcup_{\sO\in \mathrm{Nil}(\p_\mathsf s), \sO\subset \CO } \CK_{\sO}(K_{\mathsf s,\C}).
\]
For every algebraic character $\chi$ of $K_{\mathsf s,\C}$, the twisting map 
\[
\CK_{\CO}(K_{\mathsf s,\C})\rightarrow \CK_{\CO}(K_{\mathsf s,\C}), \qquad \CE\mapsto \CE\otimes \chi
\]
...
In what follows we will define the associated cycle $\mathrm{AC}(\uptau)\in\CK_{\CO}(K_{\mathsf s,\C})$ of $\uptau$.

First assume that $\check \CO=\emptyset$. Then $\CO\cap \p_{\mathsf s}$ is a singleton. If $\star=\widetilde C$, we define $\mathrm{AC}(\uptau)\in\CK_{\CO}(K_{\mathsf s,\C})$ to be the element that corresponds to the one dimensional genuine representation of $K_{\mathsf s,\C} =\{\pm 1\}$. In all other cases, we  define $\mathrm{AC}(\uptau)\in\CK_{\CO}(K_{\mathsf s,\C})$ to be the element that corresponds to the one dimensional trivial representation of $K_{\mathsf s,\C}$.

Now we assume that $\check \CO\neq \emptyset$. As before, let $\check \CO'$ be its dual descent and let $\CO'$ be the Barbasch-Vogan dual of $\check \CO'$. 
Write $
  \uptau':=(\tau', \wp')\in  \PBPe_\star(\check \CO')$ for the descent of $\tau$, and suppose 
that 
\[
  \mathsf s'=(\star', p_{\tau'}, q_{\tau'}). %\uptau'\in \PBPe_\star(\check \CO',\mathsf s').
\] 
As before, we view $\CO'$ as an element of $\mathrm{Nil}(\g_{\mathsf s'})$. 
...
\[
\begin{array}{rcl}
\check \Theta_{\tau'}^{\tau}: \CK_{\CO'}(K_{\mathsf s',\C})&\rightarrow &\CK_{\CO}(K_{\mathsf s,\C}),\smallskip\\
  \CE\in \CK_{\sO'}(K_{\mathsf s,\C})&\mapsto & {\sum}_{\sO}   \check \Theta_{\sO'}^\sO(\CE),
  \end{array}
\]
where $\sO'$ is an element of $\mathrm{Nil}(\p_\mathsf s')$ that is contained in $\CO'$, and $\sO$ in the sum runs over the set
\[
  \{\sO\in \mathrm{Nil}(\p_\mathsf s)\mid  \sO\subset \CO, \sO\textrm{ is contained in the image of $M_\mathsf s$ and $\nabla_{\mathsf s'}^{\mathsf s}(\sO)=\sO'$}\}.
\]

Similar to the definition of $\pi_\uptau$ in the Introduction, we inductively define 
$\mathrm{AC}(\uptau)\in \CK_{\CO}(K_{\mathsf s,\C}) $ by 
 \[
   \mathrm{AC}(\uptau):=\left\{
     \begin{array}{ll}
          %\textrm{the trivial representation $\C$}, &\hbox{if $\abs{\check \CO_\tau}\leq 1$}; \medskip\\
         \check \Theta_{\tau'}^{\tau}(\mathrm{AC}(\uptau'))\otimes ({\det}_{-1})^{\varepsilon_{\tau}}, &\hbox{if  $\star=B$ or $D$}; \smallskip\\
         \check \Theta_{\tau'}^{\tau}(\mathrm{AC}(\uptau')\otimes \det^{\varepsilon_{\wp}}), &\hbox{if $\star=C$ or $\widetilde C$};\smallskip \\
              \check \Theta_{\tau'}^{\tau}(\mathrm{AC}(\uptau')), &\hbox{if $\star=C^*$ or $D^*$}. \\
            \end{array}
   \right.
 \]
...
\begin{prop}\label{thmac1}
 For every $\uptau\in \PBPe_\star(\check \CO)$,  the associated cycle $\mathrm{AC}(\uptau)\in \cK_{\CO}(K_{\mathsf s, \C}) $ is a nonzero  sum of  pairwise distinct elements of $\mathrm{AOD}_{\CO}(K_{\mathsf s, \C})$.
...
Suppose that  $\check \CO$ is pseudo-distinguished.  If $\star\in \{B,D\}$ and $(\star, \check \CO)\neq (D, \emptyset)$, then  the map
\[
\mathrm{AC}: \PBPe_\star(\check \CO,\mathsf s)\times \Z/2\Z \rightarrow  \cK_{\CO}(K_{\mathsf s, \C}),\quad (\uptau, \epsilon)\mapsto \mathrm{AC}(\uptau)\otimes {\det}^{\epsilon}
\]
is injective. In all other cases, the 
map
\[
\mathrm{AC}: \PBPe_\star(\check \CO,\mathsf s)\rightarrow  \cK_{\CO}(K_{\mathsf s, \C})
\]
is injective.
\end{prop}
...
\[
\mathrm{AC}: \PBPe_\star(\check \CO,\mathsf s)\times \Z/2\Z \rightarrow  \mathrm{AOD}_{\CO}(K_\C),\quad (\uptau, \epsilon)\mapsto \mathrm{AC}(\uptau)\otimes {\det}^{\epsilon}
\]
...
The following two propositions are also important for us. 
...
\subsection{Distinction of the representations}


Let $\uptau\in \PBPe_\star(\check \CO,\mathsf s)$. Let $\pi_\uptau$ be the Casselman-Wallach representation of $G_{\mathsf s}$ as defined in the Introduction. 
We will prove the following Theorem in Sections \ref{sec:Integrals} and \ref{sec:AC}.

\begin{thm}\label{thmpitau}
The representation $\pi_\uptau$ is irreducible,  unitarizable and  attached to $\check \CO$. Moreover,  
\[
\mathrm{AC}_\CO(\pi_\uptau)=\mathrm{AC}(\tau)\in \cK_{\CO}(K_{\mathsf s, \C}). 
\]
\end{thm}


As in the Introduction, $\Unip_{\check \cO}(G_\mathsf s)$ is the  set of all isomorphism classes of irreducible Casselman-Wallach  representations of $G_\mathsf s$ that are attached to $\check \CO$.
...
\end{thm}
\begin{proof}
In view of Theorem \ref{thmcount}, we only need to show the injectivity of  the two maps in the Theorem. We prove by induction on $\mathbf c_1(\check \CO)$. The Theorem is trivial in the case when  $\check \CO=\emptyset$. So we assume that $\check \CO\neq \emptyset$  and the Theorem has been proved for the dual descent $\check \CO'$. 
...
Finally, Theorem \ref{thm1} follows from Theorems \ref{thmpitau} and \ref{thmac7}. Theorem \ref{thmac0}   follows from Propositions \ref{thmac1} and \ref{thmac2},  and Theorems \ref{thmpitau} and \ref{thmac7}.
...
\[
\check{\Theta}:   \CK_{\CO'}(K_{{\mathsf s'},\C})\rightarrow   \CK_{\CO}(K_{\mathsf s,\C}),
\]
...
\bib{BMSZ2}{article}{
  ...
  author = {Sun, B.-Y.},
  author = {Zhu, C.-B.},
  title = {Counting the special unipotent representations : orthogonal and symplectic groups},
