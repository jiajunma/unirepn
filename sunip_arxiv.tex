...
This is known as the orbit method (or the method of coadjoint orbits). Due to its
resemblance with the process of attaching a quantum mechanical system to a
classical mechanical system, attaching a unitary representation
...
Lie groups, work of Mackey and Duflo \cite{Ma,Du82} reduce the problem to the case of reductive groups.
As established by Vogan, see for example \cite{VoBook,Vo98,Vo00}, the problem is further
reduced to the case of quantizing nilpotent coadjoint orbits. The ``corresponding''
...
was established by Barbasch as a key part of his classification of the unitary duals of
complex classical groups \cite[Section 10]{B.Class}. Shortly after, Barbasch outlined a proof of
the unitarity of special unipotent representations for real classical groups in his 1990 ICM talk \cite{B.Uni}.
The second major development is Vogan's theory of associated varieties \cite{Vo89}
in which Vogan pursues the method of coadjoint orbits by investigating the
relationship between a Harish-Chandra module and its associated variety. Roughly
...
end of 1980's that the theory is relevant for constructing unitary representations of
classical groups. The relevant works include the classification of unitary highest weight modules \cite{KV78, EHW},
the notion of rank by Howe \cite{HoweRank}, the description of discrete spectrum by Adams \cite{Ad83} and Li \cite{Li90},
...
Vogan. Here $\check \CO$ is a nilpotent adjoint orbit of $\check \g$, the Lie algebra of the Langlands dual of $G$ (or the metaplectic dual of $G$ when $G$ is a real metaplectic group \cite{BMSZ1}).
...
\cite{BVUni} and \cite{B17}), we will focus on the following groups with classical type Lie algebras:
\be\label{typebcd}
...
Let $\check \CO\in \Nil(\check \g)$. An irreducible Casselman-Wallach representation $\pi$ of $G$  is special unipotent attached to $\check \CO$ if
\begin{itemize}
\item  $I_{\check \CO}$ annihilates $\pi$; and
\item $\pi $ is genuine if $G$ is a metaplectic group.
\end{itemize}
Write  $\Unip_{\check \cO}(G)$ for the  set of isomorphism classes of irreducible Casselman-Wallach  representations of $G$ that are special unipotent attached to $\check \CO$.
\end{defn}
...
A main result of the current paper is the construction of all special unipotent representations of $G$, and as a consequence to show that all of them are unitarizable, as predicted by  the Arthur-Barbasch-Vogan conjecture (\cite[Introduction]{ABV}). (We remark that the spherical special unipotent representations in \cite{B10} fit in the framework of this paper as well. The paper highlights the role of special unipotent representations in the classification of the unitary dual.) Along the way, we also determine precise structure of the associated cycles of special unipotent representations of $G$.
...
We introduce the notion of painted bipartitions as well as the descent of a painted bipartition. The key object of study is a (extended) set $\mathrm{\PBPes}(\check \CO)$ of painted bipartitions attached to $\check \CO $. This will be our parameter set for special unipotent representations attached to $\check \CO$. Given a painted bipartition $\uptau \in \mathrm{\PBPes}(\check \CO)$, we attach a real classical group $G_{\uptau}$ of type $B$, $C$, $D$, and we define inductively a special unipotent representation $\pi_{\uptau}$ of $G_{\uptau}$ via the method of theta lifting and in terms of $\pi_{\uptau'}$, where $\uptau'$ is the descent of $\uptau $.
(The fact that $\pi_{\uptau}$ is irreducible, unitarizable and special unipotent attached to $\check \CO$ is in Theorem \ref{thmpitau} and is proved in Section \ref{sec:equac}.)
...
$G_{\uptau}$ and $G_{\uptau'}$, and which permits us to geometrically lift (from $G_{\uptau'}$ to $G_{\uptau}$) nilpotent orbits as well as equivariant algebraic vector bundles over these orbits. This yields ultimately an element in a certain $\CK$ group, which we refer to as the associated cycle of $\uptau$.
...
In Sections \ref{sec:Integrals} to \ref{sec:equac}, we develop the key representation theory tools of the paper. Section \ref{sec:Integrals} is devoted to the model of theta lifting by matrix coefficient integrals, as well as a general criterion on unitarity in this context. In Section \ref{sec:dtheta}, we investigate double theta lifting and its connection with degenerate principal series representations. (Some of the key ideas of Section \ref{sec:dtheta} were due to He \cite{He}, and may be viewed as a variant of the doubling method \cite[Section II]{Ra}.) In Section \ref{sec:AC}, we prove ``upper bounds'' on the associated cycles, using largely geometric arguments. In Section \ref{sec:equac}, we match the associated cycle of $\pi_{\uptau}$ with the associated cycle of $\uptau$, as a consequence of the results on associated cycles in Section \ref{sec:AC} and results of Section \ref{sec:dtheta}.
%(Heuristically one may view results of Section \ref{sec:dtheta} as providing a bound of associated cycles from below.)
...
We call $\star'$
 ...
 \[
  ...
  We define the descent of primitive pairs to be the map
  \[
    \begin{array}{rcccl}
      \DD\colon & A(\ckcO) & \rightarrow & A(\ckcOp)\\
      & \wp &\mapsto & \wp':= &\set{(i,i+1)| (i+1,i+2)\in \wp}.
    \end{array}
  \]
...
We first define the naive descent of a painted bipartition. Let
\[
\tau=(\imath,\cP)\times (\jmath,\cQ)\times \alpha
\]
be a  painted bipartition such that $\star_\tau=\star$. Put
...
\[
\tau=(\imath,\cP)\times(\jmath,\cQ)\times \alpha \in  \PBP_\star(\tau_{\wp})
\]
and write
\[
...
For notational convenience, we shall also write $G_{\uptau}:=G_{\tau}$ and $G_{\uptau'}:=G_{\tau'}$.
 ...
 In what follows we will construct a representation $\pi_{\uptau}$ of $G_{\uptau}$ by the method of theta lifting. For the initial case when $\check \CO$ is the empty Young diagram, define
 \[
 \pi_{\uptau}:= \left\{
     \begin{array}{ll}
 ...
 \textrm{the one dimensional trivial representation} , &\hbox{ otherwise.}            \end{array}
   \right.
\]
When $\check \CO\neq\emptyset$,   we define the representation $\pi_{\uptau}$ of $G_{\uptau}$ by induction on the number of nonempty rows of $\check \CO$:
...
\def\Vsp{V_{\sfss'}}
\begin{lem}
 % [{\cf \cite[Lemma 13]{Ohta}, \cite[Lemma 3.4]{Zh}}]
  \label{descko}
...
\begin{proof}
 % Fix $\bfee\in \sO$, it suffice to show that $\Ms^{-1}(\bfee)\cap \cXsso$ is a single $\Ksp$-orbit.
...
%    \begin{array}{l}\text{$V''$ is $\Lsp$-invariant, non-degenerate and}\\
%  \text{\eqref{eq:Imphi} is satisfied}
  \]
...
\noindent (a) For every $\uptau = (\tau,\wp)\in \PBPes(\check \CO)$, the representation $\pi_{\uptau}$ of $G_\tau$  is irreducible and special unipotent attached to $\ckcO$.
...
The reduction of \cite{BMSZ2} allows us to classify all special unipotent representations attached to a general $\check \CO\in \Nil(\check \g)$ from those which have $\star$-good parity, by using irreducible unitary parabolic inductions. We thus conclude the following generalization of Corollary \ref{cor1}.
...
The representation $\pi_\uptau$ is irreducible, unitarizable and special unipotent attached to $\check \CO$. Moreover,
\[
\mathrm{AC}_\CO(\pi_\uptau)=\mathrm{AC}(\tau)\in \CK(\CO).
\]
\end{thm}

As mentioned in Section \ref{classical}, our group $G$ is also denoted by $G_\mathsf s$ (to indicate its dependence on the classical signature $\mathsf s$). Recall the set $\PBPe_\star(\check \CO,\mathsf s)$ (in \eqref{def:PBPs}) and the set $\Unip_{\check \cO}(G_\mathsf s)$ of isomorphism classes of irreducible Casselman-Wallach representations of $G_\mathsf s$ that are special unipotent attached to $\check \CO$.
...
\begin{itemize}
  ...
  \item $L_{\St{i}} = (-1)^{\floor{\frac{i-1}{2}}}\, \Sym^{i-1}(\text{\tiny$\begin{pmatrix}\phantom{-}0 & 1 \\-1 & 0 \end{pmatrix}$})$,
  \item $\innsti{\varphi_i(x) u}{v}+ \innsti{u}{\varphi_i(x) v} = 0$ for all  $u,v\in \St{i}$ and $x\in \slt$,
  \item  the Hermitian form $(u,v)\mapsto \innsti{\Lsti(u)}{\Jsti(v)}$ on $\St{i}$ is positive definite.
\end{itemize}
Note that $\innsti{}{}$ is a uniquely determined by the above conditions up to a positive scalar.
We fix the paring $\innsti{}{}$ for each $i$.
...
\bib{BV83}{article}{
 ...
 journal = {Representation theory of reductive groups (Park City, Utah, 1982), Progress in Math.},
 volume = {40},
 pages = {21--33},
 year = {1983}
}
...
journal={Representation Theory of Reductive Groups. Progress in Mathematics, vol 40. Birkh\"auser, Boston},
year={1983},
pages={97--144},
}
