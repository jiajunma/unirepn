...
It is easy to see the following formula and we made the following definition:
\[
  \begin{split}
  \Tdk &:= f_{D,\yrow{2k-1,1},\emptyset}^{\set{d}}= \bfpp\bfqq(\RS_{k-1} + \bfpp\bfqq\RS_{k-2}) \\
  \Trck &= f_{D,\yrow{2k-1,1},\emptyset}^{\set{rc}}= (\bfpp\bfqq+\bfpp^{2})\RS_{k-2}\\
  \Tsk &= f_{D,\yrow{2k-1,1},\emptyset}^{\set{s}}= \bfqq^{2n}
  \end{split}
\]
...
% \begin{defn}
% The dual descent of a partition $\ckcO$ is the partition $\ckDD$
...
% \subsubsection{Naive descent of a painted bipartition }
% \def\bipartl{\mathrm{bi\cP_L}}
% \def\bipartr{\mathrm{bi\cP_R}}
% \def\dsdiagl{\mathrm{DS_L}}
% \def\dsdiagr{\mathrm{DS_R}}
...
% \subsubsection{Descents of painted bipartitions}\label{sec:desc}
