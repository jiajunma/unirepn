...
\begin{itemize}
\item 
empty in $\check \CO$, if $\mathbf r_i(\check \CO)=\mathbf r_{i+1}(\check \CO)=0$;
\item
...
\item 
distinguished in $\check \CO$, if  it is not empty, balanced or tailed in $\check \CO$. 
\end{itemize}

\end{defn}

Let $\mathrm{DP}_\star(\check \CO)$ denote the  set of all $\star$-pairs that are distinghuished in $\check \CO$. For every subset $\wp\subset \mathrm{DP}_\star(\check \CO)$, we define a pair \[
   ...
   \left\{
     \begin{array}{ll}
        ...
        (\frac{\mathbf r_{2i}(\check \CO)-1}{2},  0), & \hbox{if $(2i, 2i+1)$ is tailed in $\check \CO$};\smallskip\\
         (0,  0), &\hbox{if $(2i, 2i+1)$ is empty in $\check \CO$};\\
   ...
   \left\{
     \begin{array}{ll}
        ...
        (\frac{\mathbf r_{2i-1}(\check \CO)-1}{2},  0), & \hbox{if $(2i-1, 2i)$ is tailed in $\check \CO$};\smallskip\\
         (0,  0), &\hbox{if $(2i-1, 2i)$ is empty in $\check \CO$};\\
...
Put
\[
\mathrm{BP}_\star(\check \CO) :=\{(\imath_\wp, \jmath_\wp)\mid \wp\subset \mathrm{DP}_\star(\check \CO)\}
\]
and
\[
...
Let $\imath$ be a Young diagram. Write $\mathrm{Box}(\imath)$ for the set of all boxes in $\imath$, which has $\abs{\imath}$ elements in total. We say that a Young diagram $\imath'$ is contained 
...
\begin{itemize}
\item
 $\CP^{-1}(S)$ is a Young subdiagram of $\mathrm{Box}(\imath)$ when $S=\{\bullet\}, \{\bullet, s \}, \{\bullet, s, r\}$ or $\{\bullet, s, r, c \} $;
 \item
...
\item
 $\CP^{-1}(\bullet)$ and $\CQ^{-1}(\bullet)$ corresponding to the same Young diagram;
 \item
...
\begin{itemize}
\item $\star\in \set{C^*,D^*}$;
\item 
 $\star\in \set{C,\wtC}$ and $(1,2)\notin \DP(\ckcO)$;
\item  $\star \in \set{B,D}$ and $(2,3)\notin \DP(\ckcO)$.
\end{itemize}
...
\section{The main results}\label{sec:intro}
...
Let $\epsilon=\pm 1$.  Let $V$ be an $\epsilon$-symmetric complex bilinear space, namely a finite dimensional complex vector space equipped with a non-degenerate $\epsilon$-symmetric bilinear form $\la \,,\,\ra_V: V\times V\rightarrow \C$.  Write $G_V$ for the isometry group of $V$, and write $\g_V$ for the  Lie algebra of $G_V$.

 
We identify $\g_V^*$ with $\g_V$ via the trace form. Let $\mathrm{Nil}(\g_{ V}^*)$ ($=\mathrm{Nil}(\g_{ V})$) denote the set of  nilpotent  $G_{ V}$-orbits in $\g_{ V}^*$ ($=\g_V$).
The following  result is due to Dixmier.
...
\subsubsection{The oscillator representation}\label{secosc0}
...
\begin{Example} Suppose that $\star=C$, and $\check \CO$ is the following Young diagram which is $\star$-even. 
\begin{equation*}\label{eq:sp-nsp.C}
  \tytb{\ \ \ \ \ \ \ \ \ , \ \ \ \ \ , \ \ \ \ \ , \ \ \ \ \ , \ \ \ , \  ,\  }
   \end{equation*}
   Then 
\[
  \mathrm{DP}_\star(\check \CO)=\{(1,2), (5,6)\}.
\]
and $(\imath_\star(\check \CO, \wp), \jmath_\star(\check \CO, \wp))\in \mathrm{BP}_\star(\check \CO)$ has the  following form. 
...
\section{The case of general orbits}\label{secapp}

Let $G$, $\check \g$ and  $\star\in \{B, C, \widetilde C, D, C^*, D^*\}$ be as in Section \ref{secsu}. Suppose that $G$ has type $\star$.  Let $\check \CO\in \Nil(\check \g)$, to be considered as a Young diagram as usual.  By counting the row lengths, we identify Young diagrams with finite multisets of positive integers. We have a unique pair $(\check \CO_{\star-\mathrm{good}}, \check \CO_{\star-\mathrm{bad}})$ of  Young diagrams such that 
\begin{itemize}
\item   if $\star\in \{B, \widetilde{C}\}$, then  all nonzero row lengths of $\check \CO_{\star-\mathrm{good}}$ are even and all nonzero row lengths of $\check \CO_{\star-\mathrm{bad}}$ are odd;
\item
 if $\star\in \{C, D, C^*, D^*\}$, then  all nonzero row lengths of $\check \CO_{\star-\mathrm{good}}$ are odd and all nonzero row lengths of $\check \CO_{\star-\mathrm{bad}}$ are even;
\item
as  multisets, $\check \CO$ is the union of $\check \CO_{\star-\mathrm{good}}$ and $\check \CO_{\star-\mathrm{bad}}$. 
\end{itemize}

Note that $\check \CO_{\star-\mathrm{good}}$  has $\star$-good parity as defined in Section \ref{secbip}. Also 
note that every positive integer occurs in the multiset $\check \CO_{\star-{\mathrm{bad}}}$  with even multiplicity. Thus  there is a Young diagram $\frac{1}{2}\check \CO_{\star-{\mathrm{bad}}}$  such that the union of it with itself equals $\check \CO_{\star-{\mathrm{bad}}}$. 
...
\[
  \mathbb K:=   
   \left\{
  \begin{array}{ll}
        \R, &\hbox{if $\star=B, C, D$ or $\widetilde{C}$}; \smallskip\\
         \textrm{the  quaternions algebra $\mathbb H=\R\oplus \R\mathbf i\oplus \R\mathbf j\oplus \R\mathbf k$}, & \hbox{if $\star=C^*$ or $D^*$},
            \end{array}
   \right.
\]
and put
...
l:=l_{\check \CO}:= \left\{
  \begin{array}{ll}
        \frac{1}{2}\abs{\check \CO_{\star-{\mathrm{bad}}}}, &\hbox{if $\star=B, C, D$ or $\widetilde{C}$}; \smallskip\\
        \frac{1}{4}\abs{\check \CO_{\star-{\mathrm{bad}}}} , & \hbox{if $\star=C^*$ or $D^*$}.
            \end{array}
   \right.
\]
Special unipotent representations of general linear groups are well-understood. 
Similar to the situation  in Section \ref{secsu}, write $\mathrm{Unip}_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}(\GL_{l}(\mathbb K))$ for the set of all isomorphism classes of  irreducible Casselman-Wallach representations of $\GL_{l}(\mathbb K)$ that are attached to $\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}$. The row lengths of the Young diagram $\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}$ forms a partition of $\abs{\frac{1}{2}\check \CO_{\star-{\mathrm{bad}}}}$, and hence corresponds to a standard parabolic subgroup $P_{\frac{1}{2}\check \CO_{\star-{\mathrm{bad}}}}$ of $\GL_{l}(\mathbb K)$. It is well-known that
\[
  \mathrm{Unip}_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}(\GL_{l}(\mathbb K))=\left\{\Ind_{P_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}}^{\GL_{l}(\mathbb K))} \chi\mid \chi \textrm{ is a quadratic character of } P_{\frac{1}{2}\check \CO_{\star-{\mathrm{bad}}}}\right\}.
\]
Note that this set is a singleton in the quaternionic case. 

When the real rank of $G$ is at least $l$, respectively define 
\[
 G_l:=  \oO(p-l,q-l), \  \Sp_{2n-2l}(\R), \  \widetilde \Sp_{2n-2l}(\R), \ \Sp(p-l,q-l), \  \textrm{or }   \oO^*(2n-2l)
 \]
 if 
 \[
  G= \oO(p,q), \Sp_{2n}(\R), \  \widetilde \Sp_{2n}(\R), \ \Sp(p,q), \ \textrm{or }  \oO^*(2n).
 \]
 


If $\star=\widetilde C$ so that $G=\widetilde \Sp_{2n}(\R)$. Write $\varepsilon_G$ for the non-trivial element in the kernel of the covering map $G\rightarrow \Sp_{2n}(\R)$. The group $\Sp_{2n}(\R)$ has a Levi subgroup of the form $\GL_{l}(\R)\times \Sp_{2n-2l}(\R)$. Write $\widetilde{\GL}_{l}(\mathbb R)$  for the inverse images of 
${\GL}_{l}(\mathbb R)$  under the covering map $G\rightarrow \Sp_{2n}(\R)$. Then  
\[
  L_l:=  \frac{\widetilde{\GL}_{l}(\mathbb R)\times G_l}{\textrm{the diagonal group }\ \{1,\varepsilon_G\}} 
\]
is a Levi subgroup of $G$. We fix a finite order genuine character $\zeta_l$ of $\widetilde{\GL}_{l}(\mathbb R)$. 

 
 If $\star\neq \widetilde C$, then the group $G$ has a Levi subgroup of the form 
\[
     L_l:=  \GL_{l}(\mathbb K)\times G_l.
     \]

 
In all case, let $P_l=L_l\ltimes N_l$ be a parabolic subgroup of $G$ such that $L_l$ is a Levi factor of it and $N_l$ is the unipotent radical. Every representation of $L_l$ is viewed as a representation of $P_l$ with the trivial action of $N_l$. Given  $\sigma\in \mathrm{Unip}_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}(\GL_{l}(\mathbb K))$ and $\pi\in \mathrm{Unip}_{\check \CO{\star-{\mathrm{good}}}}(G_l)$, put
\[
  \sigma\ltimes \pi:= \left\{
  \begin{array}{ll}
       \Ind_{P_l}^{G} ((\sigma\otimes \zeta_l)\widehat \otimes \pi), &\hbox{if $\star=\widetilde{C}$}; \smallskip\\
          \Ind_{P_l}^{G} (\sigma \widehat \otimes \pi) , & \hbox{otherwise}.
            \end{array}
   \right.
\]
...
\begin{thm}
\noindent (a) If the real rank of $G$ is smaller than $l$, then the set $\mathrm{Unip}_{\check \CO}(G)$ is empty.

\noindent (b) Suppose that  the real rank of $G$ is greater than or equal to  $l$. Then for  every $\sigma\in \mathrm{Unip}_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}(\GL_{l}(\mathbb K))$ and $\pi\in \mathrm{Unip}_{\check \CO{\star-{\mathrm{good}}}}(G_l)$, the representation $\sigma\ltimes \pi$ is irreducible and belongs to the set $\mathrm{Unip}_{\check \CO}(G)$.
Moreover, the map 
\[
\mathrm{Unip}_{\frac{1}{2} \check \CO_{\star-{\mathrm{bad}}}}(\GL_{l}(\mathbb K)) \times\mathrm{Unip}_{\check \CO{\star-{\mathrm{good}}}}(G_l) \rightarrow \mathrm{Unip}_{\check \CO}(G),\qquad (\sigma, \pi)\mapsto \sigma\ltimes \pi
\]
is bijective. 
\end{thm}
...
\printindex
  ...
  \[
       \tytb{{\cdots}\cdots\cdots\cdots,{\ast}{\ast}{\cdots}{\cdots},{*(srcol)s}{*(srcol)r}{\cdots}\cdots,
        {*(srcol)\vdots}{*(srcol)\vdots},{*(srcol)s}{*(srcol)r},{x_{0}}{y_{1}},{x_{1}}{y_{2}},{\vdots},{x_{n}}}
               \]
