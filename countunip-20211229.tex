...
\begin{thm}\label{countup}
  ...
  \[
    ...
    \left\{
    \begin{array}{ll}
        2^{\sharp(\CPP_\star(\check \CO))} \cdot \sharp (\PBP_{G}(\ckcO)),  &\hbox{if $\star\in \{B, C,D,\widetilde {C}\}$}; \smallskip\\
       \sharp (\PBP_{G}(\ckcO)),  & \hbox{if $\star\in \{C^*,D^*\}$}.
    \end{array}
  \right.
  \]
\end{thm}
...
\begin{lem}\label{hcgoldie}
  ...
  minimal among all representations occurs in $\Coh_{[\lambda]}(\CK(G))(\CC)$.
  % that
  % \begin{itemize}
...
The coherent continuation representation may be computed by using the basis  $\set{\Psi_{\olgamma}}_{\olgamma\in \cP_{\Lam}(G)}$ (see \cite[Definition 14.4]{V4}). 
...
\begin{proof} First note that $\WF(\pi')=[\sO']$.
  ...
  \]
  Here $\star' \in \set{A^\R,A^{\bH}}$ depending on $G$ and the last equality is
  \Cref{prop:BP.PP}.
...
%\subsubsection{Injectivity }
 We first prove the injectivity of $\fI$ by translating problems to the
 ``endoscopic group'' $\Gb\times \Gg$ at regular infinitesimal
 character. The method was used in \cite{Mat} and also see
...
To reconcile the notation in \cite{GI}, we set
% $G'_{2}:=\Gpb \times \Gg$,
%   $G_{2} := G_{\mathrm b}\times G_{\mathrm g}$ and $G_{1}:=G$.
\[ G'_{2}:=\Gpb \times \Gg \AND
  G_{2} := G_{\mathrm b}\times G_{\mathrm g} %$ and $G_{1}:=G$.
\]
  We always fix an identification of $\Gpb$ with a the Levi subgroup of a fixed Siegel parabolic
  in $\Pb$ in $\Gb$ and so $G'_{2}$ as a subgroup of $G_{2}$.

  Fix a maximally split Cartan subgroup
  $H_{2}^{s}\subset\Gpb\times \Gg\subset \Gb\times \Gg$.
  There is a is naturally mapping to a maximally split Cartan $H^{s}$ in $G$ via the natural
  embedding $\Gpb\times \Gg \subseteq P\subseteq G$. The map induces an
  isomorphism of $\Lie(H_{2}^{s})$ and $\Lie(H^{2})$ and is an isomorphism
  except the metaplectic group case \cite{RT2}*{\S 5}. Moreover, there are
\begin{itemize}
  \item a natural embeddings $\Gpb\subseteq \Gb\subseteq G$ and $\Gg\subseteq G$
  ...
  $\Lie(H_{2}^{s})$
\end{itemize}
  such that the choice of $\fbb_{2}$ and $\fbb$ compatible in the sense that
  ...
  With the above fixed choices of Borel subgroups, we identify the abstract Cartan subalgebras
  ...
  The coherent family and coherent continuation representation can
  extend to these lattices, \cf \cite{Vg}*{Lemma~7.2.6}.
  By abuse of notation,
  we have $\Coh_{\Lamck}(\cK'(G'_{2}))$ and $\Coh_{\Lamck}(\cK'(G_{2}))$ where
  $\cK(G'_{2})$
  and $\cK(G_{2})$ denote the Grothendieck groups of genuine representations
  ...
  \]
  constructed by matching regular characters.   % of $G'_{2}$, $G_{2}$ and $G$.
  See \cite{GI}*{p 97} for the odd orthogonal group case, \cite{RT2}*{p26} for
  the metaplectic group case, and the other cases are similar which can also be
  established by using the Vogan duality twice \cite{V4}*{Section~11}.
  In all cases, the map $f$ is injective.
  \trivial[]{
...
\varphi:=\varphi^{f}\colon&  \Coh_{\Lamck}(\cK(G_{2})) &\longrightarrow& \Coh_{\Lamck}(\CK(G)) & \\
  &  \Psi_{\olgamma_{2}} & \mapsto & \Psi_{f(\olgamma_{2})} & \text{for each }
  ...
  \[
    \varphi_{\nu}\left((\Ind_{P_{b}\times \Gg}^{\Gb}\pi')\otimes \pi_{0}\right) = \Ind_{P}^{G}(\pi'\otimes \pi_{0})
...
\begin{proof}
  \begin{enumT}
    \item This is clear by the construction.
    \item  The $\varphi$ is $\WLam$ can be see directly from the explicit
    ...
    see \cite{RT2}*{Theorem~1.1} for metaplectic groups, \cite{GI}*{p91} and
    \cite{ABV}*{Theorem~16.22}.
    \item The claim on the injectivity of $\varphi_{\nu}$ is clear from
    \Cref{prop:ev} and the injectivity of \eqref{eq:cohinj}.\trivial[h]{Note
      that $\WLamck$ is a finite group!}

    For the second claim, may
  ...
  By the injectivity of $\varphi_{\nu}$, it is non-zero and so the
   ...
   parabolic induction \cite{Vg}*{Theorem~6.6.14}, the claims follow from induction by stages.
   ...
   cit. non-vanish since $\Pb$ and $P$ are real parabolic subgroups.
   The claim follows.
  \end{enumT}
\end{proof}
...
\begin{proof}[Proof of \Cref{thm:red}]
  In views of \Cref{lem:cohred}~\ref{it:phinu} and \ref{it:ind}, the injectivity
  of $\fI$ and the irreducibility of $\pi'\rtimes \pi_{0}$ follows immediately
  from those claims in the bad parity case, see \Cref{lem:Unip.BP}.

  The representation $\pi'\rtimes \pi_{0}$ is in $\Unip_{\ckcO}(G)$ now immediately follows from
  the formula on wavefront cycle of parabolic induction \cite[Corollary
  5.0.10]{B.Orbit}.
  \trivial[]{
...
\begin{proof}
 ...
 Without loss of generality we assume that $\lambda_{\check \CO}$ is dominant.  Let $\Psi_{2}$ be a basal element in
  ...
  Since $\pi \neq 0$ and $\pi_{2}\neq 0$ has infinitesimal character $\lamck$,  we have
...
\end{split}
  ...
  \end{equation}
  Then the lemma follows by taking dimensions of the both sides.
  ...
  attached to $I_{\lamck}$ dose not occurs in $\Coh_{B_{2}}$ and $\ev{\lamck}(\Coh_{\Lamck}(\cK_{\bcO}(G))\cap \Coh_{B_{2}}) = 0$ by
...
\begin{proof}
Suppose that
  $\emptyset\neq \wp\subset \CPPs(\ckcO)$, and there was an element $\uptau = (\imath_{\wp}, \cP)\times (\jmath_{\wp},\cQ)\times \star\in \PBP_{\star}(\tau_{\wp})$.
 ...
 This contradicts \eqref{eq:res.D*} and therefore  $\PBP_{\star}(\tau_{\wp})= \emptyset$.
