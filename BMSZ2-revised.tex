...
In this appendix we spell out the computation of orbit induction and classify the open locus in several special cases used in \Cref{prpaseq}.  Let $D\in\{\mathbb R,\mathbb C,\mathbb H\}$
...
If $g\in Z_G(x)$ fixes the two lines, then $g$ must perserve $E$ by definition. Which means $g\in Z_G(X_{S,T}) \cap P  =  Z_P(X_{S,T})$.  
$m(G\cdot X_{S,T},\cO_0)= [Z_{G}(X_{S,T}) : Z_{P}(X_{S,T})]=2$.
\end{proof}

\subsection{The generalized good descent case}\label{sec:app-gd}

We now specialize to the case $\star\in\{B,D\}$, i.e.\ $\bar{\ }=\id$ and $D=\mathbb R$.  Write $\cO_0=\cO\subset\mathfrak g_0$ and let $\cOp$ denote an open orbit in $\Ind_P^G(\cO)$.

Suppose that
\[
\bfcc_1(\cOp)-\bfcc_2(\cO)>1,\qquad \kappa=\bfcc_1(\cOp)-1\ge \bfcc_2(\cO).
\]
Then $c=\bfcc_1(\cO)$ and $r=\kappa-c=\bfcc_1(\cOp)-1-\bfcc_1(\cO)$.  Since $\star\in\{B,D\}$ and $\epsilon=1$, the forms in $\mathrm{Iso}_{r,-\epsilon}=\mathrm{Iso}_{r,-1}$ are skew-symmetric bilinear forms on $\mathbb R^r$ of maximal rank.  When $r$ is even, such forms are non-degenerate and there is a unique isometry class; when $r$ is odd, the maximal rank is $r-1$ and again there is a unique isometry class (with one-dimensional radical).  In either case $|\mathrm{Iso}_{r,-1}|=1$, so the open induced orbit is unique.
