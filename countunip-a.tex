...
Define the set $\CPP_\star(\ckcOg)$ as in the real case. Then by the work of Barbasch-Vogan (\cite[Corollary 5.29]{BVUni}), we have  
   \[
    \sharp(\Unip_{\check \CO}(G))=\sharp(\Unip_{\ckcOg}(\Gg))=2^{\sharp(\CPP_\star(\ckcOg))}.
  \]
   As in the real case, every representation in $\Unip_{\check \CO}(G)$ is obtained through irreducible parabolic induction via those of $\Unip_{\ckcO'_{\mathrm b}}( G'_{\mathrm b})\times \Unip_{\ckcO_{\mathrm g}}( G_{\mathrm g})$ (see Theorem \ref{reduction}), and every representation in $\Unip_{\ckcOg}(\Gg)$ is obtained through iterated theta lifting (see \cite[Theorem 3.5.1]{B17} and \cite{BMSZ2}).
...
\begin{prop}\label{prop:ev}
  Suppose  $W_{\Lam}$-submodule $\cS$ of $\Coh_{\Lam}(\cK(G))$ is basal.
  ...
  where
  \[
    \cS_{W_{\nu}}:=  \cV /\Span\Set{\Phi- w\cdot \Phi | \Phi\in \cS, \, w\in W_{\nu}}
  \]
  is the maximal $W_{\nu}$-invariant quotient of $\cV$.
\end{prop}
\begin{proof}
  \def\BS{\cB_{\cS}}
Without loss of generality we assume that $\nu$ is dominant.
Let $\BS$ be the basis of $\cS$.
% By \Cref{sur111}, there is a surjective evaluation  map
%   \[
%     \ev{\nu, \sfS} \, :\, \Coh_{[\lambda]}(\CK_\sfS(G)) \longrightarrow \Grt_{\nu, \sfS}(G).
%   \]
  ...
  \begin{eqnarray*}
    ...
    &  \subseteq &  \ker (\ev{\lambda}|_{\cS}). \qquad \text{(by $(\Phi-w\cdot \Phi)(\nu) = \Phi(\nu)-\Phi(w^{-1}\cdot \nu) = 0$)}\\
  \end{eqnarray*}
