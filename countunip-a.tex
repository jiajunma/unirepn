...
polynomial function $\tilde p_{\g,\Lam,w}$ on $\h^*$, called a \emph{Goldie rank polynomial} such that
\begin{itemize}
...
The fundamental result \Cref{thm:prim} due to Joseph shows that the Goldie rank polynomials
classify the set of primitive ideals.
 Recall the coherent family defined in \eqref{eq:hw.b}.
By the construction of coherent family via translation functor, one can see that
if $w,w'\in \WLam$ such that
$\Ann(\overline\Psi_{w} (\nu)) = \Ann(\overline \Psi_{w'}(\nu))\subsetneq \cU(\fgg)$
for a dominant element $\nu\in \Lam$, then
$\Ann(\overline\Psi_{w} (\mu)) = \Ann(\overline \Psi_{w'}(\mu))$ for every dominant
...
\begin{thm}[\cite{J2}*{Theorem~5.4 and Theorem~5.5}]
  ...
  For $J_{1}, J_{2}\in \Prim_{\nu}(\fgg)$,
  \[
    J_{1}= J_{2}\quad  \text{ if and only if }\quad \tpp_{J_{1}} =\tpp_{J_{2}}
  \]
  Moreover, for each $J\in \Prim_{\nu}(\fgg)$,
  \begin{itemize}
    \item the $\WLam$-subrepresentation $\sigma_{J}$ in $\bC[\fhh^{*}]$ generated by
    $\tpp_{J}$ is a univalent irreducible representation of $\WLam$;
    \item the polynomial $\tpp_{J}$ is homogeneous whose degree equals the fake
    degree $a(\sigma_{J})$.
  ...
  % \]
  \end{itemize}
   The representation $\sigma_{J}$ is called the \emph{Goldie rank representation} attached to $J$.
\qed
\end{thm}
...
$\phi\in \CB$, write $\la \phi\ra:=\la \{\phi\}\ra$ for simplicity. On the set
$\CB$, define a preorder $\leq$ by
\[
  \phi_2\leq \phi_1 \quad \textrm{ if and only if
  } \quad \la \phi_1\ra \subset \la \phi_2 \ra,
\]
...
We recall some results about the order on $\Irr(\Wlam). $
The preorder $\leqLR$ on $\Coh_{\Lam}(\cK_{\Lam}(\fgg,\fbb))$ induces a preorder on $\Irr(\WLam)$:
\[
\sigma_{1}\leqLR \sigma_{2} \quad \text{if and only
  if} \quad \exists\, \text{a double cell $\cC$ such that }
\begin{array}{l}\sigma_{1}\otimes \sigma_{1} \text{ occurs in }
  V(\cC) \text{ and }\\
  \sigma_{2}\otimes \sigma_{2} \text{ occurs in } \braket{\overline \cC}.
\end{array}
\]
We say $\sigma_{1}\approxLR \sigma_{2}$ if and only
$\sigma_{1}\leqLR \sigma_{2}\leqLR\sigma_{1}$.
Clearly double cells in $\Irr(\WLam)$ are nothing but the equivalent class of
$\approxLR$.
% The some properties about the preorder $\leqLR$ is summaried in the following
% lemma. %see \cite{BV2}
\begin{lem}\label{lem:ord}
  \begin{enumerate}
    \item Suppose $J_{1},J_{2}$ are two primitive ideals $\cU(\fgg)$ such that $J_{1}\subsetneq J_{2}$. Then
          \begin{enumerate}[label=(\alph*),ref=(\alph*)]
            \item\label{it:o1} $\AVC(J_{1})\supsetneq \AVC(J_{2})$ and
            \item\label{it:o2} $\sigma_{J_{1}}\lneqLR \sigma_{J_{2}}$.
          \end{enumerate}
    \item Suppose $\sigma_{1}$ and $\sigma_{2}$ are in $\Irrsp(\WLam)$ such that
    $\sigma_{1}\leqLR\sigma_{2}$ and $\sigma_{1}\neq \sigma_{2}$.
   Then $\sigma_{1}\lneqLR\sigma_{2}$ and $a(\sigma_{1})<a(\sigma_{2})$.
  \end{enumerate}
  \trivial[h]{
    \begin{enumerate}
      \item Fix a regular dominant vector $\nu\in \Lam$. The map from
            $\set{\cL left cells in \Coh_{\Lam}}(\cK_{\Lam}(\fgg,\fbb))$ to
            $\Prim_{\nu}(\fgg)$ by $\cL\mapsto \Ann(\phi(\nu))$ for any
            $\phi\in \cL$ is well defined an order preserving bijection.
    \end{enumerate}
  }
\end{lem}
\begin{proof}
  \begin{enumerate}
    \item
    The statement \ref{it:o1} is \cite[Korollar 3.6]{BK}.
    For \ref{it:o2},
    it suffice to  assume $J_{1}$ and $J_{2}$
    has the same regular infinitesimal character by translation principle.
    Now the statement is proved in \cite{BV2}*{Proof of Proposition~3.18 (c)}.
    \item This is \cite{BV2}*{Lemma~3.22}.
  \end{enumerate}
\end{proof}
...
\begin{equation}\label{eq:Pram}
  \cP_{\Lam}(G) := \bigcup_{H} \overline \Irr_\Lam(H)
\end{equation}
be the set of parameters for the coherent families based on $[\lambda]$
where $H$ running over a representative set of conjugacy classes of Cartan
subgroups of $G$.

Fix a regular dominant element $\mu\in \Lam$.
For each parameter
$\overline\gamma \in\cP_{\gamma}$, we pick a $(\fbb,\Gamma, \nu)\in \overline \gamma$ such that $\xi_{\fbb}(\mu)=\nu$.
Let $\rX((\Gamma,\nu))$ be the standard module defined in \cite{Vg}*{Notational
  Convention~6.6.3} (where $\bargamma$ is $\nu$ in our setting).
Since $\nu$ is non-singular,
$\rX((\Gamma,\nu))$ has a unique irreducible submodule $\olrX((\Gamma,\nu)))$, see
\cite{Vg}*{Theorem~6.5.10}.
Similar to the highest weight module case, for each parameter
 $\overline\gamma \in\cP_{\gamma}$
we can define a coherent family $\Psi_{\olgamma}$ standard module and a coherent
family $\olPsi_{\olgamma}$ of irreducible module such that
\begin{equation}\label{eq:psigamma}
  \Psi_{\olgamma}(\mu):=\rX{((\Gamma,\nu))}
  \AND
  \olPsi_{\olgamma}(\mu):=\olrX{((\Gamma,\nu))}.
\end{equation}
The Langlands classification implies that
$\set{\olPsi_{\olgamma}|\olgamma\in \cP_{\Lam}(G)}$ is our fixed basis of
$\Coh_{\Lam}(\cK(G))$.  % \cite{Vg}*{??? cc}.

The set $\set{\Psi_{\olgamma}}$ is also a basis of $\Coh_{\Lam}(\cK(G))$ under
which the coherent continuation representation is relative easy to compute due
to Barbasch-Vogan.
...
\newcommand{\ffcoh}{\varphi}
  ...
  The rest of the section is devoted to prove the theorem.



%Recall the notion of the set of regular characters
%
  We set $G_{1} := G_{\mathrm b}\times G_{\mathrm g}$ and $G_{2}:=G$.

\subsubsection{The injectivity of $\fI$}
...
\cite{GI}*{Section~3} for a comprehensive account.
  ...
  Fix a regular dominant vector $\lambda\in \lamck$.

  Consider the embedding,
  \begin{equation}\label{eq:cohinj}
    \Coh_{[\lamckb]}(\CK(\Gb))\otimes \Coh_{[\lamckg]}(\CK(\Gg)) \xrightarrow{\ \ \ \ffcoh\ \ \ } \Coh_{[\lamck]}(\CK(G)).
  \end{equation}

% There is a natural map $f\colon $
...
% $f \colon \fhh\rightarrow \fhh$ is given by putting the coordinates of
...
%  We first record the following lemma which translating to the
% ``endoscopic group'' $\Gb\times \Gg$. The method was used in \cite{Mat} and also see
% \cite{GI}*{Section~3} for a comprehensive account.


\subsubsection{The injectivity of $\fI$}

 We first prove the injectivity of $\fI$. Let % $\mu_{2} = \lamck$
      % and
  $\mu_{1} = \ff^{-1}(\lamck)$.
  ...
  \trivial[h]{ Let
    $\Phi'\otimes \Phi_0\in \Coh_{[\lambda'_{\mathrm b}]}(G'_{\mathrm b})\otimes \Coh_{[\lambda_{\mathrm g}]}(G_{\mathrm g})$ be coherent family such that $\Phi'\otimes \Phi_0(\lamck) = \pi'\otimes \pi_0$. Now $\Ind_{P_{\mathrm b}}(\pi')\otimes \pi_{0} = (\Ind_{P_{\mathrm b}}\otimes \id) (\Phi'\otimes \Phi_{0})(\mu_{1})$ and $\Ind_{P}(\pi'\otimes \pi_{0}) = \ffcoh(\Ind_{P_{\mathrm b}}\otimes \id (\Phi'\otimes \Phi_{0}))(\lamck) = (\Ind_{P}^{G}) (\Phi'\otimes \Phi_{0})(\lamck)$}
...
\begin{proof}
  ...
  %Let $G_{1} = \Gb\times \Gg$.
  Let $\Psi_{1}$ be a basal element in
  $\Coh_{[\lamb]}(\CK(\Gb))\otimes \Coh_{[\lamg]}(\CK(\Gg))$ and $\Psi_{2}:=\varphi(\Psi_{1})$ such that
  $\pi_{2}:=\Psi_{2}(\lamck) \neq 0$.

  % \xrightarrow[\ \ \ \varphi  \ \ \ ]{\cong}
  Via the injective basal homomorphism \eqref{eq:cohinj}, we have $\braket{\Psi_{1}}\stackrel{\varphi}{\cong} \braket{\Psi_{2}}$.
  By \Cref{hcgoldie}, $\sigma_{\Ann(\pi_{1})} = \sigma_{\Ann(\pi_{2})}$ is the irreducible representation of $\WLamck$ having minimal fake degree
  occurring in $\braket{\Psi_{1}}\stackrel{\varphi}{\cong} \braket{\Psi_{2}}$.

  Let $I_{\lamck}$ and $I'_{\lamck}$ be the maximal primitive ideal in $\rU(\fgg)$
  and $\rU(\fgg_{1})$ with infinitesimal
  ...
  $\Ann(\pi_{2})\subseteq I_{\lamck}$ and $\Ann(\pi_{1})\subseteq I'_{\lamck}$.
  By \Cref{lem:ord},
  \[
\begin{split}
   & \AVC(\pi_{2})  \subseteq \bcO %\\
  \IFF  \Ann(\pi_{2})  = I_{\lamck} \\
  \IFF & \sigma_{\Ann(\pi_{2})}  = \sigma_{I_{\lamck}} %\\
  \IFF  \sigma_{\Ann(\pi_{1})}  = \sigma_{I'_{\lamck}}\\
  \IFF & \Ann(\pi_{1})  = I'_{\lamck} %\\
   \IFF \AVC(\pi_{1})  \subseteq  \bcOb\times \bcOg
\end{split}
  \]
