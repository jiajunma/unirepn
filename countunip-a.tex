...
%the associated variety of the maximal ideal $I_{\check \CO}$, and $\CO$ is the unique open $G_\C$-orbit in  $\overline{\CO}$.
...
In this subsection, we assume that  %$\ckstar \in \set{B,C,D}$, i.e
...
Suppose that $\star\in  \{A^\R, A^\BH, A, \widetilde A\}$ in this section.
Recall that  $W_{[\lamck]}  = \sfS_{\abs{\ckcO_{\mathrm g}}}\times \sfS_{\abs{\ckcO_{\mathrm b}}}$. It is easy to verify that
\begin{equation}\label{eq:WA}
  \begin{split}
      W_{\lamck} & = \prod_{i\in \bN^{+}}\sfS_{\bfcc_{i}(\ckcO_{\mathrm g})}\times \prod_{i\in \bN^{+}}\sfS_{\bfcc_{i}(\ckcO_{\mathrm b})},\\
    \LC_{\lamck} & = \set{\tau_{\lamck}}, \quad \textrm{where }  \tau_{\lamck} := (j_{W_{\lamck}}^{W_{[\lamck]}}\sgn )\otimes \sgn =  \ckcOg^{t}\otimes \ckcOb^{t}.
    %, \AND \\
...
%\subsection{Special Unipotent representations of $G=\GL_{n}(\bH)$}
...
[\tau_{\lamck}:\Cint{\lamck}(\CK(G))]  = \begin{cases}
   1,
    ...
    \end{cases}
  \]
This proves the theorem. % The rest is clear.
...
This proves the proposition in the case of  $\ngg = 0$.
  ...
  in \Cref{sec:explicitCoh}, the branching rules
  % of $\sfW_{n}$ in
  % \eqref{eq:CC.C} and
  \eqref{eq:indSW} and
...
% \[
% \sharp (\Unip_{\ckcO}(G))
%   \leq \sum_{\wp\in \bar{\mathrm A}(\ckcO)} [\tau_{\wp}: \Coh_{[\lambda_{\check \CO}]}(\CK'(G))].
%   \]
...
% Suppose that $\star=D^*$ so that $G=\oO^*(2n)$. Then $W_{[\lambda_{\check \CO}]} = \sfW'_{n}$, and Proposition \ref{prop:cohBCD44} implies that
% \[
% \Coh_{[\lambda_{\check \CO}]}(\CK'(G))\cong   \bigoplus_{\substack{2t+a=n}} \Ind_{\sfH_{t} \times \sfS_{a}}^{\sfW'_{n}}
%          \tilde{\varepsilon} \otimes \sgn.
%          \]
...
% \[
% \begin{split}
%  &[\tau_{\wp} : \Coh_{[\lambda_{\check \CO}]}(\CK'(G))]\\
%  = &
...
%   contributing $1$ to the multiplicity $[\tau_{\wp} : \Coh_{[\lambda_{\check \CO}]}(\CK'(G))]_{\sfW'_{n}}$.
% \end{proof}
 ...
 G_{\mathrm b} =
  \begin{cases}
   \SO(\nnb,\nnb+1), &\quad \text{if } \star =B; \\
    ...
    \SO(\nnb,\nnb), &\quad \text{if } \star \in\{C,D\}; \\
    % ( \Sp(p-\frac{n_{\mathrm b}}{2},q-\frac{n_{\mathrm b}}{2}), \Sp(n_{\mathrm b},n_{\mathrm b})), &\quad \text{if
    % } \star = C^{*}, \\
    \widetilde{\Sp}_{2\nnb}(\R), &\quad \text{if } \star =\wtC; \\
        % (\tSp(2n_{\mathrm b},\bR),\widetilde \Sp(2n_{\mathrm g},\bR)), &\quad \text{if
    % } \star = \wtC; \\
  \rO^{*}(2n_{\mathrm b}), &\quad  \text{if } \star \in\{C^*,  D^{*}\}. \\
  \end{cases}
\]
Note that $\ckGb$ is the Langlands dual of $\Gb$ and $\ckcOb$ is a nilpotent
orbit in $\Nil(\check \fgg_{\mathrm b})$.
...
% %   \]
  ...
  By the construction of $\pi'$  (\Cref{thm:mainR,thm:mainH}) and
...
\subsection{Matching coherent continuation representations}

Following \cite{GI}, we set
% $G'_{2}:=\Gpb \times \Gg$,
...
\[
\begin{split}
\sharp (\Unip_{\ckcO}(G))&\leq \sum_{\sigma\in \LC_{\ckcO}} [\sigma: \Coh_{[\lambda_{\check \CO}]}(\CK'(G))]\\
  & =\sum_{\wp\in \barA(\ckcO)} [\tau_{\mathrm b}\otimes \tau_{\wp}: \Coh_{[\lambda_{\check \CO}]}(\CK'(G))] \quad (\textrm{by Lemma \ref{lem:Lcell}}).
\end{split}
\]
In general, we will need to use the descriptions of $\tau_{\mathrm b}$ and $\tau_{\wp}$ (in \eqref{eq:taub} and \eqref{eq:tauwp}) to compute the multiplicities, using branching rules of Weyl groups in \eqref{eq:CC.C} and \eqref{eq:indSW} as well as \Cref{lem:WLcell}.
...
%   \[
%     \begin{split}
%       \abs{\Unip_{\ckcO}(G)} & = \sum_{\tau_{\mathrm b}\otimes \tau_{\mathrm g}\in \LC_{\ckcO}} [\tau_{\mathrm b}\otimes \tau_{\mathrm g}:\Coh_{[\lambda],\bcO}(\CK(G))\cap \Coh_{B_1}]\\
%     & \ \ + \sum_{\tau_{\mathrm b}\otimes \tau_{\mathrm g}\in \LC_{\ckcO}}[\tau_{\mathrm b}\otimes \tau_{\mathrm g}:\Coh_{[\lambda],\bcO}(\CK(G))\cap \Coh_{B_2}] \\
%   &= \sum_{\tau_{\mathrm b}\otimes \tau_{\mathrm g}\in \LC_{\ckcO}} [\tau_{\mathrm b}\otimes \tau_{\mathrm g}:
...
The key is to define the descent of a painted bipartition. As before, let  $\star\in \set{B, C,  D, \wtC,  C^*, D^*}$ and let $\check \CO$ be a Young diagram that has $\star$-good parity.
...
A painted bipartition $\uptau = (\imath,\cP)\times (\jmath,\cQ)\times \alpha$ now remembers where it come from.

\subsection{Tail of painted bipartition}
In this section, We assume $\star\in \set{B,C^{*}, D}$ and define the tail of
$\uptau$.
...
\trivial[h]{
In fact, we only need the proposition where $G$ running over all inner classes.
In such case, the proof is relatively simple.
