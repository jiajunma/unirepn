...
\subjclass[2000]{22E46, 22E47} \keywords{Special unipotent representation, associated variety, coherent continuation, primitive ideal, cell, classical group}
...
When no confusion is possible, for every object $O$ in an abelian category, we  still use $O$ to indicate the Grothendieck group element represented by the object $O$.
...
In view of \Cref{lem:lcell.BV0}, \eqref{leq2} and Theorem \ref{counteq} have the following consequence.
%nonumber  Combining \eqref{leq2}, \eqref{leq111} and  Propositions \ref{lem:lcell.BV0}, we obtain the following inequality.
 ...
 \begin{cor}
   ...
   \end{equation}
   holds. If for every Harish-Chandra cell representation $V$  in $\Coh_{\Lam}(\CK(G))$, the set $\{\sigma\in \Irr(W_{[\lambda]}\,|\, [\sigma: V]\neq 0\}$ is contained in a single double cell, then the equality holds in
   \eqref{boundc}.
 ...
 Let $\star$ be one of the 9 symbols
 \[
   \textrm{ $A$, $A^\bH$, $A^*$, $B$, $D$, $C$, $\wtC$,
     $D^*$, $C^*$. }
 \]
 ...
 \[
   \begin{array}{c}
     \GL_n(\R),  \  \GL_n(\bH),\  \oU(p,q),\smallskip\\
 ...
 \[
   \begin{array}{c}
     \GL_n(\C),  \  \GL_{2n}(\C),\  \GL_{p+q}(\C),\smallskip\\
...
Using the identification
 ...
 \ee
we view $\lambda_{\check \CO}$ as an element of $ W\backslash \hha^*$, and write $I_{\check \CO}:=I_{\star, \check \CO}$ for the maximal ideal of $\mathcal U(\g)$ with infinitesimal character $\lambda_{\check \CO}$. We remark that in the metaplectic case when $G=\widetilde \Sp_{2n}(\R)$, both $\hha$ and $\hha^*$ are  identified with $\C^n$ as usual and hence \eqref{sse} still holds.
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item when $S=\{c\}$ or $ \{d \}$, every column of $\imath$ has at most one
          box in $\CP^{-1}(S)$.
  \end{itemize}
A   painted Young diagram is a pair $(\imath, \CP)$ consisting of a Young diagram $\imath$ and a painting $\CP$ on it. 
\end{defn}
...
well-understood (see \cite[page 298]{ABV}). In particular, we have the following counting result for general linear groups.

\begin{thm}\label{GLcase}
 Suppose that  $\star\in\{A, A^\bH\}$. Then 
  \[
...
\[
  ...
  \end{cases}
\]
Here $ \widetilde{ \GL}_{l}(\bR)$ is the double cover of $ \GL_{l}(\bR)$ that fits to the following Cartesian diagram of Lie groups: 
...
\begin{thm}\label{reduction}
 If  $G$ has a closed subgroup isomorphic to $\Gpb$, then parabolic induction yields
  ...
  \begin{equation}\label{eq:IND}
    \begin{array}{rccc}
      \fI\colon &   \Unip_{\ckcO'_{b}}( G'_{b})\times \Unip_{\ckcO_{g}}( G_{g})&         \longrightarrow &\Unip_{\ckcO }(G) \\
...
\end{defn}
We remark that the set $\CPP_{\star}(\ckcO)$ appears implicitly in
...
In \cite{BMSZ2}, the authors have constructed $2^{\sharp(\CPP_\star(\check \CO))} \cdot \sharp (\PBP_{G}(\ckcO))$ number of representations in
...
In this subsection, we introduce  five more symbols $A^\C, B^\C,D^\C, C^\C$, and $\widetilde C^\C$, and let $\star$ be one of them. Let $G$ be a complex classical group of type $\star$, namely $G=\GL_n(\C)$, $\SO_{2n+1}(\C)$, $\SO_{2n}(\C)$, $\Sp_{2n}(\C)$, or   $\Sp_{2n}(\C)$ ($n\geq 0$), respectively. The Langlands dual $\check G$ of $G$ is respectively defined to be $\GL_n(\C)$, $\Sp_{2n}(\C)$, $\SO_{2n}(\C)$, $\SO_{2n+1}(\C)$, or   $\Sp_{2n}(\C)$. Let $\check \CO$ be a $\check G$ orbit in $\Nil(\check \g)$ where $\check \g$ is the Lie algebra of $\check G$. As in the real case we have a maximal ideal $I_{\check \CO}:=I_{\star, \check \CO}$ of $\CU(\g_0)$, where $\g_0$ is the Lie algebra of $G$ (which is a complex Lie algebra). 
 ...
 $\Ind_{P}^{G} 1_P$, where $P$ is the standard parabolic subgroup whose Levi subgroup equals 
 \[
 \GL_{\mathbf r_1(\check \CO)}(\C)\times \GL_{\mathbf r_2(\check \CO)}(\C)\times \dots \times \GL_{\mathbf r_{\mathbf c_1(\check \CO)}(\check \CO)}(\C),
 \]
  and $1_P$ denote the trivial representation of $P$.  See  \cite[page 298]{ABV} and \cite{V.GL}. 
...
\]
 ...
 Define the set $\CPP_\star(\ckcOg)$ as in the real case. Then by the works of Barbasch-Vogan and Barbasch (\cite{B17}), we have that 
   \[
    \sharp(\Unip_{\check \CO}(G))=\sharp(\Unip_{\ckcOg}(\Gg))=2^{\sharp(\CPP_\star(\ckcOg))} .
  \]
  Every representation in $\Unip_{\check \CO}(G)$ is obtained through irreducible parabolic induction as in the real case (Theorem \ref{reduction}), and every representation in $\Unip_{\ckcOg}(\Gg)$ is obtained through iterated theta lift (see \cite{B17} and \cite{BMSZ2}). We remark that the method of this paper and \cite{BMSZ2} works also in the complex case. 
  
  

Here are some words on the contents and the organization of this article. In Section 2, we give some generalities on the coherent continuation representation, which lead to the proofs of Theorems \ref{count1}, \ref{count2} and \ref{counteq}. The generalities include coherent continuation representations for highest weight modules, primitive ideals and Goldie rank polynomials, as well as cell representations in the coherent continuation setting. In Section 3, we give an explicit formula for the coherent continuation representation $\Coh_{\Lam}(\CK(G))$, which is a result of Barbasch and Vogan. Sections 4 to 7 are devoted to the main concern of the article, which is to give a precise count of special unipotent representations of all real classical groups, using results of Sections 2 and 3. We begin with the general linear groups, followed by the unitary groups, and then to real classical groups of type $\mathrm{BCD}$. All answers are given in terms of combinatorial constructs described earlier in the introductory section. It is worthwhile to note, while the algebraic theory developed in Sections 2 and 3 yields ultimately an upper bound of the count, we are unable to prove the precise count using the algebraic theory alone, due to a certain technical issue on the relationship of a Harish-Chandra cell and a Lusztig double cell, which we have formerly stated as Conjecture \ref{conjcell}. In the case at hand, namely for the real classical groups, we rely on the analytic theory of theta lifting to construct the right number of special unipotent representations (\cite{BMSZ2}), thus arriving at the precise count. It will be clearly desirable to demonstrate the precise count, without recourse to the analytic theory.
...
As before, we  define the subcategory $\Rep_\sfS(\g,H,\b)$ of $\Rep(\g,H,\b)$,  the Grothendieck groups $\CK(\g,H, \b)$ and $\CK_\sfS(\g,H, \b)$, and the coherent continuation representations $\Coh_{\Lam}(\CK(\g,H, \b))$ and  $\Coh_{\Lam}(\CK_\sfS(\g,H, \b))$.
...
Proposition 4.11 of \cite{Cas} implies that the functor $\gamma_{\n_i}^q$ commutes with tensor product with the finite-dimensional representations. Thus $\gamma_G$ is a $\mathcal R(\g)$-homomorphism. Finally, \cite[Corollary 4.15]{Cas} implies that $\gamma_G$ satisfies the property in \eqref{gammag}.
\end{proof}
...
Thus
\be\label{decLam}
 \Coh_{\Lam}( \CK(\g,\b))=\bigoplus_{w\in W/W_\Lam}\Coh_{\Lam}(\CK_{ w \cdot \Lam}(\g, \b) ).
\ee
...
Put
\[
   \Irr_\sfS^{\mathrm{sp}}(W_\Lam):=\{\sigma_0\in  \Irr^{\mathrm{sp}}(W_\Lam)\mid \mathrm{Spinger}^{-1}(j_{W_\Lam}^W \sigma_0)\subset \sfS\},
\]
...
Write $\widetilde \Irr_\Lam(H)$ for the set of all triples $(\b, \gamma, \nu)$ with the following properties:
\begin{itemize}
...
On the other hand, following \cite[Definition 8.3.1]{Vg}, we define the cross action of $W_\Lam$ on the set $ \widetilde \Irr_\Lam(H)$ by
\begin{eqnarray*}
  && w\cdot (\b, \gamma, \nu)\\
...
and $\chi$ is a character of $P$ trivial on the connected component of $P$. See \cite{V.GL}*{p450}. We will review their classifications in the framework of this article.

\medskip
% We assume that $G  = \GL_{n}(\bC), \GL_{n}(\bR), \GL_{\half n}(\bH)$ and $\star = A,A^{\bC}, A^{\bH}$ respectively.

We retain the notation of Sections \ref{sec:defunip}-\ref{secorgp0}.  
In this section, suppose that $\star\in \{A, A^\bH\}$ and $\ckG = \GL_{n}(\bC)$  ($n\geq 0$). Thus 
\[
  G= \begin{cases}
    \GL_{n}(\bR)  & \text{if } \star=A; \\
    \GL_{\frac{n}{2}}(\bH) & \text{if } \star =A^\bH. \\
  \end{cases}
\]
Note that $n$ has to be even in the last case. 
...
More precisely, for $ \cO \in \overline \Nil( \g)=\YD_{n}$, the Springer
...
We decompose $\ckcO$ into even and odd parts and write %$\ckcO\in \Nil(\ckGc)$ and decompose
\[
...
We list 
some easily verifiable facts: (see Section \ref{sec:intro} for the notation)
\begin{equation}\label{eq:WA}
  \begin{split}
    ...
    \wttau_{\lamck} & := j_{W_{[\lamck]}}^{W}\tau_{\lamck} = \cO.
  \end{split}
\end{equation}
...
In this section, we assume $\star=A$ so that $G = \GL_{n}(\bR)$.
...
The group $\sfW_n$ is naturally embedded in $\sfS_{2n}$. Note that $\epsilon $ is also the restriction of $\sgn$ of $S_{2n}$ to $W_n$.

The following branching formula is known (see \cite{BV.W}*{Lemma~4.1~(b)}):
...
\begin{lem} \label{lem:GL.count}
  ...
  \begin{equation}\label{eq:A.count}
    [\tau_{\lamck}: \Cint{\lamck}(\CK(G))] = \# (\PP_{A}(\ckcO_{e}))\times
...
\begin{proof}
  ...
  The multiplicity formula follows from \eqref{IndFor} and Pieri's rule, in view of the description of left cell representation $\tau_{\lamck}$ in \eqref{eq:WA}.  The last equality
...
where
\begin{itemize}
  \item $j$ runs over all column lengths in $\ckcO^t$,
...
\begin{thm}[c.f. \cite{ABV}*{Example~27.6}] Let $\star =A$ so that $G =\GL_n(\bR)$. Then the following map is a bijection
  \[
    \begin{array}{ccc}
      \PP_{A}(\ckcO) & \longrightarrow & \Unip_{\ckcO}(G)\\
      \uptau & \mapsto & \pi_{\uptau}.
    \end{array}
  \]
\end{thm}
\begin{proof}
  For the irreducibility of $\pi_{\uptau}$, see \cite{V.GL}.
  The map is injective since $\pi_{\uptau_{1}}$ and $\pi_{\uptau_{2}}$ have distinct cuspidal data if $\uptau_{1}\neq \uptau_{2}$.
  The map is bijective by \Cref{lem:GL.count}.
\end{proof}

\subsection{Special Unipotent representations of $G=\GL_{n}(\bH)$} In this section, we assume $\star=A^{\bH}$. We will let $G = \GL_{\frac{n}{2}}(\bH)$, where $n$ is even. Then
$\ckG = \GL_{n}(\bC)$ and we will freely tap notation of the two previous sections.
...
\begin{thm}
  ...
  If $\ckcO \neq \ckcO_e$, we have $\Unip_\ckcO(G) = \emptyset$. When $\ckcO = \ckcO_{e}$, we have
  \[
    \Unip_{\ckcO}(G) = \set{\pi_{\ckcO}}
  \]
  where
  ...
  \]
  $k$ is the number of rows of $\ckcO$
...
\end{thm}
\begin{proof} The assertion on the coherent continuation representation follows from Theorem \ref{thm:cohHC}. For the irreducibility of $\pi_{\ckcO}$, see \cite{V.GL}.
In view of the description of left cell representation $\tau_{\lamck}$ in \eqref{eq:WA}, it follows from \eqref{IndFor} that $[\tau_{\lamck}:\Cint{\lamck}(\CK(G))] = 0$ if $\ckcO \neq \ckcO_{e}$ and $[\tau_{\lamck}:\Cint{\lamck}(\CK(G))] = 1$ otherwise. The rest is clear.
...
\end{cases}
\]
where $\tU(p,q)$ is double cover of $\rU(p,q)$ defined by a square root of the determinant character. When it is convenient, we also use the symbol $U$ to represent either
$A^{*}$ or $\wtA^{*}$.
...
We call a number to have
\[
  \text{the good parity} = \begin{cases}
...
\]
where every row in $\ckcO_{g}$ (resp. $\ckcO_{b}$) has the good (resp. bad) parity.

Let $(N_g,N_b) = (\abs{\ckcO_g},\abs{\ckcO_b})$. We have
\begin{equation}\label{eq:LC.A}
  \begin{split}
    \WLamck &= \sfS_{N_g}\times \sfS_{N_b},\\
...
representations of $G$ with generalized infinitesimal character $\Lamck$, if $\star = \wtA^{*}$.
...
\[
 ...
 \cC_{b} &= \begin{cases}
  \Ind_{\sfW_{\frac{N_b}{2}}}^{\sfS_{N_b}} 1 & \text{if $N_b$ is even},\\
  0 & \text{otherwise}.
 \end{cases}
  \end{split}
\]
Then, as $\WLamck =\sfS_{N_g}\times \sfS_{N_b}$ modules,
\[
  \Coh_{\Lamck}(\CK(G)) \cong \begin{cases}
    \cC_{p-\half N_b,q-\half N_b}\otimes \cC_{b} & \text{if $N_b$ is even and
      $p,q \geq \half N_b$},\\
    0 & \text{otherwise}.
  \end{cases}
\]
...
Recall the branching formula (\cite{BV.W}*{Lemma~4.1~(b)}):
\begin{equation}\label{IndFor2}
  \Ind_{\sfW_{n}}^{\sfS_{2n}} 1 = \bigoplus_{\substack{\sigma\in \YD_{2n}\\
      \bfrr_{i}(\sigma) \text{ is even } \forall i\in \bN}} \sigma.
\end{equation}

\begin{thm} We retain the setting and notation of this section. Then
  \begin{enumerate}[label=(\alph*)]
    \item $\Unip_{G}(\ckcO)= \emptyset$ unless all the following conditions are
          satisfied
          \begin{itemize}
            \item[(i)] $N_b$ is even and $p,q \geq \half N_b$,
            \item[(ii)] each row length in $\ckcOb$ occurs in even multiplicity, i.e.
                  $\bfcc_{i}(\ckcOb)$ is even for every $i\in \bN^{+}$,
            \item[(iii)] and $p+q$ is even when $G = \wtU(p,q)$.
          \end{itemize}
    \item If all conditions in (a) are satisfied, let $\Gg = \rU(p-\half N_b,q-\half N_b)$ (if $\star = A^{*}$)
    or $\Gg = \tU(p-\half N_b,q-\half N_b)$ (if $\star = \wtA^{*}$), and view $\ckcO_{g}$ as an element in $\Nil(\check{\Gg})$. Then
                   \[
          \sharp(\Unip_{G}(\ckcO)) = \sharp(\Unip_{\Gg}(\ckcO_{g}).
          \]
    \item Let $\pi_{\ckcOpb}$ be the unique element in
          $\Unip_{\GL_{\half N_b}(\bC)}(\ckcOpb)$. Then we have a bijection
          \[
          \begin{array}{ccc}
...
\begin{proof} Recall from \eqref{eq:LC.A} the left cell representation
  $\tau_{\lamck} = \ckcO_{g}^{t}\boxtimes \ckcO_{b}^{t}$.
  It is clear that for $[\tau_{\lamck}:\Cint{\lamck}(G)]\ne 0$, we must have (i) by \Cref{lem:ccrU}, and also (ii) in view of \eqref{IndFor2}. When $G = \tU(p,q)$ and $p+q$ is odd, the good parity is even, and so $N_g$ is even, which forces $N_b$ to be odd. This proves (a).
  %There must be a row in $\ckcOb$ occurs in odd multiplicity, which implies $\Unip_{G}(\ckcO)=\emptyset$.
  For (b), we first observe that structure of Harish-Chandra cells for $G$ (see \cite[Theorem 5]{Bo}) implies an equality in the assertion of \Cref{cor:bound}. Thus
  \begin{equation}\label{eq:eqU}
     \sharp(\Unip_{G}(\ckcO)) = [\tau_{\lamck} : \Cint{\lamck}(G)].
  \end{equation}
  When all conditions in (a) are satisfied, we have $[\ckcO_{b}^{t}:\cC_{b}]=1$, in view of \eqref{IndFor2}. This proves (b). For (c), note that the given map is an injection and so a bijection by the counting assertion in (b).
\end{proof}
...
The above result tells us that the problem of construction of all special unipotent representations attached to $\ckcO$ is reduced to the case when $\ckcO = \ckcO_g$.
Now assume $\ckcO = \ckcO_g$.
% and so $\Cint{\ckcO}(G)$ corresponds to the blocks of the infinitesimal
% character of the trivial representation.
In this good parity setting, we shall state a counting result on
$\Unip_{G}(\ckcO)$ and sketch the construction of all elements of $\Unip_{G}(\ckcO)$ by the method of theta lifting.
...
\begin{thm} Suppose that $\ckcO = \ckcO_g$.
\begin{enumerate}[label*=(\alph*)]
\item We have
\[
    \sharp(\Unip_{\ckcO}(G))= \sharp \set{\CP\in \mathrm{PAP}_{U}(\ckcO)|(p_\CP, q_\CP)=(p,q)}=\sharp(\Nil_{G}(\cO)).
  \]
Here $\Nil_{G}(\cO)$ denotes the set of real nilpotent orbits in $G$ whose complexification is $\cO$.
\item The associated variety of every special unipotent representation in $\Unip_{\ckcO}(G)$ is irreducible. Moreover, the following map is a bijection:
\[
  \begin{array}{rcl}
  \Unip_{\ckcO}(G) &\longrightarrow& \Nil_{G}(\cO)\\
  \pi & \mapsto & \WF(\pi).
  \end{array}
\]
\end{enumerate}
...
\begin{proof} For (a), the first equality follows from \eqref{eq:eqU}, \eqref{IndFor2} and Pieri's rule. The second equality follows from the one-one
correspondence of Harish-Chandra cells in $\Cint{\ckcO}(G)$ with the real nilpotent orbits in $\Nil_{G}(\cO)$, by \cite{BV.W}*{Theorem~4.2} and \cite[Theorem 5]{Bo}.
(b) follows from (a) and \cite[Theorem 5]{Bo}.
\end{proof}
...
\begin{remark}
  ...
  a reducible associated variety. Nevertheless, it is easy to see that the map
  $\Unip_{\ckcO}(G) \ni \pi \mapsto \WF(\pi)$ is still injective.
\end{remark}

We are still in the setting that $\ckcO = \ckcO_g$. We will sketch the construction of all elements in $\Unip_{\ckcO}(G)$ by iterated theta lifting, as follows.
...
\end{cases}
\]
For $\ckcO\in \Nil(\check \g)$, we have the decomposition
\[
  \ckcO = \ckcO_g\cuprow \ckcO_b,
\]
where every row in $\ckcO_{g}$ (resp. $\ckcO_{b}$) has the good (resp. bad) parity. Note that bad parity rows occur with even multiplicity.
%We have $\ckcG_{\lamck} = \ckcG_{g}\otimes \ckcG_{b}$.


Recall also the Weyl group of type $B_n$ (or $C_n$) realized as $\sfW_n := \sfS_n \ltimes \set{\pm 1}^n$.
Let $\sfW'_n$ be the Weyl group of type $D_{n}$, realized as the kernel of the quadratic character $\epsilon$ of $\sfW_n$. Note that we have implicitly chosen a
  preferred embedding of $\sfS_{n}$ into $\sfW'_{n}$ via the root system of type $D_{n}$.


Let $n_{g}$ and $n_{b}$ be defined by
%be the rank of ${\check \g}_{g}$ and ${\check \g'}_{b}$, respectively:
  \begin{equation}\label{eq:ngnb}
    (n_g,n_b) =
    \begin{cases}
      (\half\abs{\ckcO_{g}}, \half \abs{\ckcO_{b}}) & \text{when
      } \star \in \set{B,\wtC,D,D^{*}},\\
      (\half(\abs{\ckcO_{g}}-1), \half \abs{\ckcO_{b}}) & \text{when
      } \star \in \set{C,C^{*}}.\\
          \end{cases}
  \end{equation}
 Then the integral Weyl group $\WLamck$ is a direct product:
  ...
  \begin{equation}\label{eq:Wbg}
    ...
    \Wg & := \begin{cases}
      ...
      \sfW'_{n_{g}} & \text{when } \star \in \set{\wtC,D,D^{*}},\end{cases}\\
      \Wb & := \begin{cases}
...
Given the chosen imbedding of $\sfS_{n}$ into $\sfW'_{n}$, we also let $(\tau_{L},\tau_{R})_{I}$ denote the unique irreducible character of $\sfW'_{n}$ given by
  ...
  \]
  as $\sfW'_{n}$ characters.


  \subsection{The left cells}
  \label{sec:LCBCD}
  In this subsection, we describe the Lusztig left cell
  %$\LC_{\lambda_{\ckcO}}$
  attached to $\lambda_{\ckcO}$ in each of the cases, where
  $\star \in \set{B,C,\wtC,C^{*},D,D^{*}}$. We make some definitions first.

  Define the irreducible $W_{b}$ representation $\tau_{b}$ attached to $\ckcO_{b}$ by the following formula:
  ...
  \begin{equation}\label{eq:tauwp}
    ...
    \end{cases}
  \end{equation}
  and $(\imathp, \jmathp)$ is given by the following formulas:
  \begin{itemize}
    ...
    \item Suppose $\star = \wtC$. Then for all $i\geq 1$,
          ...
          \end{cases}
          \]
          In this case, it is clear that $\tau_{\wp} = \tau_{\wp^{c}}$, where $\wp^{c}$ is the complement of $\wp$ in $\CPPs(\ckcO_{g})$.
  \end{itemize}

  %For $\wp\subset\CPPs(\ckcO_{g})$, let
  ...
  identifying $\wp$ with its complement $\wp^{c}$. Note that by its very definition, we have $\wtA(\ckcO)=\wtA(\ckcO_{g})$ and
...
\end{remark}
  ...
  \begin{lem}[c.f. Barbasch-Vogan {\cite{BVUni}*{Proposition~5.28}}]
    ...
    \[
      \begin{array}{lccccccc}
        \barA(\ckcO)&=&\barA(\ckcO_{g}) & \longrightarrow & \LC_{\ckcO_{g}}
        & \longrightarrow & \LC_{\ckcO}\\
                    &  &\wp & \mapsto & \tau_{\wp} &
                                                     \mapsto & \tau_{\wp}\otimes \tau_{b}.
      \end{array}
    \]
    Moreover,
    \[
      \tau_{\ckcO}:=\tau_{\emptyset}\otimes \tau_{b}
    \] is the unique special representation in $\LC_{\ckcO}$ and
    \begin{equation}\label{eq:dBV.W}
      \Spr ^{-1}(j_{\WLamck}^{W}(\tau_{\ckcO})) = \dBV(\ckcO) = \dBV(\ckcO_{g}) \cupcol \ckcO_{b}^{t}.
    \end{equation}
  ...
  \end{lem}
  \begin{proof}
    For $\ckcO_{g}$, the lemma is given in \cite{BVUni}*{Proposition~5.28}. In general, the lemma follows from an induction on number of columns of
    $\ckcO_{g}$ using Lusztig's formula of $J$-induction in \cite{Lu}*{\S 4.4-4.6}. The equality
    ...
    We first consider the good parity part $\ckcO_{g}$, where each row has
    even length.
    ...
    %   \cO_{g} = [2r_{1},2r_{2}, \cdots, 2r_{2k-1},2r_{2k}] = (C_{1},C_{1}, C_{2},C_{2},\cdots, C_{l}, C_{l}).
    % \]
    The Weyl group $W_g$ of good parity is $\sfW'_{m_{g}}$ with
    $m_{g} = \half\abs{\ckcO_{g}}$. For $1\leq k\leq l$, let
    \[
    ...
    Denote by $\ckLC_{\ckcO_{g}}$ the multiset of irreducible constituents of $\ckLV_{\ckcO_{g}}$.

    First assume $\CPPs(\ckcO_{g}) = \emptyset$. Define
    \[
      \tA'(\ckcO):= \bZ_{2}[\emptyset]
    \]
    to be the trivial group.
    Then $\ckLV_{\ckcO_{g}}$ is
    irreducible with the bipartition marked by label $I$.  Hence, $\LC_{\ckcO_{g}}$
    %$\LC_{\ckcO)}$
    ...
    \[
      \tA'(\ckcO):= \set{\wp\in \bZ_{2}[\CPPs(\ckcO_{g})]|(2i_{0}-1,2i_{0})\notin \wp} \longrightarrow \ckLC_{\ckcO_{g}}
    \]
    which records the bifurcation when attaching odd length columns. Again by Lusztig's formula of $J$-induction, a bijection is given by sending
    ...
    \[
      ...
      \end{cases}
    \]
    We thus obtain the parametrization of $\LC_{\ckcO_{g}}$ via $\tA'(\ckcO)$ by interchanging the two parts of all bipartitions belonging to $\ckLC_{\ckcO_{g}}$, which sends $\wp$ to $\tau_{\wp}$.
    ...
    \]
    where
    $ \cO'_{b} := (\ckcO'_{b})^{t}:=(\bfrr_{2}(\ckcO),\bfrr_{4}(\ckcO),\cdots , \bfrr_{2c_{0}}(\ckcO))$, and $\sfS_{\cO'_{b}} := \prod_{1\leq i\leq c_{0}}\sfS_{\bfrr_{2i}(\ckcO)}$. The proof for the first part is now complete.
    ...
    \]
    with $c = \min\set{i|\bfrr_{2i+1}(\ckcO)=0}$. The general case follows from the compatibility with parabolic induction.  \end{proof}
...
Recall from \eqref{eq:Wbg} that $\Wg =\sfW'_{n_{g}}$, when $\star \in \set{\wtC,D,D^{*}}$. Since the representation theory of $\sfW_{n}$ is more elementary than that of
$\sfW'_{n}$, we prefer to induce everything from $\sfW'_{n}$ to $\sfW_{n}$. Due to this, we also define
\begin{equation}\label{eq:ttauwp}
\wttau_{\wp} = (\imath_{\wp},\jmath_{\wp}),
\end{equation}
for any $\wp\in \tA(\ckcO)$. See \eqref{eq:tauwp} and below for the description of $\imath_{\wp}$ and $\jmath_{\wp}$.

For later use, we record the following lemma, which follows immediately from our explicit descriptions of the left cells.

\begin{lem}\label{lem:WLcell}
  Suppose $\star=\wtC$.  Then
  \[
    \Ind_{\sfW'_{n_{g}}\times \sfW_{n_{b}}}^{\sfW_{n_{g}}\times \sfW_{n_{b}}} \tau_{\wp}\boxtimes \tau_{b} =
    \begin{cases}
       \wttau_{\emptyset}\boxtimes \tau_{b} & \text{if } \tA(\ckcO) =\emptyset,\\
      \wttau_{\wp}\boxtimes \tau_{b} \oplus \wttau_{\wp^{c}}\boxtimes \tau_{b}
      &\text{otherwise}.
    \end{cases}
  \]
Let
\[
  \tLV_{\ckcO}:= \Ind_{\sfW'_{n_{g}}\times \sfW_{n_{b}}}^{\sfW_{n_{g}}\times \sfW_{n_{b}}}\LV_{\ckcO}
\]
and $\tLC_{\ckcO}$ be the set of irreducible constituents of the
$\sfW_{n_{g}}\times \sfW_{n_{b}}$-module $\tLV_{\ckcO}$. Then we have a
bijection:
  \[
      \begin{array}{lccccccc}
        ...
        & \longrightarrow & \tLC(\ckcO)\\
                  &  &\wp & \mapsto & \wttau_{\wp}
        & \mapsto & \wttau_{\wp}\otimes \tau_{b}.
      \end{array}
  \]
  Suppose $\star\in\set{D,D^{*}}$. Then
  \[
    \Ind_{\sfW'_{n_{g}}\times \sfW'_{n_{b}}}^{\sfW_{n_{g}}\times \sfW_{n_{b}}} \tau_{\wp}\boxtimes \tau_{b} =
    \begin{cases}
      \wttau_{b} & \text{if } n_{g}=0,\\
      \wttau_{\wp}\boxtimes \wttau_{b}  \oplus \wttau_{\wp}^{s}\boxtimes \wttau_{b}
      &\text{otherwise}.
    \end{cases}
  \]
  Here $\wttau_{\wp}^{s}:= \wttau_{\wp}\otimes \brsgn \neq \wttau_{\wp}$, and $\wttau_{b} = \Ind_{\sfW'_{n_{b}}}^{\sfW_{n_{b}}}\tau_{b}$.
...
We first recall some subgroups of Weyl groups and related branching rules.
...
\[
  ...
  \end{array}
\]
Here are the two formulas which are crucial for us (\cite{Mc}*{p220 (6)}):
...
Two other formulas which will be important for us are
\begin{equation}\label{eq:indSW}
\begin{split}
\Ind_{\sfS_{n}}^{\sfW_{n}}\sgn = \bigoplus_{a+b=n}\Ind_{\sfW_{a}\times \sfW_{b}}^{\sfW_{n}} \bsgn\boxtimes \sgn =\bigoplus_{a+b=n} ((a,),(b,)), \\
\Ind_{\sfS_{n}}^{\sfW_{n}} 1 = \bigoplus_{a+b=n}\Ind_{\sfW_{a}\times \sfW_{b}}^{\sfW_{n}} 1 \boxtimes \epsilon =\bigoplus_{a+b=n} ([a,],[b,]).
\end{split}
\end{equation}
...
Now assume $\rank_{\bC} \Gc = n$. Identify $\fhh^{*}$ with $\bC^{n}$, and let $Q$
...
\[
  ...
  \end{cases}
\]
For $n_{b}, n_{g}\in \bN$ such that $n_{b}+n_{g}=n$, we consider the lattice $\Lambda_{n_{b},n_{g}}=\lambda_{n_{b},n_{g}}+Q$, where
\[
  \lambda_{n_{b},n_{g}} =
  \begin{cases}
    (\underbrace{\half, \cdots, \half}_{n_{b}\text{-terms}}, \underbrace{0, \cdots, 0}_{n_{g}\text{-terms}})  & \text{when
    } \star\in \set{C,C^{*}, D,D^{*}}\\%\subset \fhh^{*}.
    (\underbrace{0, \cdots, 0}_{n_{b}\text{-terms}}, \underbrace{\half, \cdots, \half}_{n_{g}\text{-terms}})  & \text{when
...
\]
where $W_{b}$ and $W_{g}$ are defined in \eqref{eq:Wbg}.
...
\begin{prop}\label{prop:cohBCD}
  The coherent continuation representation $\Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))$ is zero unless $(p_g,q_g):= \Sign(G) - (n_{b},n_{b})\in \bZ^{\geq 0}\times \bZ^{\geq 0}$. When this is satisfied, $\Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))$ is isomorphic, as a $W_{\Lambda_{n_{b},n_{g}}}:= W_{b}\times W_{g}$-module, to the restriction to $W_{\Lambda_{n_{b},n_{g}}}$ of
  \[
    \begin{cases}
      \cC_{b}^{n_{b}}\boxtimes\cC_{g}^{p_g,q_g} & \text{if } \star \in \set{B,C,\wtC,C^{*},D},\\
      \Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW_{n_b,n_g}'}(\cC_{b}^{n_{b}}\boxtimes\cC_{g}^{p_g,q_g}) & \text{if } \star = D^{*}, \\
    \end{cases}
  \]
  where
  \[
    \sfW_{n_b,n_g}' := \left(\sfW_{n_{b}}\times \sfW_{n_{g}}\right)\cap \sfW'_{n_{g}+n_{b}} \quad \text{when $\star = D^{*}$.}
    %W'' := \left(\sfW_{n_{b}}\times \sfW_{n_{g}}\right)\cap \sfW'_{n_{g}+n_{b}} \quad \text{when $\star = D^{*}$.}
  \]
\end{prop}

\begin{remark}
  When $\star \in \set{C^{*},D^{*}}$ and $n_{b}$ is odd, we have in particular $\Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))=0$.
\end{remark}

\begin{proof}
%[Sketch of the proof]
  When $G$ is linear, the set of regular characters may be enumerated using
  \cite{AC}. When $G$ is the metaplectic group, the enumeration
  of regular characters is contained in Renard-Trapa's work \cite{RT1,RT2}. All assertions will then follow from \Cref{thm:cohHC}. See also
...
\begin{defn}
  Let $\PBPs(\ckcOb;\tau_{b})$ be the set of all pairs
...
We define the partition $\ckcOpb$ by $\ckcOb = 2\ckcOpb$. To be more precise, $\bfrr_{i}(\ckcO'_{b}):= \bfrr_{2i}(\ckcO_{b})$, for all $i\in \bN^{+}$. Let $\cO'_{b}:= (\ckcO'_{b})^{t}$.
  %\item $\cO_{b}:= \cO'_{b}\cupcol \cO'_{b}$.
 %$\ckcOpb$ is a partition of $\half\abs{\ckcOb}$.

We also define
\[
  G'_{b} := \begin{cases}
    \GL(n_b,\bR) & \text{when } \star \in \set{B,C,\wtC,D}, \\
    \GL(\half n_b,\bH) & \text{when } \star \in \set{C^{*},D^{*}}.\\
  \end{cases}
\]
...
\[
    \begin{split}
      \abs{\PBP_{\star}(\ckcO_{b};\tau_b)} = \abs{\PP_{G'_{b}}(\ckcO'_{b})} = \abs{\Unip_{\ckcO'_{b}}(G'_{b})},
    \end{split}
  \]
where $\PP_{G'_{b}}(\ckcO'_{b})$ is the set of painted partitions of type $A$ or $A^{\bH}$ attached to $\ckcO'_{b}$.
\end{prop}

\begin{proof} Suppose $\star \in \Set{C^{*},D^{*}}$. Then
\[
    \begin{split}
      \abs{\PBP_{\star}(\ckcO_{b};\tau_b)} = \abs{\PP_{A^{\bH}}(\ckcO'_{b})} = \abs{\Unip_{\ckcO'_{b}}(G'_{b})} = 1.
    \end{split}
\]
  Suppose $\star \in \Set{B,C,\wtC,D}$. Write $\tau_{b} = (\tau_{L,b},\tau_{R,b})$ and
  $\tau'_{b} = \cO'_{b}$. It is easy to see that we have a bijection:
  \[
    \begin{array}{ccc}
      \PBP_{\star}(\ckcO_{b};\tau _b) &  \longrightarrow & \PP_{A^{\bR}}(\ckcO'_{b})\\
...
and
\[
  \tPBP_{\star}(\ckcO_g) := %\bigsqcup_{\tau\in\LC(\ckcOg)}\PBP_{\star}(\tau).
  \bigsqcup_{\wp \subseteq \CPP(\ckcO_g)}\PBP_{\star}(\wttau_{\wp}),
\]
where $\wttau_{\wp} := (\imath_{\wp},\jmath_{\wp})$ (see \eqref{eq:ttauwp}). Similarly, define
\[
  \tPBP_{G_g}(\ast):= \Set{\uptau\in \tPBP_{\star}(\ast )|\Sign(\uptau)= \Sign(G_g)}, \quad  \quad
  \ast  = \ckcO_g \text{ or } \tau.
\]

Recall from \Cref{cor:bound}, we have the inequality 
\[
\begin{split} 
\sharp (\Unip_{\ckcO}(G))&\leq \sum_{\sigma\in \LC_{\ckcO}} [\sigma: \Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))]\\
  & =\sum_{\wp\in \barA(\ckcO)} [\tau_{b}\otimes \tau_{\wp}: \Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))] \quad (\textrm{by Lemma \ref{lem:Lcell}}).
\end{split}
\] 
...
\begin{prop}\label{prop:countBCD}
  ...
  % \]
  \[
  \sum_{\wp\in \barA(\ckcO)} [\tau_{b}\otimes \tau_{\wp}: \Coh_{\Lambda_{n_{b},n_{g}}}(\CK(G))] 
      = \sharp (\PBP_{\star}(\ckcO_{b};\tau _b))\cdot \sharp(\tPBP_{G_{g}}(\ckcO_{g})).
  \]
 In particular, when $\ckcO = \ckcO_g$, namely $\ckcO $ has good parity, we have
  \[
 ...
 \begin{remark} It will be shown in \Cref{prop:PBP} that $\sharp(\tPBP_{G}(\ckcO)) = 2^{\sharp(\CPPs(\ckcO))}\cdot \sharp(\PBP_{G}(\ckcO))$. Proposition \ref{prop:countBCD} will thus imply Theorem \ref{countup} in the introductory section.
 \end{remark}

\begin{proof} We use the descriptions of $\tau_{b}$ and $\tau_{\wp}$ (in \eqref{eq:taub} and \eqref{eq:tauwp}) to compute the multiplicities, using branching rules of Weyl groups in \eqref{eq:CC.C} and \eqref{eq:indSW} as well as \Cref{lem:WLcell}. We skip the details when $\star \in \set{B,\wtC, C,D,C^{*}}$, and present the computation for $\star = D^{*}$, which is the most complicated case.

  Recall that $(W_{b},W_{g}) = (\sfW'_{\nbb},\sfW'_{\ngg})$, where $n_b=\half \abs{\ckcO_{b}}$ and $n_g=\half\abs{\ckcO_{g}}$.

  First suppose $\ngg = 0$. Then $\tau_{b} = (\cOpb,\cOpb)_{I}$ and
  \[
    \begin{split}
      [\tau_{b}:\cC_{b}^{\nbb}] = &
  ...
  \begin{equation}\label{eq:W''}
    \Ind_{\sfW'_{\nbb}\times \sfW'_{\ngg}}^{\sfW_{n_b,n_g}'} \tau_{b}\boxtimes \tau_{\wp}
    = (\wttau_{b}\otimes \wttau_{\wp})|_{\sfW_{n_b,n_g}'}.
  \end{equation}
  ...
  For ease of notation, write $\sfW'':=\sfW_{n_b,n_g}'$. We then have
  \[
    \begin{split}
      & [\tau_{b}\boxtimes \tau_{\wp} :
      \Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW''} \cC_{b}^{\nbb} \boxtimes \cC_{g}^{p_g,q_g}]_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}\\
      = & [\Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW''} \tau_{b}\boxtimes \tau_{\wp} :
      \Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW''} \cC_{b}^{\nbb} \boxtimes \cC_{g}^{p_g,q_g}]_{\sfW''}\\
      = & [(\wttau_{b}\boxtimes \wttau_{\wp})|_{\sfW''}:
      \Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW''} \cC_{b}^{\nbb} \boxtimes \cC_{g}^{p_g,q_g}]_{\sfW''}\\
      = & [\wttau_{b}\boxtimes \wttau_{\wp}:
      \Ind_{\sfW'_{n_{b}}\times \sfW'_{n_{g}}}^{\sfW_{n_{b}}\times \sfW_{n_{g}}} \cC_{b}^{\nbb} \boxtimes \cC_{g}^{p_g,q_g}]_{\sfW_{n_{b}}\times \sfW_{n_{g}}}\\
       = & [\wttau_{b}:\Ind_{\sfW'_{n_{b}}}^{\sfW_{n_{b}}} \cC_{b}^{\nbb}]_{\sfW_{n_{b}}}\cdot
           [\wttau_{\wp}:\Ind_{\sfW'_{n_{g}}}^{\sfW_{n_{g}}} \cC_{g}^{p_g,q_g}]_{\sfW_{n_{g}}}\\
      =& \sharp(\PBP_{\star}(\ckcO_{b}; \tau _b)))\cdot \sharp(\PBP_{\star}(\wttau_{\wp})).
    \end{split}
  \]
  The last equality follows from the explicit description of $\cC_{b}^{\nbb}$ and $\cC_{g}^{p_g,q_g}$ in Section \ref{subsec:explicitCoh}, and
  the branching rules of $\sfW_{n}$, where the
  contribution of $\sfH_{t}$ corresponds to ``$\bullet$'',
  the contribution of $\sfS_{a}$ corresponds to ``$s$'' on the left and
  ``$r$'' on the right.
\end{proof}
...
\begin{lem}\label{lem:Unip.BP} We are in the setting of \Cref{prop:BP.PP}. Suppose that $n_{g}=0$.
\begin{enumerate}
\item Then we have a bijection
  \[
    \begin{array}{rccc}
      \fI_{b}: &\Unip_{\ckcO'_{b}}(G'_{b}) & \longrightarrow & \Unip_{\ckcO}(G) \\
      &\pi' & \mapsto & \pi:=\Ind_{P}^{G}\pi'
    \end{array}
  \]
  where $P$ is the standard parabolic subgroup of $G$ whose Levi component is
  isomorphic to $G'_{b}$.
 \item Denote by $\sO'$ the real nilpotent orbit in $G'_{b}$ such that $\sO'_{\bC}=\cO$, and let $\sO$ be the real induction of $\sO'$ to $G$.
...
\begin{proof} First note that $\WF(\pi')=[\sO']$.
  ...
  wavefront cycle of $\pi:=\Ind_{P}^{G}\pi'$ is $[\sO]$, i.e. with multiplicity one.
  ...
  Note that representations in $\Unip_{G'_{b}}(\ckcO'_{b})$ have distinct
  cuspidal data/Langlands parameters. This implies that $\Ind_{P}^{G}\pi'$ has distinct cuspidal data/Langlands parameters as $\pi'$ varies.
    Therefore, $\fI_{b}$ is injective. The bijection follows from the counting
  inequalities below:
  \[
    \abs{\PP_{\star'}(\ckcO'_{b})}=\abs{\Unip_{\ckcO'_{b}}(G'_b)}\leq \abs{\Unip_{\ckcO}(G)}
    \leq \abs{\PBP_{\star}(\ckcO_{b}; \tau_b)} = \abs{\PP_{\star'}(\ckcO'_{b})}.
  \]
  Here $\star' \in \set{A^{\bR},A^{\bC}, A^{\bH}}$ depending on $G$. The last equality holds because of \Cref{cor:bound} and \Cref{prop:cohBCD}.
\end{proof}
...
\[
  (G_{b},G_{g}) =
  \begin{cases}
    (\SO(n_{b},n_{b}+1),\SO(p-n_b,q-n_b)) & \text{when } \star = B, \\
    (\Sp(2n_{b},\bR),\Sp(2n_{g},\bR)) & \text{when } \star = C, \\
    (\Sp(n_{b},n_{b}), \Sp(p-\frac{n_b}{2},q-\frac{n_b}{2})) & \text{when } \star = C^{*}, \\
    (\Mp(2n_{b},\bR),\Mp(2n_{g},\bR)) & \text{when } \star = \wtC, \\
    (\rO^{*}(n_{b}), \rO^{*}(n_{g})) & \text{when } \star = D^{*}, \\
    (\SO(n_{b},n_{b}), \SO(p-n_b,q-n_b) )& \text{when } \star = D. \\
  \end{cases}
\]
The group $G'_b$ (defined earlier) is the Levi component of a Siegel parabolic subgroup $P_b$ of $G_{b}$.

Let $P$ be the standard parabolic subgroup of $G$ whose Levi component is isomorphic to $G'_{b}\times G_{g}$.
\begin{thm}\label{thm:red}
  There is a bijection
  \begin{equation}\label{eq:IND}
      \begin{array}{rccc}
    \fI\colon &   \Unip_{\ckcO'_{b}}(G'_b)\times \Unip_{\ckcO_{g}}(G_{g})&         \longrightarrow &\Unip_{\ckcO}(G) \\
     &   (\pi',\pi_{0}) & \mapsto & \pi'\rtimes \pi_{0}.
      \end{array}
    \end{equation}
...
\begin{lem}[{c. f. \cite{GI}*{Lemma~3.3}}]\label{lem:ff.irr} Suppose that
  \begin{enumerate}[label=(\roman*),series=KLff]
    \item \label{it:KLff.1} $G_{1}$ and $G_{2}$ are two real reductive groups in the Harish-Chandra class;
          \item there is an isomorphism
          \[ \ff\colon \fhhaso\rightarrow \fhhast
          \]
          of the duals of the abstract Cartan subalgebras of $G_{1}$ and $G_{2}$;
    ...
    \item \label{it:KLff.4} $\ff$ induces a bijection between
          $\Delta _{[\lambda_{1}]}$ and $\Delta _{[\lambda_{2}]}$, through which we identify $W_{[\lambda_{1}]}$ with $W_{[\lambda_{2}]}$;
  \end{enumerate}
  ...
  \]
  such that $\ffcoh_{\mu_{1}}(\pi)$ is irreducible if $\pi \in \ev{\mu_{1}}(\Coh_{1})$ is irreducible.
\end{lem}
\begin{proof}
...
\begin{enumerate}[KLff]
  ...
  \item there is an injection
        ...
        \]
        where $\cP_{\lambda}(G)$ denotes the set of $K$-conjugate classes of regular characters with infinitesimal character
        $\lambda$ \cite{Vg}, and $B\subseteq \cP_{\lambda_{1}}(G_{1})$ is a union of blocks.
  \item for $\gamma_{1}\in B$ and $\gamma_{2} = \ff(\gamma_{1})\in \cP_{\lambda_{2}}(G_{2})$ the
...
$\Phi_{\pi_{\gamma_{2}}}$ (see \Cref{sec:pfGeneral} for notation). In fact, %the above map on coherent continuation representations
$\ffcoh$ can be lifted to a map of Hecke algebra modules and the validity of
...
The map $\ff\colon B\rightarrow \cP_{\lambda}(G_{2})$ is given by the natural map between real Cartan subalgebras.
...
\begin{lem}\label{lem:BGcount}
  ...
  \[
    \abs{\Unip_{\ckcO}(G)} =
    \abs{\Unip_{\ckcO_{b}}(G_{b})}\cdot
    \abs{\Unip_{\ckcO_{g}}(G_{g})}.
  \]
\end{lem}
...
\begin{proof}
  ...
  $(\lambda_{b},\lambda_{g}):=\ff^{-1}(\lambda)$. We will require some more
  ...
  $\cP_{\lambda_{g}}(G_{g})$. By \Cref{counteq,lem:lcell.BV0}, this implies $\ffcoh$
  restricts to an isomorphism
  \[
    \Coh_{[\lambda_{b}],\bcO_{b}}(\CK(G_{g}))\otimes \Coh_{[\lambda_{g}],\bcO_{g}}(\CK(G_{b})) \xrightarrow{\ \ \ \ffcoh\ \ \ } \Coh_{[\lambda],\bcO}(\CK(G)).
  \]
  and the lemma follows by the evaluation at $\lamck$.

  Now consider the case where $\star = D^{*}$. In this case, $\cP_{\lambda}(G)$
  has $2$ blocks. Both $\cP_{\lambda_{b}}(G_{b})$ and
  $\cP_{\lambda_{g}}(G_{g})$ have only have $1$ block each. Let
  ...
  Then \[
    \begin{split}
      \Coh_{B_{1}} &\cong \cC_{b}^{\nbb}\otimes \cC_{g}^{p_g,q_g} = \Coh_{[\lambda_{b}],\bcO_{b}}(\CK(G_{g}))\otimes \Coh_{[\lambda_{g}],\bcO_{g}}(\CK(G_{b})),\\
      \Coh_{B_{2}} &\cong (\cC_{b}^{\nbb})^{s}\otimes (\cC_{g}^{p_g,q_g})^{s},
    \end{split}
  \]
   and
  \[
  ...
  and we have
  \[
    \Coh_{[\lambda_{b}],\bcO_{b}}(\CK(G_{g}))\otimes \Coh_{[\lambda_{g}],\bcO_{g}}(\CK(G_{b})) \xrightarrow{\ \ \ \ffcoh\ \ \ } \Coh_{[\lambda],\bcO}(\CK(G))\cap \Coh_{B_{1}}.
  \]

  Observe that $[\tau_{b}:(\cC_{b}^{\nbb})^{s}]=0$. Therefore
  \[
    \begin{split}
      \abs{\Unip_{\ckcO}(G)} & = \sum_{\tau_{b}\boxtimes \tau_{g}\in \LC_{\ckcO}} [\tau_{b}\otimes \tau_{g}:\Coh_{[\lambda],\bcO}(\CK(G))\cap \Coh_{B_1}]\\
    & \ \ + \sum_{\tau_{b}\boxtimes \tau_{g}\in \LC_{\ckcO}}[\tau_{b}\otimes \tau_{g}:\Coh_{[\lambda],\bcO}(\CK(G))\cap \Coh_{B_2}] \\
  &= \sum_{\tau_{b}\boxtimes \tau_{g}\in \LC_{\ckcO}} [\tau_{b}\otimes \tau_{g}:
  \Coh_{[\lambda_{b}],\bcO_{b}}(\CK(G_{b}))\otimes \Coh_{[\lambda_{g}],\bcO_{g}}(\CK(G_{g}))]\\
  &= \abs{\Unip_{\ckcO_{b}}(G_{b})}\cdot \abs{\Unip_{\ckcO_{g}}(G_{g})}.
\end{split}
\]
\end{proof}
...
\begin{proof}[Proof of \Cref{thm:red}]: We first prove the injectivity of $\fI$. Let % $\mu_{2} = \lamck$
  ...
  \[
    \begin{tikzcd}[column sep={4cm,between origins}]
      &  \Coh_{[\lambda'_{b}]}(\CK(G'_{b}))\otimes \Coh_{[\lambda_{g}]}(\CK(G_{g})) \ar[dl,"\Ind_{P_{b}}^{G_{b}}\otimes \id"']\ar[dr,"\Ind_{P}^{G}"]&\\
      \Coh_{[\lambda_{b}]}(\CK(G_{b}))\otimes \Coh_{[\lambda_{g}]}(\CK(G_{g})) \ar[d,"\ev{\mu_{1}}"'] \ar[rr,"\ffcoh"]& & \Coh_{[\lamck]}(\CK(G)) \ar[d,"\ev{\ckcO}"]\\
      \Grt_{\mu_{1}}(G_{b}\times G_{g}) \ar[rr,"\ffcoh_{\mu_{1}}"]& &
      \Grt_{\lamck}(G).\\
    \end{tikzcd}
  \]
  %Here $P_{b}$ is the parabolic subgroup of $G_{b}$ whose Levi component is isomorphic to $G'_{b}$ and
  %$P$ is the parabolic of $G$ whose Levi component is isomorphic to $G'_{b}\times G_{g}$.
  ...
  $\Ind_{P_{b}}^{G_b}(\pi')\otimes \pi_{0}$ to $\Ind_{P}^{G} (\pi'\otimes \pi_{0})$.
  \trivial[h]{ Let
...
The counting result of \Cref{lem:BGcount} will finally imply the bijectivity of $\fI$.
\end{proof}
...
\begin{prop} \label{prop:PBP} When $\star\in \set{C^{*}, D^{*}}$,
 ...
 Consequently we always have
  \[
    \sharp(\tPBP_{\star}(\ckcO)) = 2^{\sharp(\CPPs(\ckcO))}\cdot \sharp(\PBP_{\star}(\ckcO)), \,\, \text{ and } \,\, \sharp(\tPBP_{G}(\ckcO)) = 2^{\sharp(\CPPs(\ckcO))}\cdot \sharp(\PBP_{G}(\ckcO)).
  \]
\end{prop}
